// ==UserScript==
// @name		bro3_display_skill
// @namespace	https://gist.github.com/RAPT21/
// @description	ブラウザ三国志 武将スキル表示拡張 by RAPT
// @include		http://*.3gokushi.jp/*
// @exclude		http://*.3gokushi.jp/maintenance*
// @require		http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.min.js
// @grant		none
// @connect		3gokushi.jp
// @author		5zen + RAPT
// @version		5.7 (2020/05/05)
// ==/UserScript==

// 本スクリプトの最新版は下記から入手できます。
// https://github.com/RAPT21/bro3-tools

// 2015.08.16	1.0	beyond 3.0.74α (2015/06/28) から 5zen 氏原作のカード表示拡張機能のみ抜粋
// 2015.09.17	1.1	トレード出品画面で武将スキル表示対応
//					デッキ画面で合成スキルも表示対応
// 2015.09.20	1.2	合成画面で合成スキルも表示対応
//					合成時の合計LV329以上、合計スコア5040025以上のとき合成確率が正常に表示されない問題を修正
// 2015.09.27	1.3	Google Chrome だと合成画面で合成スキル表示時に、スキル LVUP ボタンが押せなくなる問題を修正
// 2015.09.30	1.4	カード合成時、カード情報が無くても合成リストがある場合は、合成スキル表示できるようにした。
//					ただし、ステ振りの自動検出はできないため確率ソートを行なわず、確率表示末尾に(!)を表示するようにした。
// 2015.10.04	1.5	Google Chrome でもデッキ画面で合成スキルも表示対応
//					デッキにセットする！等でカード裏面表示となりますが、再度クリックでセット可能です。
// 2015.10.14	1.6	一部合成レシピを追加
// 2015.10.25	1.7	一部合成レシピを追加
// 2015.10.28	1.8	一部合成レシピを追加,剣兵増強の合成レシピを修正
// 2015.11.02	1.9	一部合成レシピを追加,カード情報追加
// 2015.11.03	2.0	優姫の敬愛の合成レシピを修正,カード情報追加
// 2015.11.06	2.1	カード情報追加
// 2015.11.22	2.2	2015.11.21 のアップデートでブショーダス画面で合成スキル表示できなくなっていたのを修正、カード情報追加
// 2016.01.11	2.3	一部合成レシピを追加
// 2016.03.16	2.4	一部合成レシピを追加
// ★v1.2の影響で、Google Chrome だと合成時に素材カード選択画面でカード表面表示のとき選択できないバグがあります。カード裏面だと選択できます。修正予定。
// 2016.04.07	2.5	Google Chrome+Tampermonkey でスクリプトヘッダーに @connect が無いと警告が出る件の対応
// 2016.05.01	2.6 徳義為政の合成レシピの表示がずれる問題を修正,一部カード情報追加
// 2016.05.11	2.7	一部合成レシピを追加
// 2016.07.09	2.8	兵器速攻の合成レシピを修正,一部カード情報を追加,一部合成レシピを追加
// 2016.07.23	2.9	一部カード情報を追加,一部合成レシピを追加
// 2016.08.19	3.0	一部カード情報を追加,一部合成レシピを追加
// 2016.09.01	3.1	一部カード情報を追加,一部合成レシピを追加
// 2016.09.30	3.2	一部カード情報を追加,一部合成レシピを追加
// 2016.10.09	3.3	一部カード情報を追加,一部合成レシピを追加
// 2016.11.02	3.4	一部カード情報を追加,一部合成レシピを追加
// 2016.11.20	3.5	11/20新カード追加対応
// 2016.11.23	3.6	一部合成レシピを追加
// 2016.12.18	3.7	一部カード情報を追加,一部合成レシピを追加
// 2016.12.27	3.8	12/25新カード追加対応
// 2017.01.01	3.9	1/1新カード追加対応
// 2017.01.02	4.0	一部カード情報を追加
// 2017.01.12	4.1	1/11新カード追加対応(CardNo.1229,1230,1231,1232,1233)
// 2017.01.15	4.2	1/15新カード追加対応(CardNo.1235,2235)
// 2017.01.25	4.3	1/25新カード追加対応(CardNo.4382,4383,4384,4385,4386)
// 2017.02.02	4.4	副将設定画面で合成スキル表示対応,一部カード情報を追加
// 2017.02.15	4.5	新カード追加対応(CardNo.2236,3216,3217,3218,3219,3220,3221,4387,4388,4389,4390)
// 2017.03.20	4.6	新カード追加対応(CardNo.1235,1236,1237,2237,2238,2239,2240,2241,2242,2243,2244,2245,3222)
// 2017.04.26	4.7	新カード追加対応(CardNo.1238,1239,3223,4394,4395,4396,4397,4398,5050)
// 2017.05.20	4.8	新カード追加対応(CardNo.2246,2247,3224,4405,4406,4407)
// 2017.05.31	4.9	新カード追加対応(CardNo.1241,1242,2248,2249,3225,3226,3227,3228,4408)
// 2017.07.08	5.0	新カード追加対応(CardNo.1240,1243,1244,3229,4409,4410,4411,4412,5051,5052,5053,5054)
// 2017.10.14	5.1	新カード追加対応(CardNo.1245,1246,1247,1248,1249,1250,1251,1252,1253,2250,2251,2252,2253,2254,3230,3231,3232,3233,3234,3235,3236,3237,4352,4413,4414,4415,4416,4417,4418,4419,4420,4421,4422,4423,4424,4425,4426,4427,4428,4429,4430,4431,4432,5055,5056)
// 2018.02.19	5.2	新カード追加対応(CardNo.1254,1255,1256,1257,1258,1261,1262,1263,1264,1265,2255,2256,2257,2258,2259,2260,2261,2262,2263,2264,2266,2272,3238,3239,3242,3243,3244,4433,4434,4439,4440,4441,4442,4443,4444,4445,4446,4447,4448,4452,4453,4454,5057,5058,5059,5060)
// 2018.06.25	5.3	新カード追加対応(CardNo.1266,1267,1268,1269,1270,2265,2273,2274,2275,2276,2277,2278,2279,3246,3247,3248,4455,4456,4457,4458,4459,4460,4461,4462,4463,5061,5062,5063,5064)
// 2018.08.05	5.4	一部合成レシピを追加
// 2020.02.16	5.5	ブショーダスのラインナップを見るに対応、表示レイアウトを調整
// 2020.02.17	5.6	新カード追加対応、一部合成レシピを追加
// 2020.05.05	5.7	新カード追加対応、一部合成レシピを追加、デッキ画面でSLカードのスキル表示対応


//================================================
// 定数宣言一式
//================================================

// スキル合成表
const gousei_skill = {
	'剣兵の進撃':['剣兵の進撃', '剣兵防御', '剣兵行軍', '剣兵の強撃'],
	'槍兵の進撃':['槍兵の進撃', '槍兵防御', '槍兵行軍', '槍兵の強撃'],
	'弓兵の進撃':['弓兵の進撃', '弓兵防御', '弓兵行軍', '弓兵の強撃'],
	'騎兵の進撃':['騎兵の進撃', '騎兵防御', '騎兵行軍', '騎兵の強撃'],
	'兵器の進撃':['兵器訓練', '兵器防御', '兵器行軍', '兵器の強撃'],
	'剣兵の強撃':['剣兵の進撃', '剣兵防御', '剣兵強行', '剣兵の猛撃'],
	'槍兵の強撃':['火神の攻勢', '槍兵防御', '槍兵強行', '槍兵の猛撃'],
	'弓兵の強撃':['火神の攻勢', '弓兵防御', '弓兵強行', '弓兵の猛撃'],
	'騎兵の強撃':['火神の攻勢', '騎兵防御', '騎兵行軍', '騎兵の猛撃'],
	'兵器の強撃':['蛮族の襲撃', '兵器防御', '火神の攻勢', '兵器の猛撃'],
	'剣兵の猛撃':['剣兵の強撃', '剣兵堅守', '食糧技術', '剣兵の極撃'],
	'槍兵の猛撃':['槍兵の強撃', '槍兵堅守', '石切技術', '槍兵の極撃'],
	'弓兵の猛撃':['弓兵の強撃', '弓兵堅守', '伐採技術', '弓兵の極撃'],
	'騎兵の猛撃':['騎兵の強撃', '騎兵堅守', '製鉄技術', '騎兵の極撃'],
	'兵器の猛撃':['兵器の強撃', '英雄', '製鉄技術', '兵器の極撃'],
	'剣兵の極撃':['剣兵突撃', '剣兵方陣', '練兵修練', '剣兵突覇'],
	'槍兵の極撃':['槍兵突撃', '槍兵方陣', '兵舎修練', '槍兵突覇'],
	'弓兵の極撃':['弓兵突撃', '弓兵方陣', '弓兵修練', '弓兵突覇'],
	'騎兵の極撃':['騎兵突撃', '騎兵方陣', '厩舎修練', '騎兵突覇'],
	'兵器の極撃':['兵器の猛撃', '覇道', '加工知識', '深慮遠謀'],
	'剣兵の大極撃':['剣兵の極撃', '守護神', '剣将の采配', '暴君の覇道'],
	'槍兵の大極撃':['槍兵の極撃', '守護神', '槍将の采配', '槍戟鬼神'],
	'騎兵の大極撃':['騎兵の極撃', '騎兵の聖域', '騎将の采配', '戦蹄轟撃'],
	'弓兵の大極撃':['弓兵の極撃', '弓兵の聖域', '弓将の采配', '天弓雨撃'],
	'兵器の大極撃':['兵器の極撃', '守護神', '兵器修練', '兵器突覇'],
	'剣兵の大神撃':['剣兵の大極撃', '金剛不壊', '剣兵増強', '臥龍覚醒'],
	'槍兵の大神撃':['槍兵の大極撃', '金剛不壊', '槍兵増強', '昭烈帝'],
	'弓兵の大神撃':['弓兵の大極撃', '金剛不壊', '弓兵増強', '大皇帝'],
	'騎兵の大神撃':['騎兵の大極撃', '金剛不壊', '騎兵増強', '魏武王'],
	'兵器の大神撃':['兵器の大極撃', '金剛不壊', '攻城の檄文', '智将器撃'],
	'剣兵突撃':['剣兵強行', '剣兵堅守', '千里行', '剣兵の猛撃'],
	'槍兵突撃':['槍兵強行', '槍兵堅守', '千里行', '槍兵の猛撃'],
	'弓兵突撃':['弓兵強行', '弓兵堅守', '千里行', '弓兵の猛撃'],
	'騎兵突撃':['騎兵強行', '騎兵堅守', '千里行', '騎兵の猛撃'],
	'兵器突撃':['兵器強行', '剣兵堅守', '千里行', '剣兵の猛撃'],
	'剣兵突覇':['剣兵突撃', '練兵修練', '神速', '一騎当千'],
	'槍兵突覇':['槍兵突撃', '兵舎修練', '神速', '一騎当千'],
	'弓兵突覇':['弓兵突撃', '弓兵修練', '神速', '一騎当千'],
	'騎兵突覇':['騎兵突撃', '厩舎修練', '神速', '一騎当千'],
	'兵器突覇':['兵器突撃', '兵器訓練', '神速', '一騎当千'],
	'奇計百出':['伐採技術', '弓兵修練', '剣兵突覇', '守護神'],
	'神算鬼謀':['兵器の極撃', '八卦の陣', '覇道', '神医の術式'],
	'深慮遠謀':['兵舎修練', '王佐の才', '農林知識', '攻城の檄文'],
	'麒麟児':['槍兵の極撃', '一騎当千', '兵器の極撃', '神算鬼謀'],
	'混元一気':['騎兵の極撃', '王者の護り', '騎将の采配', '臥龍覚醒'],
	'連環の計':['深慮遠謀', '城壁補強', '槍兵増強', '兵器の極撃'],
	'蛮族の襲撃':['蛮族の襲撃', '兵器訓練', '製鉄知識', '蛮王の襲撃'],
	'蛮王の襲撃':['一騎当千', '兵舎修練', '兵器の猛撃', '太平要術'],
	'覇王の進撃':['騎兵の猛撃', '騎兵の聖域', '覇道', '飛将'],
	'覇王の強撃':['槍兵の猛撃', '槍兵の聖域', '千里行', '隣地猛攻'],
	'覇王の猛撃':['弓兵の猛撃', '弓兵の聖域', '神速', '遠地猛攻'],
	'覇王の極撃':['未実装', '未実装', '未実装', '未実装'],
	'喬姫の祈り':['覇王の進撃', '強襲突撃', '傾国', '軍神'],
	'白菊の祈り':['覇王の進撃', '強襲突撃', '傾国', '鬼神突貫'],
	'紅梅の祈り':['覇王の進撃', '闘将の守勢', '傾国', '鬼神の守勢'],
	'青藍の祈り':['覇王の進撃', '強襲突撃', '傾国', '食糧革命'],
	'喬姫の激励':['弓兵の極撃', '弓兵突覇', '弓兵の聖域', '弓将の采配'],
	'軍神':['一騎当千', '槍兵方陣', '兵舎修練', '飛将'],
	'武神':['一騎当千', '槍兵の聖域', '兵舎修練', '軍神'],
	'鬼神':['武神', '守護神', '兵舎修練', '神飛将'],
	'龍神':['武神', '金剛不壊', '兵舎修練', '鬼神'],
	'燕人武陣':['槍兵の極撃', '槍兵の聖域', '槍将の采配', '軍神'],
	'国士無双':['軍神', '城壁補強', '騎兵の勝鬨', '武神'],
	'剣神降臨':['万夫不当', '剣兵方陣', '剣兵突覇', '剣兵の強攻'],
	'槍神降臨':['万夫不当', '槍兵方陣', '槍兵突覇', '槍兵の強攻'],
	'弓神降臨':['万夫不当', '弓兵方陣', '弓兵突覇', '弓兵の強攻'],
	'騎神降臨':['万夫不当', '騎兵方陣', '騎兵突覇', '騎兵の強攻'],
	'英雄':['騎兵の強撃', '騎兵堅守', '奇計百出', '覇道'],
	'覇道':['豪傑', '鉄壁', '千里行', '兵器の極撃'],
	'剛将の畏怖':['一騎当千', '鉄壁', '騎兵強行', '覇道'],
	'太平要術':['兵器の極撃', '槍兵の聖域', '農林技術', '深慮遠謀'],
	'猛虎進撃':['槍兵突覇', '八卦の陣', '急速援護', '槍兵の極撃'],
	'魏王の号令':['騎兵突覇', '騎兵の聖域', '厩舎修練', '富国'],
	'魏王の覇軍':['騎兵突覇', '王者の護り', '覇王の進撃', '魏武王'],
	'暴虐外道':['覇王の進撃', '守護神', '攻城の檄文', '猛将の鹵獲'],
	'暴君の覇道':['猛将の鹵獲', '八卦の陣', '覇道', '暴虐外道'],
	'槍戟鬼神':['槍兵の極撃', '覇道', '千里行', '槍将の采配'],
	'天弓雨撃':['弓兵の極撃', '千里行', '覇道', '弓将の采配'],
	'戦蹄轟撃':['騎兵の極撃', '覇道', '千里行', '騎将の采配'],
	'孫呉の烈火':['大皇帝', '地の利堅壁', '弓将の采配', '神医の施術'],
	'白馬将':['拠点襲撃', '戦蹄轟撃', '騎兵増強', '飛蹄進軍'],
	'精鋭の進撃':['精鋭の進撃', '兵器防御', '神速', '奇計百出'],
	'臥龍覚醒':['市場繁栄', '八卦の陣', '神算鬼謀', '神医の術式'],
	'醒龍出師':['太平要術', '八卦の陣', '神算鬼謀', '弓兵の勝鬨'],
	'昭烈帝':['槍兵突覇', '王者の護り', '軍神', '神算鬼謀'],
	'大皇帝':['弓兵突覇', '覇王の進撃', '富国', '神医の術式'],
	'魏武王':['騎兵突覇', '城壁補強', '豊穣', '飛将'],
	'蜀軍の極撃':['槍兵の極撃', '富国', '王者の護り', '神算鬼謀'],
	'魏軍の極撃':['騎兵の極撃', '豊穣', '騎兵の聖域', '軍神'],
	'呉軍の極撃':['弓兵の極撃', '加工技術', '守護神', '強兵の檄文'],
	'群雄の極撃':['太平要術', '農林技術', '八卦の陣', '神医の術式'],
	'強襲速撃':['迅速援護', '弓兵堅守', '槍兵の強撃', '奇計百出'],
	'強襲突撃':['伐採技術', '剣兵の聖域', '槍兵方陣', '強兵の檄文'],
	'強襲突覇':['神速', '鉄壁', '八卦の陣', '攻城の檄文'],
	'強襲烈覇':['強襲突覇', '守護神', '王者の護り', '強兵の檄文'],
	'強襲大烈覇':['強襲烈覇', '金剛不壊', '忠節不落', '攻城の義勇軍'],
	'強襲奮迅':['蛮王の襲撃', '王者の護り', '豊潤祈祷', '奇襲'],
	'胡王の猛襲':['蛮王の襲撃', '兵器強行', '覇道', '強襲突覇'],
	'火神の攻勢':['石切知識', '蛮族の襲撃', '練兵訓練', '兵器の強撃'],
	'蛮勇の一撃':['拠点襲撃', '地の利堅壁', '農林技術', '槍兵の極撃'],
	'胡王奮迅':['天弓雨撃', '火神の攻勢', '弓兵の聖域', '奇襲'],
	'趁火打劫':['豪傑', '厩舎訓練', '鉄壁', '奇計百出'],
	'桃賊の襲撃':['趁火打劫', '呉の治世', '恵風', '桃色吐息'],
	'猛将の鹵獲':['一騎当千', '守護神', '厩舎修練', '深慮遠謀'],
	'鬼神の鹵獲':['万夫不当', '金剛不壊', '強兵の檄文', '麒麟児'],
	'劉備の大徳':['昭烈帝', '槍兵の聖域', '神医の術式', '醒龍出師'],
	'迅速劫略':['槍兵突覇', '神速援護', '千里行', '猛将の鹵獲'],
	'神速劫略':['発憤興起', '神速', '趁火打劫', '迅速劫略'],
	'豪傑':['剣兵の強撃', '蛮族の襲撃', '槍兵強行', '英雄'],
	'一騎当千':['槍兵突覇', '鉄壁', '加工知識', '魏王の号令'],
	'万夫不当':['騎兵突覇', '神速', '製鉄技術', '騎兵の極撃'],
	'飛将':['一騎当千', '鉄壁', '厩舎修練', '覇王の進撃'],
	'神飛将':['騎兵の極撃', '騎兵の聖域', '神速', '飛将'],
	'無双':['万夫不当', '金剛不壊', '万里行', '覇王の強撃'],
	'隣地猛攻':['一騎当千', '覇道', '強兵の檄文', '強襲突覇'],
	'遠地猛攻':['万夫不当', '覇道', '攻城の檄文', '隣地猛攻'],
	'燕神破軍':['槍兵の大極撃', '守護神', '兵舎修練', '燕人武陣'],
	'龍鱗猛攻':['万夫不当', '覇道', '攻城の檄文', '隣地猛攻'],
	'武将突貫':['豪傑', '鉄壁', '千里行', '一騎当千'],
	'闘将突貫':['一騎当千', '八卦の陣', '神速', '万夫不当'],
	'猛将突貫':['万夫不当', '守護神', '騎兵突覇', '軍神'],
	'隻眼将の軍略':['騎兵の猛撃', '神速', '加工技術', '飛将'],
	'若獅子の軍略':['騎兵突覇', '神速', '加工技術', '鬼神の極意'],
	'苦肉の計':['兵器の極撃', '兵器強行', '兵器修練', '飛将'],
	'皇叔の号令':['昭烈帝', '王者の護り', '豊穣', '神医の術式'],
	'攻城戦の極砕':['兵器の極撃', '兵器修練', '豊穣', '智将砕器'],
	'攻城戦の大神砕':['兵器の大極撃', '兵器の大号令', '豊潤祈祷', '兵器の大神撃'],
	'攻城戦の猛砕':['兵器の猛撃', '兵器訓練', '恵風', '兵器の極撃'],
	'攻城戦の極攻':['兵器の強攻', '金剛不壊', '兵器修練', '兵器の猛攻'],
	'攻城戦の大極攻':['兵器の猛攻', '護君', '兵器増強', '兵器の極攻'],
	'槍兵の大極攻':['槍兵の猛攻', '槍兵の大聖域', '槍兵増強', '槍兵の極攻'],
	'剣兵の大極攻':['剣兵の猛攻', '剣兵の大聖域', '剣兵増強', '剣兵の極攻'],
	'弓兵の大極攻':['弓兵の猛攻', '弓兵の大聖域', '弓兵増強', '弓兵の極攻'],
	'騎兵の大極攻':['騎兵の猛攻', '騎兵の大聖域', '騎兵増強', '騎兵の極攻'],
	'兵器の大極攻':['兵器の猛攻', '忠節不落', '兵器増強', '兵器の極攻'],
	'謀反の大極攻':['謀反の猛攻', '忠節不落', '飛速', '暴君の覇道'],
	'陥陣営の侵攻':['兵器の猛撃', '兵器強行', '恵風', '攻城の檄文'],
	'智将器撃':['兵器の極撃', '兵器強行', '製鉄技術', '神算鬼謀'],
	'智将砕器':['神算鬼謀', '王者の護り', '智将器撃', '混元一気'],
	'智将天砕器':['混元一気', '忠節不落', '智将砕器', '智将の大神撃'],
	'隣地攻城':['兵器の強撃', '城壁補強', '攻城の檄文', '兵器突覇'],
	'守将の出陣':['槍兵方陣', '守護神', '八卦の陣', '槍兵の聖域'],
	'守将の進軍':['兵舎訓練', '加工知識', '騎兵の聖域', '槍兵の極撃'],
	'飛蹄進軍':['騎兵突覇', '騎兵の聖域', '神速', '騎将の采配'],
	'神謀風如':['弓兵の極撃', '王者の護り', '弓兵修練', '勇将飛矢'],
	'急襲':['一騎当千', '守護神', '千里行', '強襲突撃'],
	'奇襲':['猛将の鹵獲', '強襲突覇', '騎将の采配', '飛蹄進軍'],
	'刹那の奇襲':['急襲', '千里行', '厩舎訓練', '弓兵の猛攻'],
	'勇美鼓舞':['一騎当千', '守護神', '富国', '傾国'],
	'麒麟慈心':['万夫不当', '槍兵の聖域', '富国', '槍兵の大極撃'],
	'勇将飛矢':['弓兵の極撃', '弓兵の聖域', '弓兵修練', '覇王の進撃'],
	'勇将天戟':['槍兵の極撃', '槍兵の聖域', '兵舎修練', '覇王の進撃'],
	'拠点襲撃':['槍戟鬼神', '槍兵の聖域', '兵舎修練', '槍兵の大極撃'],
	'剣兵の進攻':['剣兵の進攻', '剣兵防御', '剣兵強行', '槍兵の強撃'],
	'槍兵の進攻':['槍兵の進攻', '槍兵防御', '槍兵強行', '槍兵の強撃'],
	'弓兵の進攻':['弓兵の進攻', '弓兵防御', '弓兵強行', '弓兵の強撃'],
	'騎兵の進攻':['騎兵の進攻', '騎兵防御', '騎兵強行', '騎兵の強撃'],
	'兵器の進攻':['兵器の進攻', '鉄壁', '兵器強行', '兵器の強撃'],
	'剣兵の強攻':['剣兵の強撃', '剣兵防御', '剣兵行軍', '剣兵の強攻'],
	'槍兵の強攻':['槍兵の強撃', '槍兵防御', '槍兵行軍', '槍兵の強攻 '],
	'弓兵の強攻':['弓兵の強撃', '弓兵防御', '弓兵行軍', '弓兵の強攻 '],
	'騎兵の強攻':['騎兵の強撃', '騎兵防御', '騎兵行軍', '騎兵の強攻 '],
	'兵器の強攻':['兵器の強撃', '守護神', '兵器行軍', '兵器の強攻'],
	'剣兵の猛攻':['剣兵の強攻', '剣兵堅守', '食糧技術', '剣兵の猛撃'],
	'槍兵の猛攻':['槍兵の強攻', '槍兵堅守', '石切技術', '槍兵の猛撃'],
	'弓兵の猛攻':['弓兵の強攻', '弓兵堅守', '伐採技術', '弓兵の猛撃'],
	'騎兵の猛攻':['騎兵の強攻', '騎兵堅守', '製鉄技術', '騎兵の猛撃'],
	'兵器の猛攻':['兵器の強攻', '八卦の陣', '食糧技術', '兵器の猛撃'],
	'剣兵の極攻':['剣兵の猛撃', '剣兵の聖域', '練兵修練', '剣兵の猛攻'],
	'槍兵の極攻':['槍兵の猛撃', '槍兵の聖域', '兵舎修練', '槍兵の猛攻'],
	'弓兵の極攻':['弓兵の猛撃', '弓兵の聖域', '弓兵修練', '弓兵の猛攻'],
	'騎兵の極攻':['騎兵の猛撃', '騎兵の聖域', '厩舎修練', '騎兵の猛攻'],
	'兵器の極攻':['兵器の猛撃', '王者の護り', '兵器修練', '兵器の猛攻'],
	'剣兵速攻':['剣兵突撃', '剣兵強行', '練兵訓練', '剣兵突覇'],
	'槍兵速攻':['槍兵突撃', '槍兵強行', '兵舎訓練', '騎兵突覇'],
	'弓兵速攻':['弓兵突撃', '弓兵強行', '弓兵訓練', '弓兵突覇'],
	'騎兵速攻':['騎兵突撃', '騎兵強行', '厩舎訓練', '騎兵突覇'],
	'兵器速攻':['兵器突撃', '兵器強行', '兵器訓練', '兵器突覇'],
	'剣兵突攻':['剣兵突覇', '神速', '練兵修練', '剣兵速攻'],
	'槍兵突攻':['槍兵突覇', '神速', '兵舎修練', '槍兵速攻'],
	'弓兵突攻':['弓兵突覇', '神速', '弓兵修練', '弓兵速攻'],
	'騎兵突攻':['騎兵突覇', '神速', '厩舎修練', '騎兵速攻'],
	'兵器突攻':['兵器突覇', '神速', '兵器修練', '兵器速攻'],
	'鹵獲の強攻':['迅速劫略', '八卦の陣', '神速', '鹵獲の強攻'],
	'鹵獲の猛攻':['鹵獲の強攻', '八卦の陣', '迅速援護', '猛将の鹵獲'],
	'鹵獲の極攻':['猛将の鹵獲', '王者の護り', '神速援護', '鹵獲の猛攻'],
	'覇王の強攻':['覇王の強撃', '守護神', '神速', '覇王の強攻'],
	'覇王の猛攻':['覇王の強攻', '八卦の陣', '食糧技術', '覇王の猛撃'],
	'老将統帥':['隣地猛攻', '守護神', '弓兵増強', '勇将飛矢'],
	'武将の極意':['豪傑', '鉄壁', '急速援護', '一騎当千'],
	'闘将の極意':['一騎当千', '八卦の陣', '千里行', '武将の極意'],
	'猛将の極意':['万夫不当', '守護神', '神速', '闘将の極意'],
	'猛士の極意':['万夫不当', '守護神', '神速', '闘将の極意'],
	'鬼神の極意':['隣地猛攻', '王者の護り', '迅速擁護', '闘将の極意'],
	'戦神の極意':['隣地猛攻', '王者の護り', '迅速援護', '猛将の極意'],
	'猛暑の極意':['万夫不当', '守護神', '神速', '闘将の極意'],
	'天下布武':['万夫不当', '守護神', '神速', '闘将の極意'],
	'隣地豪襲':['万夫不当', '謀反の進攻', '攻城の檄文', '闘将の極意'],
	'隣地豪撃':['万夫不当', '謀反の進攻', '攻城の檄文', '闘将の極意'],
	'隣地極攻':['万夫不当', '剛将の畏怖', '万里行', '強襲烈覇'],
	'隣地猛襲':['武将の極意', '覇道', '練兵修練', '万夫不当'],
	'蜀将の督戦':['槍兵の極撃', '槍兵の聖域', '石切技術', '神算鬼謀'],
	'呉将の督戦':['弓兵の極撃', '弓兵の聖域', '伐採技術', '覇王の進撃'],
	'魏将の督戦':['騎兵の極撃', '騎兵の聖域', '製鉄技術', '軍神'],
	'群将の督戦':['兵器の極撃', '王者の護り', '食糧技術', '虐帝の号令'],
	'謀反の進攻':['謀反の進攻', '鉄壁', '千里行', '剣兵の強撃'],
	'謀反の強攻':['英雄', '守護神', '神速', '謀反の強攻'],
	'謀反の猛攻':['謀反の強攻', '金剛不壊', '迅速援護', '太平要術'],
	'謀反の極攻':['暴虐外道', '金剛不壊', '神速援護', '謀反の猛攻'],
	'謀反の突攻':['覇道', '守護神', '呉の治世', '謀反の速攻'],
	'謀反の速攻':['英雄', '鉄壁', '王佐の才', '覇道'],
	'背水陣の極攻':['万夫不当', '金剛不壊', '千里行', '飛将'],
	'弓襲遠攻':['弓兵の極撃', '千里雷光', '弓兵訓練', '勇将飛矢'],
	'守神の進撃':['守将の進軍', '守護神', '槍将の采配', '槍兵の大極撃'],
	'剣兵行軍':['剣兵行軍', '剣兵防御', '剣兵の進撃', '神速'],
	'槍兵行軍':['槍兵行軍', '槍兵防御', '槍兵の進撃', '神速'],
	'弓兵行軍':['弓兵行軍', '弓兵防御', '弓兵の進撃', '神速'],
	'騎兵行軍':['騎兵行軍', '騎兵防御', '騎兵の進撃', '神速'],
	'兵器行軍':['兵器行軍', '食糧知識', '兵器訓練', '兵器強行'],
	'剣兵強行':['剣兵強行', '剣兵防御', '剣兵の進撃', '剣兵の猛撃'],
	'槍兵強行':['槍兵強行', '槍兵防御', '槍兵の進撃', '槍兵の猛撃'],
	'弓兵強行':['弓兵強行', '弓兵防御', '弓兵の進撃', '八卦の陣'],
	'騎兵強行':['騎兵強行', '騎兵防御', '騎兵の進撃', '一騎当千'],
	'兵器強行':['兵器強行', '食糧技術', '兵器修練', '蛮王の襲撃'],
	'千里行':['兵器強行', '騎兵方陣', '食糧技術', '神速'],
	'万里行':['兵器突撃', '騎兵の聖域', '伐採技術', '迅速援護'],
	'天里行':['兵器突覇', '騎将の采配', '市場繁栄', '神速援護'],
	'神速':['一騎当千', '英雄', '農林知識', '仁君'],
	'烈速':['強襲突撃', '覇道', '加工知識', '闘将突貫'],
	'飛速':['強襲突覇', '太平要術', '豊穣', '猛将突貫'],
	'急速援護':['千里行', '弓兵強行', '剣兵強行', '練兵修練'],
	'迅速援護':['強襲速撃', '兵器強行', '英雄', '加工技術'],
	'神速援護':['強襲突撃', '神速', '覇道', '富国'],
	'戦女の行軍':['強襲突覇', '恵風', '神速', '急襲'],
	'神速雷光':['神速', '剣兵方陣', '深慮遠謀', '烈速'],
	'烈速雷光':['神速雷光', '剣兵方陣', '深慮遠謀', '烈速'],
	'飛速雷光':['烈速', '剣兵の聖域', '神算鬼謀', '烈速雷光'],
	'鳳速雷光':['烈速雷光', '剣将の采配', '混元一気', '飛速'],
	'神速行勢':['神速雷光', '剣兵方陣', '深慮遠謀', '烈速'],
	'千里雷光':['千里行', '騎兵行軍', '密偵召集', '千里雷光'],
	'万里雷光':['万里行', '強襲突覇', '密偵精鋭', '万里雷光'],
	'天里雷光':['万里行', '強襲突覇', '密偵精鋭', '万里雷光'],
	'凰里雷光':['万里雷光', '強襲烈覇', '市場知識', '天里行'],
	'剣兵防御':['食糧知識', '剣兵の進撃', '剣兵行軍', '剣兵堅守'],
	'槍兵防御':['石切知識', '槍兵の進撃', '槍兵行軍', '槍兵堅守'],
	'弓兵防御':['伐採知識', '弓兵の進撃', '弓兵行軍', '弓兵堅守'],
	'騎兵防御':['製鉄知識', '騎兵の進撃', '騎兵行軍', '騎兵堅守'],
	'兵器防御':['製鉄知識', '兵器の進撃', '兵器行軍', '兵器防御'],
	'剣兵堅守':['食料知識', '剣兵の進撃', '剣兵強行', '剣兵方陣'],
	'槍兵堅守':['石切知識', '槍兵の進撃', '槍兵強行', '槍兵方陣'],
	'弓兵堅守':['弓兵防御', '弓兵の進撃', '弓兵強行', '弓兵方陣'],
	'騎兵堅守':['騎兵防御', '騎兵の進撃', '騎兵強行', '騎兵方陣'],
	'剣兵方陣':['剣兵堅守', '剣兵の強撃', '剣兵強行', '剣兵の聖域'],
	'槍兵方陣':['槍兵堅守', '槍兵の強撃', '槍兵強行', '槍兵の聖域'],
	'弓兵方陣':['弓兵堅守', '弓兵の強撃', '弓兵強行', '弓兵の聖域'],
	'騎兵方陣':['騎兵堅守', '騎兵の強撃', '騎兵強行', '騎兵の聖域'],
	'剣兵の聖域':['剣兵方陣', '練兵修練', '剣兵強行', '八卦の陣'],
	'槍兵の聖域':['槍兵方陣', '兵舎修練', '槍兵強行', '八卦の陣'],
	'弓兵の聖域':['弓兵方陣', '弓兵修練', '弓兵強行', '八卦の陣'],
	'騎兵の聖域':['騎兵方陣', '厩舎修練', '騎兵強行', '八卦の陣'],
	'剣兵の大聖域':['剣兵の聖域', '剣兵増強', '剣兵強行', '王者の護り'],
	'槍兵の大聖域':['槍兵の聖域', '槍兵増強', '槍兵強行', '王者の護り'],
	'弓兵の大聖域':['弓兵の聖域', '弓兵増強', '弓兵強行', '王者の護り'],
	'騎兵の大聖域':['騎兵の聖域', '騎兵増強', '騎兵強行', '王者の護り'],
	'剣兵の大神域':['剣兵の大聖域', '剣兵の采配', '神速', '忠節不落'],
	'槍兵の大神域':['槍兵の大聖域', '槍兵の采配', '神速', '忠節不落'],
	'弓兵の大神域':['弓兵の大聖域', '弓兵の采配', '神速', '忠節不落'],
	'騎兵の大神域':['騎兵の大聖域', '騎兵の采配', '神速', '忠節不落'],
	'鉄壁':['槍兵堅守', '豪傑', '騎兵堅守', '守護神'],
	'守護神':['鉄壁', '槍兵方陣', '弓兵方陣', '王者の護り'],
	'金剛不壊':['防将戦法', '槍兵の聖域', '弓兵の聖域', '防将戦略'],
	'護君':['防将戦略', '守護堅陣', '神速援護', '忠節不落'],
	'八卦の陣':['兵器の猛撃', '剣兵の聖域', '槍兵方陣', '王者の護り'],
	'王者の護り':['槍兵方陣', '弓兵方陣', '騎兵方陣', '王佐の才'],
	'忠節不落':['覇道', '鉄壁', '千里行', '八卦の陣'],
	'覇者の護り':['槍兵の聖域', '弓兵の聖域', '騎兵の聖域', '忠節不落'],
	'不撓不屈':['強襲突覇', '守護防陣', '加工技術', '護君'],
	'堅忍不抜':['強襲突覇', '守護防陣', '加工技術', '護君'],
	'勇猛果敢':['強襲突覇', '守護防陣', '加工技術', '護君'],
	'剣将の采配':['剣兵突覇', '富国', '奇計百出', '猛将突貫'],
	'槍将の采配':['槍兵突覇', '市場知識', '奇計百出', '麒麟児'],
	'弓将の采配':['弓兵突覇', '王佐の才', '奇計百出', '飛将'],
	'騎将の采配':['騎兵突覇', '城壁補強', '奇計百出', '市場繁栄'],
	'蜀軍の防衛':['急速援護', '槍兵の聖域', '守護神', '迅速援護'],
	'魏軍の防衛':['迅速援護', '城壁補強', '王者の護り', '守将の進軍'],
	'呉軍の防衛':['迅速援護', '呉の治世', '八卦の陣', '地の利堅壁'],
	'群雄の防衛':['迅速援護', '王者の護り', '城壁補強', '守将の進軍'],
	'擒賊擒王':['槍兵突撃', '八卦の陣', '城壁補強', '王者の護り'],
	'堅牢知略':['弓兵突撃', '八卦の陣', '兵器強行', '攻城の檄文 '],
	'地の利堅壁':['兵器の猛撃', '槍兵方陣', '剣兵の聖域', '王者の護り'],
	'防将戦略':['槍兵の聖域', '弓兵の聖域', '騎兵の聖域', '守護堅陣'],
	'防将戦法':['槍兵方陣', '弓兵方陣', '騎兵方陣', '守護防陣'],
	'守護防陣':['槍兵防御', '弓兵防御', '騎兵防御', '火神の攻勢'],
	'守護堅陣':['槍兵方陣', '弓兵方陣', '騎兵方陣', '覇道'],
	'守護方陣':['槍兵の聖域', '弓兵の聖域', '騎兵の聖域', '守護堅陣'],
	'守護聖陣':['槍将の采配', '弓将の采配', '騎将の采配', '守護方陣'],
	'守衛方陣':['槍兵の聖域', '弓兵の聖域', '騎兵の聖域', '守護堅陣'],
	'守衛聖陣':['槍将の采配', '弓将の采配', '騎将の采配', '守護方陣'],
	'守備聖陣':['弓将の采配', '騎将の采配', '槍将の采配', '守護方陣'],
	'伐採知識':['伐採知識', '槍兵行軍', '練兵訓練', '伐採技術'],
	'石切知識':['石切知識', '弓兵行軍', '兵舎訓練', '石切技術'],
	'製鉄知識':['製鉄知識', '騎兵行軍', '弓兵訓練', '製鉄技術'],
	'食糧知識':['食糧知識', '剣兵行軍', '厩舎訓練', '食糧技術'],
	'伐採技術':['伐採知識', '槍兵強行', '練兵訓練', '農林知識'],
	'石切技術':['石切知識', '弓兵強行', '兵舎訓練', '加工知識'],
	'製鉄技術':['製鉄知識', '騎兵強行', '弓兵訓練', '弓兵修練'],
	'食糧技術':['食糧知識', '剣兵強行', '厩舎訓練', '厩舎修練'],
	'農林知識':['伐採技術', '食糧技術', '槍兵強行', '農林技術'],
	'農林技術':['農林知識', '英雄', '槍兵突覇', '富国'],
	'加工知識':['石切技術', '製鉄技術', '騎兵強行', '加工技術'],
	'加工技術':['加工知識', '鉄壁', '弓兵突覇', '豊穣'],
	'素材知識':['騎兵の極撃', '騎兵の聖域', '英雄', '市場知識'],
	'食糧革命':['剣兵の極撃', '剣兵の聖域', '練兵修練', '食糧技術'],
	'食糧天革命':['剣兵の大極撃', '剣兵の大聖域', '攻城の義勇軍', '富国論'],
	'富国':['槍兵の極撃', '槍兵の聖域', '仁君', '豊穣'],
	'恵風':['騎兵の極撃', '騎兵の聖域', '英雄', '市場知識'],
	'豊穣':['弓兵の極撃', '弓兵の聖域', '呉の治世', '富国'],
	'人選眼力':['伐採知識', '石切知識', '製鉄知識', '食糧知識'],
	'才女の音律':['一騎当千', '農林知識', '神速', '闘将の極意'],
	'豊潤祈祷':['剣兵の極撃', '仁君', '練兵修練', '富国'],
	'富国論':['兵器修練', '攻城の檄文', '豊穣', '富国'],
	'聡明叡知':['奇計百出', '八卦の陣', '神速', '飛将'],
	'優姫の敬愛':['弓兵の猛撃', '弓兵の聖域', '弓兵修練', '護君'],
	'孫家の恵み':['猛虎進撃', '守護防陣', '市場知識', '守護堅陣'],
	'薫風の恵み':['武将の貫攻', '槍兵の聖域', '英雄', '闘将の貫攻'],
	'暁風の恵み':['武将の守勢', '剣兵の聖域', '英雄', '闘将の守勢'],
	'石切技巧':['豊穣', '石切技術', '富国', '市場繁栄'],
	'伐採技巧':['豊穣', '伐採技術', '富国', '市場繁栄'],
	'製鉄技巧':['豊穣', '製鉄技術', '富国', '市場繁栄'],
	'食糧技巧':['豊穣', '食糧技術', '富国', '市場繁栄'],
	'練兵訓練':['練兵訓練', '剣兵防御', '剣兵の進撃', '練兵修練'],
	'兵舎訓練':['兵舎訓練', '槍兵防御', '槍兵の進撃', '兵舎修練'],
	'弓兵訓練':['弓兵訓練', '弓兵防御', '弓兵の進撃', '弓兵修練'],
	'厩舎訓練':['厩舎訓練', '騎兵防御', '騎兵の進撃', '厩舎修練'],
	'兵器訓練':['兵器訓練', '兵器防御', '兵器の進撃', '兵器修練'],
	'練兵修練':['練兵修練', '剣兵方陣', '剣兵の強撃', '農林技術'],
	'兵舎修練':['豪傑', '槍兵方陣', '槍兵の強撃', '加工技術'],
	'弓兵修練':['豪傑', '弓兵方陣', '弓兵の強撃', '農林技術'],
	'厩舎修練':['豪傑', '騎兵方陣', '騎兵の強撃', '加工技術'],
	'兵器修練':['兵器修練', '豪傑', '兵器の強撃', '加工技術'],
	'攻城の檄文':['兵器修練', '英雄', '強兵の檄文', '神医の術式'],
	'強兵の檄文':['兵舎修練', '守護神', '攻城の檄文', '神医の術式'],
	'剣兵増強':['農林技術', '千里行', '剣兵増強', '覇王の進撃'],
	'槍兵増強':['市場知識', '一騎当千', '槍兵増強', '強兵の檄文'],
	'弓兵増強':['城壁補強', '神速援護', '弓兵増強', '攻城の檄文'],
	'騎兵増強':['加工技術', '強襲突撃', '騎兵増強', '軍神'],
	'富国強兵':['攻城の檄文', '軍神', '豊穣', '市場繁栄'],
	'老巧の政令':['麒麟児', '地の利堅壁', '富国論', '市場繁栄'],
	'密偵召集':['一騎当千', '鉄壁', '呉の治世', '王佐の才'],
	'師君強兵':['神速援護', '守護堅陣', '剣将の采配', '守護聖陣'],
	'娘々強兵':['攻城の檄文', '八卦の陣', '豊穣', '強兵の檄文'],
	'呉の治世':['奇計百出', '厩舎修練', '製鉄技術', '王佐の才'],
	'呉の大治世':['混元一気', '覇者の護り', '飛速', '富国論'],
	'王佐の才':['兵器の猛撃', '兵器修練', '呉の治世', '傾国'],
	'賢妃施政':['飛将', '兵器修練', '呉の治世', '傾国'],
	'麗妃都政':['闘将の極意', '八卦の陣', '覇道', '猛将の極意'],
	'陳留王政':['神算鬼謀', '強兵の檄文', '富国', '皇帝の勅令'],
	'徳義為政':['飛蹄進軍', '攻城の檄文', '王佐の才', '神医の術式'],
	'美玉歌舞':['奇計百出', '仁君', '厩舎修練', '王佐の才'],
	'苛政虎舞':['弓兵の強撃', '兵器の極撃', '神速', '飛蹄進軍'],
	'憂姫護国':['覇道', '兵器強行', '農林知識', '飛将'],
	'領地統制':['深慮遠謀', '堅牢知略', '呉の治世', '富国論'],
	'拠点統制':['神算鬼謀', '防将戦法', '王佐の才', '富国論'],
	'仁君':['槍兵の強撃', '兵舎修練', '覇道', '八卦の陣'],
	'弓腰姫の愛':['弓兵の猛撃', '剣兵の極撃', '弓兵修練', '弓将の采配'],
	'桃色吐息':['一騎当千', '仁君', '王佐の才', '趁火打劫'],
	'熊猫の麺匠':['弓兵の猛撃', '剣兵の極撃', '弓兵修練', '弓将の采配'],
	'神医の術式':['八卦の陣', '守護神', '王佐の才', '槍将の采配'],
	'神医の施術':['仁君', '騎兵の聖域', '加工技術', '神医の術式'],
	'劉備の契り':['仁君', '守護神', '呉の治世', '神算鬼謀'],
	'傾国':['騎兵の極撃', '弓兵の聖域', '神医の術式', '強兵の檄文'],
	'才女の瞳':['騎兵突覇', '鉄壁', '覇道', '闘将の極意'],
	'皇后の慈愛':['覇王の進撃', '傾国', '富国', '神医の術式'],
	'優雅な調べ':['攻城の檄文', '八卦の陣', '豊穣', '強兵の檄文'],
	'勇姫督励':['万夫不当', '守護神', '城壁補強', '護君'],
	'文姫の慈愛':['覇王の進撃', '傾国', '富国', '神医の術式'],
	'城壁補強':['万夫不当', '八卦の陣', '豊潤祈祷', '猛将の鹵獲'],
	'発憤興起':['強襲速撃', '槍兵方陣', '鉄壁', '槍兵増強'],
	'舞姫の愛':['万夫不当', '王佐の才', '仁君', '富国'],
	'酔吟吐息':['趁火打劫', '呉の治世', '恵風', '桃色吐息'],
	'護国芳心':['剣兵の極撃', '剣兵方陣', '剛将の畏怖', '神算鬼謀'],
	'護国献心':['剣兵の猛撃', '剣兵堅守', '覇道', '深慮遠謀'],
	'月下羽衣':['傾国', '富国', '市場知識', '覇王の進撃'],
	'市場知識':['農林知識', '千里行', '豪傑', '八卦の陣'],
	'市場繁栄':['傾国', '富国', '豊穣', '弓将の采配'],
	'市場来福':['槍兵速攻', '弓兵速攻', '騎兵速攻', '迅速却略'],
	'市場栄華':['富国', '万里行', '万夫不当', '忠節不落'],
	'速撃の舞':['精鋭の進撃', '加工技術', '守護神', '市場繁栄'],
	'出撃見舞':['覇王の進撃', '守護神', '千里行', '神速'],
	'迅速斥候':['兵器修練', '八卦の陣', '加工技術', '豊穣'],
	'暴姫の命':['強襲突撃', '強兵の檄文', '神速', '猛将の鹵獲'],
	'暴姫の威令':['猛将の鹵獲', '鉄壁', '食糧知識', '護君'],
	'佳人薄命':['猛将の鹵獲', '王者の護り', '城壁補強', '鬼神の鹵獲'],
	'三顧の礼':['強襲突破', '守護神', '神算鬼謀', '深慮遠謀'],
	'槍兵の勝鬨':['槍兵の極撃', '槍兵の聖域', '槍兵突覇', '槍将の采配'],
	'弓兵の勝鬨':['弓兵の極撃', '弓兵の聖域', '弓兵突覇', '弓将の采配'],
	'騎兵の勝鬨':['騎兵の極撃', '騎兵の聖域', '騎兵突覇', '騎将の采配'],
	'兵器の勝鬨':['兵器の極撃', '剣兵の聖域', '兵器突覇', '攻城の檄文'],
	'才媛献策':['深慮遠謀', '神速援護', '富国', '傾国'],
	'才媛眼識':['深慮遠謀', '傾国', '神速援護', '富国'],
	'天子勅令':['趁火打劫', '槍兵の勝鬨', '王佐の才', '富国論'],
	'悲嘆の舞':['騎兵の強撃', '強兵の檄文', '智将器撃', '混元一気'],
	'洛神の舞':['護君', '強兵の檄文', '富国', '槍将の采配'],
	'大華の舞':['覇王の進撃', '攻城の檄文', '豊穣', '弓将の采配'],
	'小華の舞':['弓兵の強撃', '強兵の檄文', '恵風', '豊穣'],
	'勇躍の舞':['守護方陣', '強兵の檄文', '恵風', '剣将の采配'],
	'哀艶の舞':['槍兵突覇', '千里行', '農林技術', '鬼神の極意'],
	'槍兵の大号令':['麒麟慈心', '呉の治世', '槍兵の勝鬨', '智将器撃'],
	'騎兵の大号令':['神算鬼謀', '王佐の才', '騎兵の勝鬨', '奇略布陣'],
	'弓兵の大号令':['勇将飛矢', '呉の治世', '弓兵の勝鬨', '国士無双'],
	'兵器の大号令':['太平要術', '呉の治世', '王佐の才', '忠節不落'],
	'賢女の策略':['兵器の極撃', '王者の護り', '槍将の采配', '智将器撃'],
	'皇帝の勅令':['弓兵の強撃', '兵器の極撃', '神速', '強襲突覇'],
	'虐帝の号令':['兵器の猛撃', '守護神', '覇道', '蛮王の襲撃'],
	'皇帝の覇軍':['騎兵突覇', '王者の護り', '覇王の進撃', '魏武王'],
	'才略謀略':['槍兵の勝鬨', '弓兵の勝鬨', '騎兵の勝鬨', '智将器撃'],
	'奇略布陣':['強襲突覇', '加工技術', '兵器の極撃', '市場繁栄'],
	'密偵精鋭':['兵器の猛撃', '急速援護', '食糧技術', '万夫不当'],
	'大剣兵移送':['強襲突覇', '剣兵の聖域', '錬兵修練', '剣兵の極撃'],
	'矛槍兵移送':['強襲突覇', '槍兵の聖域', '兵舎修練', '槍兵の極撃'],
	'近衛騎兵移送':['迅速援護', '騎兵の聖域', '厩舎修練', '騎兵の極撃'],
	'弩兵移送':['強襲突覇', '弓兵の聖域', '弓舎修練', '弓兵の極撃'],
	'剣兵急配':['強襲突撃', '剣兵方陣', '練兵訓練', '剣兵の猛撃'],
	'槍兵急配':['強襲突撃', '槍兵方陣', '兵舎訓練', '槍兵の猛撃'],
	'弓兵急配':['強襲突撃', '弓兵方陣', '弓兵訓練', '弓兵の猛撃'],
	'騎兵急配':['強襲突撃', '騎兵方陣', '厩舎訓練', '騎兵の猛撃'],
	'衝車急配':['強襲突撃', '剣兵方陣', '兵器訓練', '兵器の猛撃'],
	'万華招来':['万夫不当', '金剛不壊', '神算鬼謀', '神医の術式'],
	'月華の舞':['猛将の極意', '攻城の檄文', '食糧技術', '騎将の采配'],
	'緊急帰還':['槍将の采配', '城壁補強', '加工技術', '槍兵の猛攻'],
	'全軍の進攻令':['全軍の進攻令', '八卦の陣', '奇計百出', '覇王の進撃'],
	'全軍の強攻令':['覇王の強撃', '王者の護り', '深慮遠謀', '全軍の強攻令'],
	'全軍の猛攻令':['全軍の強攻令', '忠節不落', '神算鬼謀', '覇王の猛撃'],
	'全軍の極攻令':['覇王の猛撃', '覇者の護り', '混元一気', '全軍の猛攻令'],
	'全軍の大極攻令':['全軍の猛攻令', '護君', '智将の大神撃', '全軍の極攻令'],
	'全兵の極攻令':['覇王の猛撃', '覇者の護り', '混元一気', '全軍の猛攻令'],
	'全兵の大極攻令':['全軍の猛攻令', '護君', '智将の大神撃', '全軍の極攻令'],
	'槍兵の大天撃':['槍兵の大神撃', '護君', '強兵の檄文', '蜀軍の極撃'],
	'弓兵の大天撃':['弓兵の大神撃', '護君', '強兵の檄文', '呉軍の極撃'],
	'騎兵の大天撃':['騎兵の大神撃', '護君', '強兵の檄文', '魏軍の極撃'],
	'剣兵の大天撃':['剣兵の大神撃', '護君', '攻城の檄文', '群雄の極撃'],
	'千華招来':['一騎当千', '守護神', '深慮遠謀', '桃色吐息'],
	'飛速迅雷':['烈速', '剣兵の聖域', '神算鬼謀', '烈速雷光'],
	'天里迅雷':['万里行', '強襲突覇', '密偵精鋭', '万里雷光'],
	'収奪の猛攻':['鹵獲の強攻', '八卦の陣', '迅速擁護', '猛将の鹵獲'],
	'収奪の極攻':['猛将の鹵獲', '王者の護り', '神速援護', '鹵獲の猛攻'],
	'帝妃都政':['奇計百出', '厩舎修練', '製鉄技術', '王佐の才'],
	'娘々武神':['一騎当千', '槍兵の聖域', '兵舎修練', '軍神'],
	'王将の心意気':['軍神', '城壁補強', '騎兵の勝鬨', '武神'],
	'食糧革命':['剣兵の極撃', '剣兵の聖域', '練兵修練', '食糧技術'],
	'護国望心':['剣兵の強撃', '剣兵防御', '英雄', '奇計百出'],
	'六文銭の雷光':['神速雷光', '剣兵方陣', '深慮遠謀', '烈速'],
	'鹵獲の進攻':['鹵獲の進攻', '鉄壁', '千里行', '	趁火打劫'],
	'天衣無縫':['忠節不落', '守護方陣', '烈速', '覇者の護り'],
	'剣兵増勢':['剣兵速攻', '剣兵方陣', '神速', '剣兵突攻'],
	'槍兵増勢':['槍兵速攻', '槍兵方陣', '神速', '槍兵突攻'],
	'守衛招来':['守護聖陣', '守護神', '深慮遠謀', '守衛聖陣'],
	'知略謀略':['石切技術', '伐採技術', '製鉄技術', '兵器突覇'],
	'槍将の封聖域':['槍兵の大極撃', '守護堅陣', '恵風', '槍兵の大聖域'],
	'弓将の封聖域':['弓兵の大極撃', '守護堅陣', '恵風', '弓兵の大聖域'],
	'騎将の封聖域':['騎兵の大極撃', '守護堅陣', '恵風', '騎兵の大聖域'],
	'剣将の封聖域':['剣兵の大極撃', '守護堅陣', '恵風', '剣兵の大聖域'],
	'エクスカリパー':['剣兵の進撃', '剣兵防御', '剣兵行軍', '剣兵の強撃'],
	'弓兵増勢':['弓兵速攻', '弓兵方陣', '神速', '弓兵突攻'],
	'剣兵烈攻':['剣兵の大極撃', '烈速', '剣兵増強', '剣兵突攻'],
	'弓兵烈攻':['弓兵の大極撃', '烈速', '弓兵増強', '弓兵突攻'],
	'槍兵烈攻':['槍兵の大極撃', '烈速', '槍兵増強', '槍兵突攻'],
	'騎兵烈攻':['騎兵の大極撃', '烈速', '騎兵増強', '騎兵突攻'],
	'兵器烈攻':['兵器の大極撃', '烈速', '兵器増強', '兵器突攻'],
	'謀反の烈攻':['剛将の畏怖', '金剛不壊', '富国', '謀反の突攻'],
	'剣兵大烈攻':['剣兵の大神撃', '飛速', '剣将の采配', '剣兵烈攻'],
	'弓兵大烈攻':['弓兵の大神撃', '飛速', '弓将の采配', '弓兵烈攻'],
	'槍兵大烈攻':['槍兵の大神撃', '飛速', '槍将の采配', '槍兵烈攻'],
	'騎兵大烈攻':['騎兵の大神撃', '飛速', '騎将の采配', '騎兵烈攻'],
	'夏神の極意':['隣地猛攻', '王者の護り', '迅速援護', '猛将の極意'],
	'龍神の縮地劫略':['飛将', '護君', '鬼神の鹵獲', '天里行'],
	'鬼神の縮地劫略':['万夫不当', '金剛不壊', '猛将の鹵獲', '万里行'],
	'猛将の縮地劫略':['一騎当千', '守護神', '迅速劫略', '千里行'],
	'猛暑の縮地収穫':['一騎当千', '守護神', '迅速劫略', '千里行'],
	'兵器の縮地天撃':['兵器の大神撃', '覇者の護り', '智将砕器', '飛速'],
	'知略徴発':['一騎当千', '八卦の陣', '千里行', '趁火打劫'],
	'才略徴発':['趁火打劫', '守護神', '神速', '迅速劫略'],
	'騎兵増勢':['騎兵速攻', '騎兵方陣', '神速', '騎兵突攻'],
	'鬼神突貫':['軍神', '王者の護り', '万里行', '武神'],
	'龍神突貫':['武神', '金剛不壊', '烈速', '鬼神'],
	'覇者の大神域':['槍将の采配', '弓将の采配', '騎将の采配', '覇者の護り'],
	'守城誠心':['兵器の極撃', '兵器修練', '太平要術', '智将の大極撃'],
	'守城真心':['兵器の大極撃', '攻城の檄文', '暴虐外道', '智将の大神撃'],
	'聖夜の天恵沢':['混元一気', '覇者の護り', '神医の術式', '攻城の義勇軍'],
	'報恩の神恵沢':['神算鬼謀', '忠節不落', '桃色吐息', '攻城の檄文'],
	'花嫁の天恵沢':['混元一気', '覇者の護り', '神医の術式', '攻城の義勇軍'],
	'花嫁の神恵沢':['神算鬼謀', '忠節不落', '桃色吐息', '攻城の檄文'],
	'感謝の聖恵沢':['深慮遠謀', '王者の護り', '仁君', '兵器修練'],
	'南瓜の天恵沢':['混元一気', '覇者の護り', '神医の術式', '攻城の義勇軍'],
	'南瓜の神恵沢':['神算鬼謀', '忠節不落', '桃色吐息', '攻城の檄文'],
	'報恩の天恵沢':['混元一気', '覇者の護り', '神医の術式', '攻城の義勇軍'],
	'報恩の聖恵沢':['深慮遠謀', '王者の護り', '仁君', '兵器修練'],
	'豊穣の天恵沢':['混元一気', '覇者の護り', '神医の術式', '攻城の義勇軍'],
	'豊穣の神恵沢':['神算鬼謀', '忠節不落', '桃色吐息', '攻城の檄文'],
	'豊穣の聖恵沢':['深慮遠謀', '王者の護り', '仁君', '兵器修練'],
	'全軍の神速令':['神速', '守護神', '迅速援護', '全軍の神速令'],
	'全軍の烈速令':['全軍の神速令', '金剛不壊', '神速援護', '烈速'],
	'全軍の飛速令':['烈速', '護君', '市場知識', '全軍の烈速令'],
	'全軍の鳳速令':['全軍の烈速令', '覇者の護り', '市場繁栄', '飛速'],
	'鬼神の守勢':['隣地猛攻', '王者の護り', '迅速援護', '猛将の守勢'],
	'猛将の守勢':['万夫不当', '守護神', '神速', '闘将の守勢'],
	'闘将の守勢':['一騎当千', '八卦の陣', '千里行', '武将の守勢'],
	'武将の守勢':['豪傑', '鉄壁', '急速援護', '守護神'],
	'龍神の守勢':['遠地猛攻', '金剛不壊', '神速援護', '鬼神の守勢'],
	'龍亀の守勢':['遠地猛攻', '金剛不壊', '神速援護', '鬼神の守勢'],
	'戦神の守勢':['隣地猛攻', '王者の護り', '迅速援護', '猛将の守勢'],
	'猛士の守勢':['万夫不当', '守護神', '神速', '闘将の守勢'],
	'神隠雷光':['強襲速撃', '堅牢知略', '魏王の号令', '覇王の進撃'],
	'烈隠雷光':['強襲突撃', '地の利堅壁', '太平要術', '覇王の強撃'],
	'飛隠雷光':['強襲突覇', '地の利堅壁', '太平要術', '覇王の強撃'],
	'鳳隠雷光':['強襲烈覇', '防将戦略', '暴君の覇道', '鬼神'],
	'強兵の義勇軍':['猛将の鹵獲', '迅速援護', '攻城の義勇軍', '神医の術式'],
	'攻城の義勇軍':['猛将の鹵獲', '迅速援護', '強兵の義勇軍', '神医の術式'],
	'鬼神の貫攻':['猛将突貫', '王者の護り', '万里行', '猛将の貫攻'],
	'猛将の貫攻':['闘将突貫', '守護神', '迅速援護', '闘将の貫攻'],
	'闘将の貫攻':['武将突貫', '八卦の陣', '千里行', '武将の貫攻'],
	'武将の貫攻':['豪傑', '鉄壁', '急速援護', '武将突貫'],
	'龍神の貫攻':['鬼神突貫', '金剛不壊', '神速援護', '鬼神の貫攻'],
	'戦神の貫攻':['猛将突貫', '王者の護り', '万里行', '猛将の貫攻'],
	'龍虎の貫攻':['鬼神突貫', '金剛不壊', '神速援護', '鬼神の貫攻'],
	'戦虎の貫攻':['猛将突貫', '王者の護り', '万里行', '猛将の貫攻'],
	'猛虎の貫攻':['闘将突貫', '守護神', '迅速援護', '闘将の貫攻'],
	'猛士の貫攻':['闘将突貫', '守護神', '迅速援護', '闘将の貫攻'],
	'女神の貫攻':['猛将突貫', '王者の護り', '万里行', '猛将の貫攻'],
	'神卜の術式':['八卦の陣', '守護神', '王佐の才', '神算鬼謀'],
	'戦神招来':['鬼神の極意', '守護神', '深慮遠謀', '戦神の極意'],
	'迅雷招来':['飛速雷光', '守護神', '深慮遠謀', '飛速迅雷'],
	'豪撃招来':['隣地豪襲', '守護神', '深慮遠謀', '隣地豪撃'],
	'天華招来':['飛将', '護君', '混元一気', '神医の施術'],
	'収奪招来':['鹵獲の極攻', '守護神', '深慮遠謀', '収奪の極攻'],
	'全兵招来':['全軍の極攻令', '守護神', '深慮遠謀', '全兵の極攻令'],
	'貫攻招来':['鬼神の貫攻', '守護神', '深慮遠謀', '戦神の貫攻'],
	'守勢招来':['鬼神の守勢', '守護神', '深慮遠謀', '戦神の守勢'],
	'天里招来':['天里雷光', '守護神', '深慮遠謀', '天里迅雷'],
	'護国誠心':['剣兵の大極撃', '剣兵の聖域', '太平要術', '混元一気'],
	'護国真心':['剣兵の大神撃', '剣兵の大聖域', '暴虐外道', '智将の大神撃'],
	'守衛大聖陣':['弓兵の大聖域', '槍兵の大聖域', '騎兵の大聖域', '守護聖陣'],
	'守護大聖陣':['弓兵の大聖域', '槍兵の大聖域', '騎兵の大聖域', '守護聖陣'],
	'援軍の聖域':['覇王の進撃', '鉄壁', '急速援護', '八卦の陣'],
	'援軍の大神域':['覇王の猛撃', '金剛不壊', '神速援護', '忠節不落'],
	'援軍の大聖域':['覇王の強撃', '守護神', '迅速援護', '王者の護り'],
	'春遊の呼集':['武神', '忠節不落', '飛速', '市場繁栄'],
	'緊急呼集':['武神', '忠節不落', '飛速', '市場繁栄'],
	'非常呼集':['軍神', '王者の護り', '烈速', '市場知識'],
	'隣地神攻城':['兵器の猛撃', '忠節不落', '強兵の檄文', '智将器撃'],
	'隣地猛攻':['一騎当千', '覇道', '強兵の檄文', '強襲突覇'],
	'隣地天豪襲':['闘将の極意', '謀反の強攻', '攻城の義勇軍', '飛将'],
	'覇王の神賢斬':['覇王の強撃', '忠節不落', '烈速', '覇王の猛撃'],
	'覇王の神護斬':['覇王の強撃', '忠節不落', '烈速', '覇王の猛撃'],
	'覇王の神俊賢':['覇王の強撃', '忠節不落', '烈速', '覇王の猛撃'],
	'覇王の神俊護':['覇王の強撃', '忠節不落', '烈速', '覇王の猛撃'],
	'覇王の神賢護':['覇王の強撃', '忠節不落', '烈速', '覇王の猛撃'],
	'お菓子の攻奪':['鹵獲の猛攻', '護君', '富国論', '鬼神の鹵獲'],
	'龍神の攻奪':['鬼神の鹵獲', '護君', '富国論', '鹵獲の極攻'],
	'鬼神の叛乱':['万夫不当', '金剛不壊', '暴虐外道', '軍神'],
	'猛将の叛乱':['一騎当千', '守護神', '太平要術', '覇王の強撃'],
	'飛軍雷光':['強襲突覇', '忠節不落', '烈速雷光', '武神'],
	'鳳軍雷光':['強襲烈覇', '覇者の護り', '飛速雷光', '鬼神'],
	'強襲天奮迅':['奇襲', '忠節不落', '豊穣', '強襲烈覇'],
	'春眠の攻奪':['猛将の鹵獲', '金剛不壊', '豊穣', '鹵獲の猛攻'],
	'鬼神の攻奪':['猛将の鹵獲', '金剛不壊', '豊穣', '鹵獲の猛攻'],
	'猛将の攻奪':['趁火打劫', '守護神', '富国', '鹵獲の強攻'],
	'謀反の大烈攻':['太平要術', '護君', '富国論', '謀反の烈攻'],
	'智将の大天撃':['智将の大神撃', '覇者の護り', '飛速', '強襲烈覇'],
	'智将の大神撃':['智将の大極撃', '忠節不落', '烈速', '強襲突覇'],
	'智将の大極撃':['麒麟児', '王者の護り', '神速', '強襲突撃'],
	'歴戦の弓将':['弓兵の大極撃', '弓将の采配', '伐採技術', '弓兵の大号令'],
	'玄冬の祝い':['闘将の極意', '八卦の陣', '千里行', '隣地猛襲'],
	'白秋の祝い':['兵器速攻', '八卦の陣', '千里行', '兵器突攻'],
	'朱夏の祝い':['食糧技巧', '八卦の陣', '千里行', '食糧極技巧'],
	'青春の祝い':['神速雷光', '八卦の陣', '千里行', '烈速雷光'],
	'機略徴発':['迅速劫略', '王者の護り', '万里行', '猛将の鹵獲'],
	'食糧極技巧':['剣兵の極撃', '剣兵の聖域', '食糧技術', '食糧技巧'],
	'覇王の帝賢斬':['軍神', '覇者の護り', '飛速', '武神'],
	'覇王の帝護斬':['軍神', '覇者の護り', '飛速', '武神'],
	'覇王の帝俊賢':['軍神', '覇者の護り', '飛速', '武神'],
	'覇王の帝俊護':['軍神', '覇者の護り', '飛速', '武神'],
	'覇王の帝賢護':['軍神', '覇者の護り', '飛速', '武神'],
	'剴切の天砕器':['兵器の大神撃', '護君', '攻城の義勇軍', '智将砕器'],
	'剴切の大神域':['槍将の采配', '弓将の采配', '騎将の采配', '覇者の護り'],
	'剴切の鳳雷光':['烈速雷光', '剣将の采配', '混元一気', '飛速'],
	'剴切の天叛攻':['謀反の猛攻', '忠節不落', '飛速', '暴君の覇道'],
	'剴切の神叛攻':['暴虐外道', '王者の護り', '神速援護', '謀反の猛攻'],
	'剴切の大極攻令':['全軍の猛攻令', '護君', '智将の大神撃', '全軍の極攻令'],
	'剴切の極攻令':['覇王の猛撃', '覇者の護り', '混元一気', '全軍の猛攻令'],
	'剴切大聖陣':['弓兵の大聖域', '槍兵の大聖域', '騎兵の大聖域', '守護聖陣'],
	'剴切聖陣':['弓将の采配', '騎将の采配', '槍将の采配', '守護方陣'],
	'副将の天明鏡':['飛将', '護君', '天里行', '鬼神の極意'],
	'天里雷鳴':['万里行', '強襲突覇', '密偵精鋭', '万里雷光'],
	'江河の祝福':['謀反の猛攻', '守護神', '神速', '謀反の極攻'],
	'飛泉の祝い':['闘将の貫攻', '八卦の陣', '千里行', '猛将の貫攻'],
	'娘々軍神':['一騎当千', '槍兵方陣', '兵舎修練', '万夫不当'],
	'娘々大神砕':['兵器の大極撃', '兵器の大号令', '豊潤祈祷', '兵器の大神撃'],
	'娘々龍神':['武神', '金剛不壊', '兵舎修練', '鬼神'],
	'娘々無双':['万夫不当', '金剛不壊', '万里行', '覇王の強撃'],
	'娘々護君':['防将戦略', '守護堅陣', '神速援護', '忠節不落'],
	'娘々無縫':['忠節不落', '守護方陣', '烈速', '覇者の護り'],
	'娘々大神域':['槍将の采配', '弓将の采配', '騎将の采配', '覇者の護り'],
	'娘々天恵沢':['混元一気', '覇者の護り', '神医の術式', '攻城の義勇軍'],
	'娘々聖水':['仁君', '騎兵の聖域', '加工技術', '桃色吐息'],
	'娘々敬慕':['弓兵の猛撃', '剣兵の極撃', '弓兵修練', '弓将の采配'],
	'兵器の娘天撃':['兵器の大神撃', '護君', '攻城の義勇軍', '智将砕器'],
	'龍神の劫略令':['鹵獲の猛攻', '覇者の護り', '天里行', '鹵獲の極攻'],
	'鬼神の劫略令':['猛将の鹵獲', '護君', '烈速', '鹵獲の猛攻'],
	'猛将の劫略令':['鹵獲の強攻', '金剛不壊', '万里行', '猛将の鹵獲'],
	'闘将の劫略令':['趁火打劫', '守護神', '神速', '鹵獲の強攻'],
	'武将の劫略令':['鹵獲の進攻', '鉄壁', '千里行', '趁火打劫'],
	'お菓子の劫略令':['鹵獲の猛攻', '覇者の護り', '天里行', '鹵獲の極攻'],
	'南瓜の劫略令':['猛将の鹵獲', '護君', '烈速', '鹵獲の猛攻'],
	'仮装の劫略令':['鹵獲の強攻', '金剛不壊', '万里行', '猛将の鹵獲'],
	'樅樹の劫略令':['鹵獲の猛攻', '覇者の護り', '天里行', '鹵獲の極攻'],
	'雪華の劫略令':['猛将の鹵獲', '護君', '烈速', '鹵獲の猛攻'],
	'玩具の劫略令':['鹵獲の強攻', '金剛不壊', '万里行', '猛将の鹵獲'],
	'鳴鳥の祝福':['鹵獲の猛攻', '守護神', '神速', '鹵獲の極攻'],
	'鳴鳥の祝い':['鹵獲の強攻', '八卦の陣', '千里行', '鹵獲の猛攻'],
	'絹雲の祝い':['守護堅陣', '八卦の陣', '千里行', '守護方陣'],
	'玉雪の祝い':['闘将の守勢', '八卦の陣', '千里行', '猛将の守勢'],
	'守衛招致':['守護方陣', '鉄壁', '奇計百出', '守衛方陣'],
	'全兵招致':['全軍の猛攻令', '鉄壁', '奇計百出', '全兵の猛攻令'],
	'略取の極攻':['猛将の鹵獲', '王者の護り', '神速援護', '鹵獲の猛攻'],
	'王佐の天資':['鬼神の鹵獲', '覇者の護り', '富国論', '智将の大神撃'],
	'剣兵の大天域':['剣兵の大神域', '攻城の檄文', '烈速', '覇者の護り'],
	'槍兵の大天域':['槍兵の大神域', '強兵の檄文', '烈速', '覇者の護り'],
	'弓兵の大天域':['弓兵の大神域', '強兵の檄文', '烈速', '覇者の護り'],
	'騎兵の大天域':['騎兵の大神域', '強兵の檄文', '烈速', '覇者の護り'],
	'お菓子の療略令':['鹵獲の猛攻', '覇者の護り', '神医の施術', '鹵獲の極攻'],
	'鋭兵の神遠征軍':['強襲烈覇', '覇者の護り', '飛速雷光', '鬼神'],
	'仮装の療略令':['鹵獲の強攻', '金剛不壊', '発憤興起', '猛将の鹵獲'],
	'玩具の叛攻令':['闘将の叛攻令', '金剛不壊', '神算鬼謀', '謀反の猛攻'],
	'騎神の導き':['騎兵の極攻', '騎兵烈攻', '覇道', '騎兵の大極攻'],
	'鬼神の叛攻令':['謀反の猛攻', '護君', '混元一気', '猛将の叛攻令'],
	'弓神の導き':['弓兵の極攻', '弓兵烈攻', '覇道', '弓兵の大極攻'],
	'強兵の神義勇軍':['鬼神の鹵獲', '神速援護', '攻城の神義勇軍', '神医の施術'],
	'強兵の大檄文':['鬼神の鹵獲', '覇者の護り', '攻城の大檄文', '神医の施術'],
	'剣神の導き':['剣兵の極攻', '剣兵烈攻', '覇道', '剣兵の大極攻'],
	'玄武の啓示':['覇者の大神域', '天衣無縫', '覇道', '守護大聖陣'],
	'攻城の神義勇軍':['鬼神の鹵獲', '神速援護', '強兵の神義勇軍', '神医の施術'],
	'攻城の大檄文':['鬼神の鹵獲', '覇者の護り', '強兵の大檄文', '神医の施術'],
	'劫略招致':['猛将の劫略令', '鉄壁', '奇計百出', '猛士の劫略令'],
	'朔風の恵み':['武将の劫略令', '守護防陣', '英雄', '闘将の劫略令'],
	'守備招来':['守護聖陣', '守護神', '深慮遠謀', '守備聖陣'],
	'守備大聖陣':['弓兵の大聖域', '槍兵の大聖域', '騎兵の大聖域', '守護聖陣'],
	'朱雀の啓示':['隣地極攻', '無双', '覇道', '隣地天豪襲'],
	'獣神の貫攻':['鬼神突貫', '金剛不壊', '神速援護', '鬼神の貫攻'],
	'獣神の極意':['遠地猛攻', '金剛不壊', '神速援護', '鬼神の極意'],
	'獣神の守勢':['遠地猛攻', '金剛不壊', '神速援護', '鬼神の守勢'],
	'兵器の大天撃':['兵器の大神撃', '護君', '攻城の義勇軍', '智将砕器'],
	'斧兵の進撃':['斧兵の進撃', '斧兵防御', '斧兵行軍', '斧兵の強撃'],
	'斧兵の強撃':['斧兵の進撃', '斧兵防御', '斧兵強行', '斧兵の猛撃'],
	'斧兵の猛撃':['斧兵の強撃', '斧兵堅守', '石切技術', '斧兵の極撃'],
	'斧兵の極撃':['斧兵の猛撃', '斧兵方陣', '斧兵舎訓練', '斧兵突覇'],
	'斧兵の大極撃':['斧兵の極撃', '守護神', '斧兵舎修練', '軍神'],
	'斧兵の大神撃':['斧兵の大極撃', '金剛不壊', '斧兵増強', '武神'],
	'斧兵の大天撃':['斧兵の大神撃', '護君', '斧将の采配', '鬼神'],
	'斧兵行軍':['斧兵行軍', '斧兵防御', '斧兵の進撃', '神速'],
	'斧兵突撃':['斧兵強行', '斧兵堅守', '千里行', '斧兵の猛撃'],
	'斧兵突覇':['斧兵突撃', '斧兵方陣', '神速', '一騎当千'],
	'斧兵強行':['斧兵強行', '斧兵堅守', '斧兵の強撃', '斧兵の猛撃'],
	'斧兵の勝鬨':['斧兵の極撃', '斧兵の聖域', '斧兵突覇', '斧将の采配'],
	'斧神降臨':['万夫不当', '斧兵方陣', '斧兵突覇', '斧兵の強攻'],
	'双兵の大神撃':['双兵の大極撃', '金剛不壊', '双兵増強', '武神'],
	'錘兵の大神撃':['錘兵の大極撃', '金剛不壊', '錘兵増強', '武神'],
	'双兵の進撃':['双兵の進撃', '双兵防御', '双兵行軍', '双兵の強撃'],
	'双兵の強撃':['双兵の進撃', '双兵防御', '双兵強行', '双兵の猛撃'],
	'双兵の猛撃':['双兵の強撃', '双兵堅守', '石切技術', '双兵の極撃'],
	'双兵の極撃':['双兵の猛撃', '双兵方陣', '双兵舎訓練', '双兵突覇'],
	'双兵の大極撃':['双兵の極撃', '守護神', '双兵舎修練', '軍神'],
	'双兵の大天撃':['双兵の大神撃', '護君', '双将の采配', '鬼神'],
	'双兵行軍':['双兵行軍', '双兵防御', '双兵の進撃', '神速'],
	'双兵突撃':['双兵強行', '双兵堅守', '千里行', '双兵の猛撃'],
	'双兵突覇':['双兵突撃', '双兵方陣', '神速', '一騎当千'],
	'双兵強行':['双兵強行', '双兵堅守', '双兵の強撃', '双兵の猛撃'],
	'双兵の勝鬨':['双兵の極撃', '双兵の聖域', '双兵突覇', '双将の采配'],
	'双神降臨':['万夫不当', '双兵方陣', '双兵突覇', '双兵の強攻'],
	'錘兵の進撃':['錘兵の進撃', '錘兵防御', '錘兵行軍', '錘兵の強撃'],
	'錘兵の強撃':['錘兵の進撃', '錘兵防御', '錘兵強行', '錘兵の猛撃'],
	'錘兵の猛撃':['錘兵の強撃', '錘兵堅守', '石切技術', '錘兵の極撃'],
	'錘兵の極撃':['錘兵の猛撃', '錘兵方陣', '錘兵舎訓練', '錘兵突覇'],
	'錘兵の大極撃':['錘兵の極撃', '守護神', '錘兵舎修練', '軍神'],
	'錘兵の大天撃':['錘兵の大神撃', '護君', '錘将の采配', '鬼神'],
	'錘兵行軍':['錘兵行軍', '錘兵防御', '錘兵の進撃', '神速'],
	'錘兵突撃':['錘兵強行', '錘兵堅守', '千里行', '錘兵の猛撃'],
	'錘兵突覇':['錘兵突撃', '錘兵方陣', '神速', '一騎当千'],
	'錘兵強行':['錘兵強行', '錘兵堅守', '錘兵の強撃', '錘兵の猛撃'],
	'錘兵の勝鬨':['錘兵の極撃', '錘兵の聖域', '錘兵突覇', '錘将の采配'],
	'錘神降臨':['万夫不当', '錘兵方陣', '錘兵突覇', '錘兵の強攻'],
	'剴切の神砕器':['兵器の大極撃', '金剛不壊', '攻城の檄文', '智将器撃'],
	'神威':['飛将', '護君', '天里行', '無双'],
	'斧兵の大帝撃':['斧兵の大天撃', '天衣無縫', '鋭兵の檄文', '龍神'],
	'双兵の大帝撃':['双兵の大天撃', '天衣無縫', '鋭兵の檄文', '龍神'],
	'錘兵の大帝撃':['錘兵の大天撃', '天衣無縫', '鋭兵の檄文', '龍神'],
	'娘々大烈覇':['強襲烈覇', '金剛不壊', '忠節不落', '攻城の義勇軍'],
	'斧兵の娘天撃':['斧兵の大神撃', '護君', '斧将の采配', '鬼神'],
	'双兵の娘天撃':['双兵の大神撃', '護君', '双将の采配', '鬼神'],
	'錘兵の娘天撃':['錘兵の大神撃', '護君', '錘将の采配', '鬼神'],
	'副将の神明鏡':['万夫不当', '金剛不壊', '万里行', '猛将の極意'],
	'主将の天明鏡':['飛将', '護君', '天里行', '鬼神の極意'],
	'主将の天明鑑':['飛将', '護君', '天里行', '鬼神の極意'],
	'主将の神明鏡':['万夫不当', '金剛不壊', '万里行', '猛将の極意'],
	'十宴祭の天明鏡':['飛将', '護君', '天里行', '鬼神の極意'],
	'覇王の天明鏡':['飛将', '護君', '天里行', '鬼神の極意'],
	'群雄の神明鏡':['万夫不当', '金剛不壊', '万里行', '猛将の極意'],
	'王春の天明鏡':['飛将', '護君', '天里行', '鬼神の極意'],
	'主将の大明鏡':['一騎当千', '守護神', '千里行', '闘将の極意'],
	'加加阿の天明鏡':['飛将', '護君', '天里行', '鬼神の極意'],
	'覇王の極賢斬':['覇王の進撃', '王者の護り', '神速', '覇王の強撃'],
	'覇王の極護斬':['覇王の進撃', '王者の護り', '神速', '覇王の強撃'],
	'覇王の極俊賢':['覇王の進撃', '王者の護り', '神速', '覇王の強撃'],
	'覇王の極俊護':['覇王の進撃', '王者の護り', '神速', '覇王の強撃'],
	'覇王の極賢護':['覇王の進撃', '王者の護り', '神速', '覇王の強撃'],
	'白鶴の極護斬':['覇王の進撃', '王者の護り', '神速', '覇王の強撃'],
	'覇王の天賢斬':['覇王の猛撃', '防将戦略', '神速援護', '軍神'],
	'覇王の天護斬':['覇王の猛撃', '防将戦略', '神速援護', '軍神'],
	'覇王の天俊賢':['覇王の猛撃', '防将戦略', '神速援護', '軍神'],
	'覇王の天俊護':['覇王の猛撃', '防将戦略', '神速援護', '軍神'],
	'覇王の天賢護':['覇王の猛撃', '防将戦略', '神速援護', '軍神'],
	'兵器増強':['恵風', '趁火打劫', '兵器増強', '蛮王の襲撃'],
	'斧兵の大聖域':['斧兵の聖域', '斧兵舎修練', '神速', '王者の護り'],
	'斧兵の大神域':['斧兵の大聖域', '斧兵増強', '烈速', '忠節不落'],
	'斧兵の大天域':['斧兵の大神域', '斧将の采配', '飛速', '覇者の護り'],
	'斧兵舎訓練':['斧兵舎訓練', '斧兵方陣', '斧兵の猛撃', '練兵修練'],
	'斧兵舎修練':['斧兵舎修練', '斧兵の聖域', '斧兵の極撃', '加工技術'],
	'斧兵増強':['斧兵の猛撃', '斧兵方陣', '斧兵増強', '軍神'],
	'斧兵増勢':['斧兵速攻', '斧兵方陣', '神速', '斧兵突攻'],
	'斧兵防御':['石切知識', '斧兵の進撃', '斧兵行軍', '斧兵堅守'],
	'斧兵堅守':['斧兵防御', '斧兵の進撃', '斧兵行軍', '斧兵方陣'],
	'斧兵方陣':['斧兵堅守', '斧兵の強撃', '斧兵強行', '斧兵の聖域'],
	'斧兵の聖域':['斧兵方陣', '斧兵舎訓練', '斧兵強行', '八卦の陣'],
	'双兵の大神域':['双兵の大聖域', '双兵増強', '烈速', '忠節不落'],
	'双兵舎修練':['双兵舎修練', '双兵の聖域', '双兵の極撃', '加工技術'],
	'錘兵の大神域':['錘兵の大聖域', '錘兵増強', '烈速', '忠節不落'],
	'錘兵舎修練':['錘兵舎修練', '錘兵の聖域', '錘兵の極撃', '加工技術'],
	'双兵の大聖域':['双兵の聖域', '双兵舎修練', '神速', '王者の護り'],
	'双兵の大天域':['双兵の大神域', '双将の采配', '飛速', '覇者の護り'],
	'双兵舎訓練':['双兵舎訓練', '双兵方陣', '双兵の猛撃', '練兵修練'],
	'双兵増強':['双兵の猛撃', '双兵方陣', '双兵増強', '軍神'],
	'双兵増勢':['双兵速攻', '双兵方陣', '神速', '双兵突攻'],
	'双兵防御':['石切知識', '双兵の進撃', '双兵行軍', '双兵堅守'],
	'双兵堅守':['双兵防御', '双兵の進撃', '双兵行軍', '双兵方陣'],
	'双兵方陣':['双兵堅守', '双兵の強撃', '双兵強行', '双兵の聖域'],
	'双兵の聖域':['双兵方陣', '双兵舎訓練', '双兵強行', '八卦の陣'],
	'錘兵の大聖域':['錘兵の聖域', '錘兵舎修練', '神速', '王者の護り'],
	'錘兵の大天域':['錘兵の大神域', '錘将の采配', '飛速', '覇者の護り'],
	'錘兵舎訓練':['錘兵舎訓練', '錘兵方陣', '錘兵の猛撃', '練兵修練'],
	'錘兵増強':['錘兵の猛撃', '錘兵方陣', '錘兵増強', '軍神'],
	'錘兵増勢':['錘兵速攻', '錘兵方陣', '神速', '錘兵突攻'],
	'錘兵防御':['石切知識', '錘兵の進撃', '錘兵行軍', '錘兵堅守'],
	'錘兵堅守':['錘兵防御', '錘兵の進撃', '錘兵行軍', '錘兵方陣'],
	'錘兵方陣':['錘兵堅守', '錘兵の強撃', '錘兵強行', '錘兵の聖域'],
	'錘兵の聖域':['錘兵方陣', '錘兵舎訓練', '錘兵強行', '八卦の陣'],
	'鋭兵の檄文':['鬼神の鹵獲', '覇者の護り', '鋭兵の檄文', '神医の施術'],
	'斧兵の義勇軍':['斧兵の義勇軍', '斧兵の大神域', '斧兵の大神撃', '神医の術式'],
	'双兵の義勇軍':['双兵の義勇軍', '双兵の大神域', '双兵の大神撃', '神医の術式'],
	'錘兵の義勇軍':['錘兵の義勇軍', '錘兵の大神域', '錘兵の大神撃', '神医の術式'],
	'龍神の極意':['遠地猛攻', '金剛不壊', '神速援護', '鬼神の極意'],
	'鶴速雷光':['烈速雷光', '剣将の采配', '混元一気', '飛速'],
	'斧兵の進攻':['斧兵の進攻', '斧兵防御', '斧兵強行', '斧兵の強撃'],
	'斧兵の強攻':['斧兵の強撃', '斧兵防御', '斧兵舎訓練', '斧兵の強攻'],
	'斧兵の猛攻':['斧兵の強攻', '斧兵堅守', '斧兵舎修練', '斧兵の猛撃'],
	'斧兵の極攻':['斧兵の猛撃', '斧兵の聖域', '斧兵増強', '斧兵の猛攻'],
	'斧兵速攻':['斧兵突撃', '斧兵強行', '斧兵舎訓練', '斧兵突覇'],
	'斧兵突攻':['斧兵突覇', '神速', '斧兵舎修練', '斧兵速攻'],
	'斧兵の大極攻':['斧兵の猛攻', '斧兵の大聖域', '斧将の采配', '斧兵の極攻'],
	'斧兵烈攻':['斧兵の大極撃', '烈速', '斧兵増強', '斧兵突攻'],
	'斧兵大烈攻':['斧兵の大神撃', '飛速', '斧将の采配', '斧兵烈攻'],
	'双兵の極攻':['双兵の猛撃', '双兵の聖域', '双兵増強', '双兵の猛攻'],
	'錘兵の極攻':['錘兵の猛撃', '錘兵の聖域', '錘兵増強', '錘兵の猛攻'],
	'祝龍の攻奪':['猛将の鹵獲', '金剛不壊', '豊穣', '鹵獲の猛攻'],
	'剴切の天攻奪':['鬼神の鹵獲', '護君', '富国論', '鹵獲の極攻'],
	'双兵の進攻':['双兵の進攻', '双兵防御', '双兵強行', '双兵の強撃'],
	'双兵の強攻':['双兵の強撃', '双兵防御', '双兵舎訓練', '双兵の強攻'],
	'双兵の猛攻':['双兵の強攻', '双兵堅守', '双兵舎修練', '双兵の猛撃'],
	'双兵速攻':['双兵突撃', '双兵強行', '双兵舎訓練', '双兵突覇'],
	'双兵突攻':['双兵突覇', '神速', '双兵舎修練', '双兵速攻'],
	'双兵の大極攻':['双兵の猛攻', '双兵の大聖域', '双将の采配', '双兵の極攻'],
	'双兵烈攻':['双兵の大極撃', '烈速', '双兵増強', '双兵突攻'],
	'双兵大烈攻':['双兵の大神撃', '飛速', '双将の采配', '双兵烈攻'],
	'錘兵の進攻':['錘兵の進攻', '錘兵防御', '錘兵強行', '錘兵の強撃'],
	'錘兵の強攻':['錘兵の強撃', '錘兵防御', '錘兵舎訓練', '錘兵の強攻'],
	'錘兵の猛攻':['錘兵の強攻', '錘兵堅守', '錘兵舎修練', '錘兵の猛撃'],
	'錘兵速攻':['錘兵突撃', '錘兵強行', '錘兵舎訓練', '錘兵突覇'],
	'錘兵突攻':['錘兵突覇', '神速', '錘兵舎修練', '錘兵速攻'],
	'錘兵の大極攻':['錘兵の猛攻', '錘兵の大聖域', '錘将の采配', '錘兵の極攻'],
	'錘兵烈攻':['錘兵の大極撃', '烈速', '錘兵増強', '錘兵突攻'],
	'錘兵大烈攻':['錘兵の大神撃', '飛速', '錘将の采配', '錘兵烈攻'],
	'南瓜の攻奪':['猛将の鹵獲', '金剛不壊', '豊穣', '鹵獲の猛攻'],
	'雪華の攻奪':['猛将の鹵獲', '金剛不壊', '豊穣', '鹵獲の猛攻'],
	'聖夜の神攻奪':['猛将の鹵獲', '金剛不壊', '豊穣', '鹵獲の猛攻'],
	'隣地天豪撃':['闘将の極意', '謀反の強攻', '攻城の義勇軍', '飛将'],
	'兵器大烈攻':['兵器の大神撃', '飛速', '攻城の檄文', '兵器烈攻'],
	'十宴祭の極意':['遠地猛攻', '金剛不壊', '神速援護', '鬼神の極意'],
	'十宴祭の貫攻':['鬼神突貫', '金剛不壊', '神速援護', '鬼神の貫攻'],
	'十宴祭の攻奪':['鬼神の鹵獲', '護君', '富国論', '鹵獲の極攻'],
	'闘将の攻奪':['一騎当千', '鉄壁', '農林技術', '鹵獲の進攻'],
	'武将の攻奪':['豪傑', '八卦の陣', '農林知識', '趁火打劫'],
	'斧軍の神遠攻':['斧兵の極攻', '飛速雷光', '鋭兵の檄文', '斧兵烈攻'],
	'双軍の神遠攻':['双兵の極攻', '飛速雷光', '鋭兵の檄文', '双兵烈攻'],
	'錘軍の神遠攻':['錘兵の極攻', '飛速雷光', '鋭兵の檄文', '錘兵烈攻'],
	'剴切の天砕攻':['兵器の猛攻', '忠節不落', '兵器増強', '兵器の極攻'],
	'熊猫大聖陣':['弓兵の大聖域', '槍兵の大聖域', '騎兵の大聖域', '守護聖陣'],
	'桃園の極攻令':['覇王の猛撃', '覇者の護り', '混元一気', '全軍の猛攻令'],
	'全兵の猛攻令':['全軍の強攻令', '忠節不落', '神算鬼謀', '覇王の猛撃'],
	'壮士聖陣':['弓将の采配', '騎将の采配', '槍将の采配', '守護方陣'],
	'祝龍の大極攻令':['全軍の猛攻令', '護君', '智将の大神撃', '全軍の極攻令'],
	'全兵の鳳速令':['全軍の烈速令', '覇者の護り', '市場繁栄', '飛速'],
	'猛士の劫略令':['鹵獲の強攻', '金剛不壊', '万里行', '猛将の鹵獲'],
	'全兵の飛速令':['烈速', '護君', '市場知識', '全軍の烈速令'],
	'全兵の烈速令':['全軍の神速令', '金剛不壊', '神速援護', '烈速'],
	'龍神の叛攻令':['猛将の叛攻令', '覇者の護り', '智将の大神撃', '鬼神の叛攻令'],
	'猛将の叛攻令':['闘将の叛攻令', '金剛不壊', '神算鬼謀', '謀反の猛攻'],
	'闘将の叛攻令':['謀反の強攻', '守護神', '深慮遠謀', '闘将の叛攻令'],
	'武将の叛攻令':['武将の叛攻令', '鉄壁', '奇計百出', '謀反の進攻'],
	'剴切の天劫令':['鹵獲の猛攻', '覇者の護り', '天里行', '鹵獲の極攻'],
	'剴切の神劫令':['猛将の鹵獲', '護君', '烈速', '鹵獲の猛攻'],
	'黄巾の神劫令':['猛将の鹵獲', '護君', '烈速', '鹵獲の猛攻'],
	'樅樹の叛攻令':['猛将の叛攻令', '覇者の護り', '智将の大神撃', '鬼神の叛攻令'],
	'舞姫の大極攻令':['全軍の猛攻令', '護君', '智将の大神撃', '全軍の極攻令'],
	'魏帝の叛攻令':['猛将の叛攻令', '覇者の護り', '智将の大神撃', '鬼神の叛攻令'],
	'孫呉大聖陣':['弓兵の大聖域', '槍兵の大聖域', '騎兵の大聖域', '守護聖陣'],
	'十宴祭の守勢':['遠地猛攻', '金剛不壊', '神速援護', '鬼神の守勢'],
	'十宴祭の大極攻令':['全軍の猛攻令', '護君', '智将の大神撃', '全軍の極攻令'],
	'全軍の大神攻令':['全軍の極攻令', '覇者の大神域', '智将の大天撃', '全軍の大極攻令'],
	'剴切の鳳速令':['全軍の烈速令', '覇者の護り', '市場繁栄', '飛速'],
	'剴切の天叛令':['猛将の叛攻令', '覇者の護り', '智将の大神撃', '鬼神の叛攻令'],
	'剴切の天療令':['鹵獲の猛攻', '覇者の護り', '神医の施術', '鹵獲の極攻'],
	'龍神の療略令':['鹵獲の猛攻', '覇者の護り', '神医の施術', '鹵獲の極攻'],
	'鬼神の療略令':['猛将の鹵獲', '護君', '神医の術式', '鹵獲の猛攻'],
	'猛将の療略令':['鹵獲の強攻', '金剛不壊', '発憤興起', '猛将の鹵獲'],
	'闘将の療略令':['趁火打劫', '守護神', '桃色吐息', '鹵獲の強攻'],
	'武将の療略令':['鹵獲の進攻', '鉄壁', '仁君', '趁火打劫'],
	'南瓜の療略令':['猛将の鹵獲', '護君', '神医の術式', '鹵獲の猛攻'],
	'樅樹の療略令':['鹵獲の猛攻', '覇者の護り', '神医の施術', '鹵獲の極攻'],
	'加護帝聖壁':['加護聖壁', '加護方壁', '飛速雷光', '加護大聖壁'],
	'加護大聖壁':['加護方壁', '加護堅壁', '烈速雷光', '加護聖壁'],
	'加護聖壁':['加護堅壁', '加護防壁', '神速雷光', '加護方壁'],
	'加護方壁':['加護防壁', '忠節不落', '烈速', '加護堅壁'],
	'加護堅壁':['忠節不落', '王者の護り', '神速', '加護防壁'],
	'加護防壁':['王者の護り', '八卦の陣', '剣兵行軍', '加護防壁'],
	'聖夜の神恵沢':['神算鬼謀', '忠節不落', '桃色吐息', '攻城の檄文'],
	'聖夜の聖恵沢':['深慮遠謀', '王者の護り', '仁君', '兵器修練'],
	'機略謀略':['槍兵の大号令', '弓兵の大号令', '騎兵の大号令', '智将砕器'],
	'花紅の舞':['剣兵突覇', '守護防陣', '加工技術', '覇者の護り'],
	'神卜の方術':['一騎当千', '八卦の陣', '覇道', '神医の術式'],
	'斧将の采配':['斧兵速攻', '斧兵方陣', '神速', '斧兵突攻'],
	'双将の采配':['双兵速攻', '双兵方陣', '神速', '双兵突攻'],
	'錘将の采配':['錘兵速攻', '錘兵方陣', '神速', '錘兵突攻'],
	'青龍の啓示':['龍神突貫', '龍神の極意', '覇道', '龍神の貫攻'],
	'白虎の啓示':['智将天砕器', '兵器の大天撃', '覇道', '兵器の大極攻'],
	'槍神の導き':['槍兵の極攻', '槍兵烈攻', '覇道', '槍兵の大極攻'],
	'斧神の導き':['斧兵の極攻', '斧兵烈攻', '覇道', '斧兵の大極攻'],
	'双神の導き':['双兵の極攻', '双兵烈攻', '覇道', '双兵の大極攻'],
	'錘神の導き':['錘兵の極攻', '錘兵烈攻', '覇道', '錘兵の大極攻'],
	'旋風の恵み':['武将の攻奪', '守護防陣', '英雄', '闘将の攻奪'],
	'鋭兵大転聖':['鋭兵大転聖', '覇者の護り', '飛速', '富国論'],
	'聖夜大転聖':['鋭兵大転聖', '覇者の護り', '飛速', '富国論'],
	'官女大転聖':['鋭兵大転聖', '覇者の護り', '飛速', '富国論'],
	'':['', '', '', '']
};
/*
錘軍の神遠攻
錘神降臨
錘兵の義勇軍
錘兵の強撃
錘兵の極攻
錘兵の大神撃
錘兵の大聖域
錘兵の大帝撃
錘兵の大天域
錘兵の大天撃
錘兵の猛攻
錘兵舎訓練
錘兵突攻
錘兵烈攻
聖夜の神恵沢
聖夜の神攻奪
聖夜の聖恵沢
聖夜大転聖
青龍の啓示
雪華の攻奪
旋風の恵み
全兵の飛速令
全兵の猛攻令
全兵の烈速令
双軍の神遠攻
双神降臨
双兵の義勇軍
双兵の強撃
双兵の極攻
双兵の大神撃
双兵の大聖域
双兵の大帝撃
双兵の大天域
双兵の大天撃
双兵の猛攻
双兵舎訓練
双兵突攻
双兵烈攻
槍神の導き
孫呉大聖陣
闘将の攻奪
闘将の叛攻令
南瓜の攻奪
南瓜の療略令
白虎の啓示
斧軍の神遠攻
斧神降臨
斧兵の義勇軍
斧兵の強撃
斧兵の極攻
斧兵の大神撃
斧兵の大聖域
斧兵の大帝撃
斧兵の大天域
斧兵の大天撃
斧兵の猛攻
斧兵舎訓練
斧兵突攻
斧兵烈攻
武将の攻奪
武将の劫略令
武将の叛攻令
副将の神明鏡
兵器の大天撃
兵器大烈攻
猛士の守勢
猛将の叛攻令
猛将の療略令
龍神
龍神の極意
龍神の叛攻令
龍神の療略令
隣地天豪撃
剴切の神砕器
剴切の天攻奪
剴切の天劫令
剴切の天療令
剴切の鳳速令
樅樹の叛攻令
魏帝の叛攻令

*/

// カードIDからレアリティ引く際に使うようにしているため、メンテ必須かつ移動禁止(2014/12/20)
const card_list = {
	"1001":{n:"劉備", r:"R", c:2.5, a:260, i:10, d1:330, bs:"仁君"},
	"1002":{n:"諸葛亮", r:"SR", c:3.5, a:400, i:25, d1:495, bs:"神算鬼謀"},
	"1003":{n:"関羽", r:"SR", c:4, a:475, i:13, d1:625, bs:"軍神"},
	"1004":{n:"張飛", r:"SR", c:3, a:360, i:7, d1:295, bs:"槍兵の猛撃"},
	"1005":{n:"趙雲", r:"R", c:3, a:335, i:10, d1:345, bs:"騎兵突撃"},
	"1006":{n:"馬超", r:"SR", c:3, a:350, i:5, d1:322, bs:"騎兵の猛撃"},
	"1007":{n:"劉備", r:"UC", c:2.5, a:245, i:10, d1:325, bs:"仁君"},
	"1008":{n:"諸葛亮", r:"R", c:2.5, a:190, i:17, d1:275, bs:"食糧知識"},
	"1009":{n:"諸葛亮", r:"UC", c:2.5, a:190, i:16, d1:260, bs:"奇計百出"},
	"1010":{n:"関羽", r:"UC", c:2.5, a:270, i:7, d1:295, bs:"槍兵の進撃"},
	"1011":{n:"関羽", r:"R", c:2.5, a:285, i:7, d1:300, bs:"槍兵の進撃"},
	"1012":{n:"張飛", r:"UC", c:2.5, a:290, i:4, d1:230, bs:"豪傑"},
	"1013":{n:"張飛", r:"C", c:2.5, a:275, i:4, d1:230, bs:"豪傑"},
	"1014":{n:"徐庶", r:"UC", c:2, a:220, i:15, d1:300, bs:"奇計百出"},
	"1015":{n:"黄忠", r:"UC", c:2, a:235, i:5, d1:255, bs:"弓兵突撃"},
	"1016":{n:"龐統", r:"R", c:2.5, a:240, i:23, d1:260, bs:"兵器の進撃"},
	"1017":{n:"廖化", r:"UC", c:2, a:205, i:5, d1:235, bs:"槍兵防御"},
	"1018":{n:"廖化", r:"C", c:2, a:200, i:5, d1:225, bs:"槍兵防御"},
	"1019":{n:"馬岱", r:"UC", c:2.5, a:245, i:6, d1:245, bs:"騎兵の進撃"},
	"1020":{n:"馬岱", r:"C", c:2.5, a:235, i:6, d1:245, bs:"騎兵の進撃"},
	"1021":{n:"周倉", r:"UC", c:2, a:220, i:4, d1:125, bs:"豪傑"},
	"1022":{n:"関平", r:"UC", c:2, a:195, i:7, d1:205, bs:"槍兵の進撃"},
	"1023":{n:"伊籍", r:"UC", c:2, a:55, i:12, d1:85, bs:"伐採知識"},
	"1024":{n:"伊籍", r:"C", c:2, a:55, i:12, d1:70, bs:"伐採知識"},
	"1025":{n:"沙摩柯", r:"UC", c:1.5, a:155, i:1, d1:100, bs:"剣兵の進撃"},
	"1026":{n:"沙摩柯", r:"C", c:1.5, a:145, i:1, d1:100, bs:"剣兵の進撃"},
	"1027":{n:"簡雍", r:"UC", c:1.5, a:65, i:9, d1:55, bs:"食糧知識"},
	"1028":{n:"簡雍", r:"C", c:1.5, a:65, i:9, d1:45, bs:"食糧知識"},
	"1029":{n:"雷銅", r:"UC", c:2, a:195, i:5, d1:170, bs:"槍兵の進撃"},
	"1030":{n:"雷銅", r:"C", c:2, a:185, i:5, d1:170, bs:"槍兵の進撃"},
	"1031":{n:"魏延", r:"UC", c:2.5, a:265, i:7, d1:235, bs:"槍兵突撃"},
	"1032":{n:"魏延", r:"C", c:2.5, a:250, i:7, d1:235, bs:"槍兵の進撃"},
	"1033":{n:"馬謖", r:"UC", c:2, a:170, i:13, d1:200, bs:"製鉄知識"},
	"1034":{n:"馬謖", r:"C", c:2, a:165, i:13, d1:190, bs:"剣兵の進撃"},
	"1035":{n:"劉備", r:"SR", c:2.5, a:275, i:12, d1:355, bs:"昭烈帝"},
	"1036":{n:"徐庶", r:"R", c:2, a:220, i:15, d1:320, bs:"八卦の陣"},
	"1037":{n:"趙雲", r:"SR", c:3.5, a:425, i:17, d1:425, bs:"槍兵突覇"},
	"1038":{n:"黄忠", r:"R", c:2.5, a:265, i:11, d1:300, bs:"弓兵の猛撃"},
	"1039":{n:"厳顔", r:"UC", c:2, a:225, i:4, d1:255, bs:"弓兵方陣"},
	"1040":{n:"厳顔", r:"C", c:2, a:205, i:4, d1:255, bs:"弓兵堅守"},
	"1041":{n:"姜維", r:"SR", c:3.5, a:400, i:22, d1:435, bs:"麒麟児"},
	"1042":{n:"魏延", r:"R", c:2.5, a:280, i:8, d1:240, bs:"槍兵の猛撃"},
	"1043":{n:"馬超", r:"R", c:2.5, a:290, i:3, d1:225, bs:"騎兵突覇"},
	"1044":{n:"夏侯氏", r:"SR", c:1, a:20, i:9, d1:100, bs:"伐採技巧"},
	"1045":{n:"法正", r:"UC", c:2.5, a:185, i:16, d1:265, bs:"全軍の進攻令"},
	"1046":{n:"劉備", r:"UR", c:3.5, a:420, i:20, d1:520, bs:"昭烈帝"},
	"1047":{n:"諸葛亮", r:"UR", c:3.5, a:400, i:26, d1:495, bs:"臥龍覚醒"},
	"1048":{n:"関羽", r:"UR", c:4, a:485, i:14, d1:625, bs:"軍神"},
	"1049":{n:"趙雲", r:"UR", c:3.5, a:430, i:16, d1:530, bs:"強襲突覇"},
	"1050":{n:"張飛", r:"UR", c:3.5, a:445, i:7, d1:500, bs:"槍兵の極撃"},
	"1051":{n:"馬超", r:"UR", c:3.5, a:435, i:7, d1:500, bs:"騎兵突覇"},
	"1052":{n:"黄忠", r:"SR", c:3, a:310, i:13, d1:395, bs:"弓兵突覇"},
	"1053":{n:"黄月英", r:"SR", c:1, a:10, i:10, d1:90, bs:"槍兵増強"},
	"1054":{n:"甘夫人", r:"SR", c:1, a:10, i:8, d1:70, bs:"騎兵増強"},
	"1055":{n:"沙摩柯", r:"R", c:1.5, a:175, i:2, d1:110, bs:"強襲突撃"},
	"1056":{n:"張飛", r:"R", c:3, a:335, i:5, d1:260, bs:"槍兵の強撃"},
	"1057":{n:"龐統", r:"SR", c:2, a:220, i:17, d1:250, bs:"市場繁栄"},
	"1058":{n:"関平", r:"C", c:2, a:185, i:7, d1:205, bs:"急速援護"},
	"1059":{n:"黄忠", r:"C", c:2, a:230, i:5, d1:255, bs:"弓兵強行"},
	"1060":{n:"馬岱", r:"R", c:2.5, a:260, i:7, d1:290, bs:"騎兵の強撃"},
	"1061":{n:"姜維", r:"UR", c:3.5, a:405, i:23, d1:440, bs:"槍将の采配"},
	"1062":{n:"劉備", r:"UR", c:3.5, a:420, i:20, d1:520, bs:"蜀軍の極撃"},
	"1063":{n:"趙雲", r:"UC", c:2.5, a:280, i:8, d1:230, bs:"騎兵の強撃"},
	"1064":{n:"王悦", r:"SR", c:4, a:450, i:8, d1:440, bs:"勇将天戟"},
	"1065":{n:"孟達", r:"R", c:3.5, a:405, i:6, d1:410, bs:"迅速劫略"},
	"1066":{n:"魏延", r:"SR", c:3, a:345, i:8, d1:310, bs:"槍兵の極撃"},
	"1067":{n:"糜夫人", r:"SR", c:1, a:10, i:9, d1:120, bs:"豊穣"},
	"1068":{n:"周倉", r:"R", c:2, a:260, i:4, d1:250, bs:"一騎当千"},
	"1069":{n:"伊籍", r:"R", c:2, a:55, i:16, d1:105, bs:"伐採技術"},
	"1070":{n:"龐統", r:"UC", c:2.5, a:180, i:17, d1:240, bs:"兵器の強撃"},
	"1071":{n:"劉禅", r:"SR", c:1, a:20, i:5, d1:195, bs:"蜀軍の防衛"},
	"1072":{n:"厳顔", r:"R", c:2, a:230, i:7, d1:265, bs:"弓兵修練"},
	"1073":{n:"黄忠", r:"R", c:3, a:350, i:12, d1:305, bs:"一騎当千"},
	"1074":{n:"馬岱", r:"SR", c:3, a:340, i:8, d1:355, bs:"騎兵の極撃"},
	"1075":{n:"黄忠", r:"UR", c:3.5, a:440, i:13, d1:460, bs:"勇将飛矢"},
	"1076":{n:"姜維", r:"R", c:3, a:330, i:17, d1:330, bs:"奇計百出"},
	"1077":{n:"敬哀皇后", r:"SR", c:1, a:10, i:10, d1:145, bs:"護国芳心"},
	"1078":{n:"馬超", r:"UC", c:2.5, a:270, i:3, d1:225, bs:"豪傑"},
	"1079":{n:"関羽", r:"UR", c:4, a:500, i:15, d1:625, bs:"武神"},
	"1080":{n:"夏侯覇", r:"SR", c:3, a:330, i:10, d1:450, bs:"槍兵突覇"},
	"1081":{n:"法正", r:"SR", c:2.5, a:220, i:19, d1:260, bs:"槍兵の勝鬨"},
	"1082":{n:"馬謖", r:"R", c:2, a:200, i:15, d1:240, bs:"厩舎修練"},
	"1083":{n:"諸葛亮", r:"UR", c:3.5, a:400, i:27, d1:495, bs:"醒龍出師"},
	"1084":{n:"徐庶", r:"SR", c:3, a:310, i:21, d1:400, bs:"智将器撃"},
	"1085":{n:"王平", r:"R", c:2.5, a:280, i:10, d1:355, bs:"守護神"},
	"1086":{n:"張飛", r:"UR", c:3.5, a:470, i:8, d1:480, bs:"燕人武陣"},
	"1087":{n:"関興", r:"SR", c:3.5, a:430, i:10, d1:450, bs:"槍兵の猛撃"},
	"1088":{n:"関平", r:"R", c:2.5, a:280, i:10, d1:300, bs:"千里行"},
	"1089":{n:"劉備", r:"R", c:2.5, a:260, i:10, d1:330, bs:"八卦の陣"},
	"1090":{n:"関羽", r:"UC", c:2.5, a:270, i:7, d1:295, bs:"槍兵突撃"},
	"1091":{n:"張飛", r:"UC", c:2.5, a:290, i:4, d1:230, bs:"槍兵の強撃"},
	"1092":{n:"魏延", r:"UR", c:3.5, a:450, i:12, d1:410, bs:"拠点襲撃"},
	"1093":{n:"沙摩柯", r:"SR", c:2, a:280, i:2, d1:140, bs:"胡王の猛襲"},
	"1094":{n:"馬超", r:"SR", c:3.5, a:430, i:5, d1:420, bs:"軍神"},
	"1095":{n:"趙雲", r:"UR", c:4, a:500, i:20, d1:520, bs:"蜀将の督戦"},
	"1096":{n:"関索", r:"SR", c:3.5, a:400, i:10, d1:470, bs:"勇美鼓舞"},
	"1097":{n:"劉封", r:"R", c:2.5, a:250, i:8, d1:345, bs:"守護堅陣"},
	"1098":{n:"劉備", r:"UR", c:3.5, a:460, i:21, d1:535, bs:"皇叔の号令"},
	"1099":{n:"黄忠", r:"SR", c:3.5, a:430, i:13, d1:430, bs:"老将統帥"},
	"1100":{n:"糜夫人", r:"R", c:1, a:10, i:10, d1:120, bs:"憂姫護国"},
	"1101":{n:"関羽", r:"UC", c:3, a:350, i:9, d1:345, bs:"槍兵の強撃"},
	"1102":{n:"姜維", r:"UR", c:3.5, a:430, i:23, d1:440, bs:"麒麟慈心"},
	"1103":{n:"関銀屏", r:"SR", c:3, a:320, i:12, d1:320, bs:"槍兵の猛攻"},
	"1104":{n:"夏侯氏", r:"R", c:1, a:10, i:8, d1:80, bs:"恵風"},
	"1105":{n:"馬謖", r:"SR", c:2.5, a:230, i:18, d1:300, bs:"矛槍兵移送"},
	"1106":{n:"龐統", r:"UR", c:3.5, a:360, i:27, d1:480, bs:"連環の計"},
	"1107":{n:"張飛", r:"R", c:3.5, a:400, i:5, d1:430, bs:"猛虎進撃"},
	"1108":{n:"王平", r:"UC", c:2.5, a:260, i:9, d1:340, bs:"槍兵方陣"},
	"1109":{n:"夏侯覇", r:"R", c:2.5, a:280, i:9, d1:380, bs:"趁火打劫"},
	"1110":{n:"劉備", r:"UR", c:3.5, a:435, i:20, d1:505, bs:"槍兵の極攻"},
	"1111":{n:"周倉", r:"SR", c:3, a:345, i:4, d1:335, bs:"迅速劫略"},
	"1112":{n:"関興", r:"R", c:3, a:330, i:8, d1:355, bs:"発憤興起"},
	"1113":{n:"鮑三娘", r:"SR", c:2, a:275, i:5, d1:130, bs:"戦女の行軍"},
	"1114":{n:"関索", r:"R", c:1.5, a:230, i:7, d1:210, bs:"一騎当千"},
	"1115":{n:"黄月英", r:"UR", c:1, a:20, i:12, d1:120, bs:"賢女の策略"},
	"1116":{n:"馬岱", r:"R", c:2.5, a:290, i:7, d1:265, bs:"騎兵の強攻"},
	"1117":{n:"馬雲緑", r:"SR", c:3.5, a:400, i:6, d1:310, bs:"騎神降臨"},
	"1118":{n:"張姫", r:"SR", c:3.5, a:415, i:8, d1:350, bs:"槍神降臨"},
	"1119":{n:"厳顔", r:"SR", c:3, a:340, i:9, d1:460, bs:"勇猛果敢"},
	"1120":{n:"雷銅", r:"R", c:2.5, a:285, i:7, d1:275, bs:"槍兵速攻"},
	"1121":{n:"馬良", r:"R", c:2, a:150, i:17, d1:260, bs:"槍兵急配"},
	"1122":{n:"蒋琬", r:"R", c:1.5, a:110, i:13, d1:105, bs:"製鉄技術"},
	"1123":{n:"劉禅", r:"UC", c:1.5, a:40, i:6, d1:120, bs:"仁君"},
	"1124":{n:"王桃", r:"UR", c:2, a:265, i:4, d1:140, bs:"桃賊の襲撃"},
	"1125":{n:"劉備・諸葛", r:"UR", c:4, a:510, i:24, d1:635, bs:"劉備の大徳"},
	"1126":{n:"姜維", r:"SR", c:1.5, a:150, i:17, d1:255, bs:"槍兵の大号令"},
	"1127":{n:"孫尚香", r:"R", c:1.5, a:225, i:9, d1:240, bs:"勇姫督励"},
	"1128":{n:"趙雲", r:"R", c:3.5, a:400, i:17, d1:445, bs:"飛将"},
	"1129":{n:"黄忠", r:"UC", c:3, a:350, i:12, d1:360, bs:"弓兵の進攻"},
	"1130":{n:"沙摩柯", r:"UR", c:2, a:295, i:3, d1:185, bs:"胡王奮迅"},
	"1131":{n:"関興", r:"UR", c:2, a:290, i:11, d1:290, bs:"神速劫略"},
	"1132":{n:"張飛", r:"UR", c:4, a:525, i:8, d1:500, bs:"燕神破軍"},
	"1133":{n:"張姫", r:"UC", c:3, a:295, i:7, d1:330, bs:"槍兵速攻"},
	"1134":{n:"孫尚香", r:"SR", c:1.5, a:210, i:14, d1:265, bs:"劉備の契り"},
	"1135":{n:"趙雲", r:"SR", c:3.5, a:410, i:10, d1:425, bs:"騎兵の猛攻"},
	"1136":{n:"関羽", r:"R", c:3, a:320, i:9, d1:335, bs:"騎兵の極撃"},
	"1137":{n:"張飛", r:"R", c:3, a:330, i:5, d1:330, bs:"槍兵の極撃"},
	"1138":{n:"黄忠", r:"R", c:3, a:340, i:11, d1:345, bs:"弓兵の極撃"},
	"1139":{n:"穆皇后", r:"SR", c:1.5, a:20, i:10, d1:200, bs:"賢妃施政"},
	"1140":{n:"姜維", r:"UC", c:2.5, a:270, i:16, d1:325, bs:"兵器の猛撃"},
	"1141":{n:"諸葛亮", r:"UR", c:3.5, a:350, i:22, d1:440, bs:"三顧の礼"},
	"1142":{n:"馬雲緑", r:"R", c:3.5, a:395, i:6, d1:280, bs:"猛将の極意"},
	"1143":{n:"呉蘭", r:"R", c:4, a:400, i:9, d1:445, bs:"闘将の極意"},
	"1144":{n:"夏侯覇", r:"UC", c:3, a:330, i:8, d1:320, bs:"闘将突貫"},
	"1145":{n:"廖化", r:"SR", c:4, a:480, i:7, d1:595, bs:"強襲烈覇"},
	"1146":{n:"関銀屏", r:"R", c:3.5, a:415, i:10, d1:420, bs:"槍兵突攻"},
	"1147":{n:"王平", r:"SR", c:4, a:380, i:13, d1:605, bs:"槍兵の大聖域"},
	"1148":{n:"黄月英", r:"UC", c:3, a:140, i:19, d1:315, bs:"兵舎修練"},
	"1149":{n:"甘夫人", r:"UC", c:3, a:130, i:17, d1:330, bs:"厩舎修練"},
	"1150":{n:"関銀屏・関羽", r:"UR", c:4, a:530, i:15, d1:520, bs:"天里雷光"},
	"1151":{n:"沙摩柯", r:"SR", c:4, a:460, i:3, d1:450, bs:"勇将飛矢"},
	"1152":{n:"黄忠・厳顔", r:"UR", c:4, a:550, i:15, d1:495, bs:"歴戦の弓将"},
	"1153":{n:"孫尚香", r:"SR", c:1.5, a:150, i:10, d1:165, bs:"勇躍の舞"},
	"1154":{n:"張姫", r:"R", c:3.5, a:415, i:8, d1:420, bs:"飛将"},
	"1155":{n:"張苞", r:"UR", c:4, a:515, i:14, d1:440, bs:"鹵獲の極攻"},
	"1156":{n:"姜維", r:"SR", c:3.5, a:420, i:20, d1:430, bs:"槍兵の猛攻"},
	"1157":{n:"夏侯氏", r:"R", c:3, a:360, i:11, d1:370, bs:"燕人武陣"},
	"1158":{n:"甘夫人", r:"UR", c:2, a:265, i:14, d1:215, bs:"劉備の大徳"},
	"1159":{n:"徐庶", r:"UR", c:4, a:430, i:26, d1:495, bs:"全軍の極攻令"},
	"1160":{n:"関銀屏", r:"SR", c:4, a:450, i:12, d1:490, bs:"全軍の猛攻令"},
	"1161":{n:"張姫", r:"UR", c:4, a:510, i:10, d1:485, bs:"燕神破軍"},
	"1162":{n:"王桃", r:"SR", c:3.5, a:360, i:5, d1:450, bs:"桃賊の襲撃"},
	"1163":{n:"張苞", r:"SR", c:4, a:465, i:12, d1:425, bs:"猛将の極意"},
	"1164":{n:"王悦", r:"UR", c:2.5, a:290, i:9, d1:230, bs:"槍兵速攻"},
	"1165":{n:"孟達", r:"UR", c:4, a:480, i:8, d1:470, bs:"謀反の極攻"},
	"1166":{n:"張苞・関興", r:"UR", c:4, a:570, i:15, d1:450, bs:"槍兵の大神撃"},
	"1167":{n:"張翼", r:"SR", c:4, a:485, i:10, d1:470, bs:"槍兵の大極撃"},
	"1168":{n:"馬良", r:"SR", c:2, a:200, i:18, d1:300, bs:"領地統制"},
	"1169":{n:"鮑三娘", r:"R", c:3.5, a:380, i:5, d1:395, bs:"騎神降臨"},
	"1170":{n:"敬哀皇后", r:"R", c:2, a:30, i:10, d1:70, bs:"市場来福"},
	"1171":{n:"劉封", r:"UC", c:1, a:90, i:7, d1:170, bs:"守護防陣"},
	"1172":{n:"王桃", r:"UR", c:2, a:240, i:4, d1:210, bs:"蜀軍の極撃"},
	"1173":{n:"関銀屏", r:"SR", c:3.5, a:405, i:13, d1:480, bs:"戦女の行軍"},
	"1174":{n:"黄月英", r:"SR", c:1.5, a:100, i:17, d1:210, bs:"衝車急配"},
	"1175":{n:"関平", r:"UR", c:4, a:490, i:12, d1:500, bs:"飛速雷光"},
	"1176":{n:"馬岱", r:"SR", c:3.5, a:420, i:8, d1:355, bs:"飛速"},
	"1177":{n:"馬鉄", r:"UC", c:2, a:200, i:6, d1:195, bs:"烈速"},
	"1178":{n:"姜維", r:"L", c:4, a:640, i:25, d1:610, bs:"槍兵の大天撃"},
	"1179":{n:"馬謖", r:"UR", c:4, a:370, i:21, d1:810, bs:"槍兵の大神域"},
	"1180":{n:"甘夫人", r:"SR", c:3.5, a:410, i:16, d1:435, bs:"烈速雷光"},
	"1181":{n:"関羽", r:"L", c:4, a:700, i:16, d1:620, bs:"龍神の極意"},
	"1182":{n:"趙雲", r:"L", c:4, a:630, i:21, d1:585, bs:"騎兵の大極攻"},
	"1183":{n:"馬岱", r:"UR", c:4, a:380, i:10, d1:725, bs:"騎将の封聖域"},
	"1184":{n:"関銀屏", r:"UR", c:4, a:515, i:15, d1:425, bs:"天里迅雷"},
	"1185":{n:"王桃", r:"SR", c:4, a:455, i:6, d1:440, bs:"謀反の極攻"},
	"1186":{n:"王桃", r:"R", c:3, a:335, i:4, d1:320, bs:"猛将の鹵獲"},
	"1187":{n:"穆皇后", r:"UR", c:1, a:30, i:11, d1:170, bs:"護国誠心"},
	"1188":{n:"龐統", r:"L", c:4, a:530, i:26, d1:560, bs:"攻城戦の大神砕"},
	"1189":{n:"馬良", r:"UR", c:4, a:420, i:25, d1:595, bs:"兵器の大神撃"},
	"1190":{n:"厳顔", r:"L", c:4, a:490, i:15, d1:865, bs:"弓兵の大天域"},
	"1191":{n:"関平", r:"R", c:3, a:340, i:11, d1:335, bs:"神隠雷光"},
	"1192":{n:"張飛", r:"L", c:4, a:700, i:10, d1:590, bs:"無双"},
	"1193":{n:"法正", r:"UR", c:4, a:405, i:25, d1:495, bs:"機略謀略"},
	"1194":{n:"馬雲緑", r:"SR", c:4, a:460, i:10, d1:460, bs:"全軍の烈速令"},
	"1195":{n:"穆皇后", r:"SR", c:3.5, a:415, i:22, d1:380, bs:"猛暑の縮地収穫"},
	"1196":{n:"関興", r:"UR", c:4, a:520, i:13, d1:485, bs:"槍兵烈攻"},
	"1197":{n:"諸葛亮・劉禅", r:"L", c:4.5, a:650, i:23, d1:1155, bs:"天衣無縫"},
	"1198":{n:"糜夫人", r:"UR", c:1, a:20, i:10, d1:120, bs:"守衛招来"},
	"1199":{n:"李厳", r:"SR", c:4, a:515, i:20, d1:415, bs:"槍兵の大神撃"},
	"1200":{n:"呉懿", r:"R", c:4, a:440, i:15, d1:495, bs:"剣兵の極攻"},
	"1201":{n:"夏侯氏", r:"R", c:1, a:30, i:10, d1:120, bs:"槍兵増勢"},
	"1202":{n:"張翼", r:"R", c:3.5, a:430, i:9, d1:445, bs:"神飛将"},
	"1203":{n:"劉備", r:"L", c:4, a:550, i:23, d1:610, bs:"全軍の大極攻令"},
	"1204":{n:"鮑三娘", r:"L", c:4, a:570, i:9, d1:545, bs:"聖夜の天恵沢"},
	"1205":{n:"甘夫人", r:"L", c:4.5, a:475, i:23, d1:630, bs:"強兵の義勇軍"},
	"1206":{n:"夏侯氏", r:"UR", c:4, a:530, i:15, d1:475, bs:"夏神の極意"},
	"1207":{n:"関索", r:"UR", c:3, a:340, i:10, d1:375, bs:"報恩の神恵沢"},
	"1208":{n:"廖化", r:"UR", c:4, a:485, i:8, d1:670, bs:"援軍の大神域"},
	"1209":{n:"黄月英", r:"L", c:4, a:540, i:25, d1:585, bs:"兵器の大天撃"},
	"1210":{n:"伊籍", r:"SR", c:4, a:340, i:21, d1:545, bs:"才略徴発"},
	"1211":{n:"諸葛鈴", r:"SR", c:1.5, a:140, i:16, d1:170, bs:"花紅の舞"},
	"1212":{n:"糜竺", r:"SR", c:1, a:70, i:9, d1:130, bs:"食糧極技巧"},
	"1213":{n:"黄忠", r:"L", c:4, a:675, i:15, d1:610, bs:"弓兵大烈攻"},
	"1214":{n:"黄権", r:"UR", c:4, a:360, i:20, d1:675, bs:"鬼神の守勢"},
	"1215":{n:"張姫", r:"L", c:4.5, a:720, i:11, d1:640, bs:"鬼神突貫"},
	"1216":{n:"敬哀皇后", r:"L", c:4, a:460, i:16, d1:625, bs:"花嫁の天恵沢"},
	"1217":{n:"張姫", r:"UR", c:3, a:390, i:8, d1:350, bs:"花嫁の神恵沢"},
	"1218":{n:"厳顔", r:"UR", c:3.5, a:360, i:12, d1:560, bs:"守護聖陣"},
	"1219":{n:"沙摩柯", r:"R", c:1, a:165, i:2, d1:105, bs:"弓兵の猛撃"},
	"1220":{n:"魏延", r:"L", c:4, a:615, i:14, d1:580, bs:"謀反の大極攻"},
	"1221":{n:"王悦",  r:"UR", c:4, a:450, i:9, d1:645, bs:"覇者の護り"},
	"1222":{n:"法正",  r:"L", c:4, a:515, i:26, d1:570, bs:"緊急呼集"},
	"1223":{n:"楊儀",  r:"UR", c:4, a:375, i:23, d1:580, bs:"非常呼集"},
	"1224":{n:"劉備・関羽",  r:"L", c:4.5, a:675, i:21, d1:1000, bs:"守護大聖陣"},
	"1225":{n:"沙摩柯",  r:"L", c:4, a:530, i:6, d1:860, bs:"龍神の守勢"},
	"1226":{n:"呉蘭",  r:"SR", c:4, a:500, i:9, d1:450, bs:"隣地猛襲"},
	"1227":{n:"甘夫人",  r:"R", c:1, a:10, i:7, d1:80, bs:"薫風の恵み"},
	"1228":{n:"廖化", r:"L", c:4, a:650, i:9, d1:625, bs:"槍兵の大天撃"},
	"1229":{n:"劉備", r:"L", c:4.5, a:670, i:23, d1:985, bs:"天衣無縫"},
	"1230":{n:"張苞", r:"UR", c:4, a:580, i:14, d1:485, bs:"槍兵の大神撃"},
	"1231":{n:"馬謖", r:"UR", c:4, a:340, i:19, d1:880, bs:"守護聖陣"},
	"1232":{n:"張嶷", r:"SR", c:4, a:525, i:17, d1:515, bs:"剣兵烈攻"},
	"1233":{n:"孟達", r:"R", c:4, a:450, i:6, d1:465, bs:"猛将の貫攻"},
	"1234":{n:"馬忠", r:"UR", c:4, a:375, i:14, d1:745, bs:"護君"},
	"1235":{n:"王悦", r:"L", c:4.5, a:770, i:11, d1:710, bs:"無双"},
	"1236":{n:"黄権", r:"SR", c:4, a:470, i:19, d1:560, bs:"収奪の極攻"},
	"1237":{n:"関索", r:"SR", c:2.5, a:270, i:8, d1:335, bs:"報恩の聖恵沢"},
	"1238":{n:"王平", r:"UR", c:2, a:200, i:11, d1:405, bs:"守護聖陣"},
	"1239":{n:"関銀屏", r:"L", c:4.5, a:760, i:16, d1:655, bs:"龍神の貫攻"},
	"1240":{n:"馬雲緑", r:"UR", c:2, a:270, i:6, d1:235, bs:"飛隠雷光"},
	"1241":{n:"周倉", r:"UR", c:4, a:505, i:7, d1:500, bs:"飛速"},
	"1242":{n:"黄月英", r:"R", c:1, a:15, i:10, d1:80, bs:"白秋の祝い"},
	"1243":{n:"諸葛亮", r:"L", c:4, a:530, i:29, d1:800, bs:"智将天砕器"},
	"1244":{n:"劉封", r:"UR", c:3, a:325, i:11, d1:570, bs:"鬼神の守勢"},
	"1245":{n:"黄権", r:"R", c:4, a:430, i:18, d1:420, bs:"全軍の猛攻令"},
	"1246":{n:"諸葛亮・徐庶", r:"L", c:4, a:700, i:28, d1:1010, bs:"覇王の帝賢斬"},
	"1247":{n:"劉封", r:"L", c:2, a:290, i:10, d1:420, bs:"剴切の大神域"},
	"1248":{n:"徐庶", r:"L", c:4, a:495, i:29, d1:850, bs:"智将天砕器"},
	"1249":{n:"黄忠", r:"R", c:2, a:270, i:11, d1:230, bs:"老将統帥"},
	"1250":{n:"厳顔", r:"R", c:4, a:245, i:10, d1:440, bs:"勇猛果敢"},
	"1251":{n:"黄忠・厳顔", r:"R", c:4, a:390, i:13, d1:460, bs:"勇将飛矢"},
	"1252":{n:"馬超", r:"L", c:4.5, a:740, i:10, d1:640, bs:"龍神突貫"},
	"1253":{n:"楊儀", r:"SR", c:4, a:360, i:22, d1:560, bs:"全軍の極攻令"},
	"1254":{n:"王桃", r:"SR", c:4, a:510, i:5, d1:450, bs:"仮装の劫略令"},
	"1255":{n:"簡雍", r:"UR", c:4, a:390, i:18, d1:555, bs:"食糧革命"},
	"1256":{n:"劉備・関羽・張飛", r:"L", c:1, a:230, i:19, d1:190, bs:"剴切の大極攻令"},
	"1257":{n:"呉懿", r:"UC", c:3, a:345, i:11, d1:315, bs:"闘将の貫攻"},
	"1258":{n:"関羽", r:"L", c:4, a:690, i:16, d1:625, bs:"鳳軍雷光"},
	"1261":{n:"敬哀皇后", r:"L", c:4, a:450, i:27, d1:790, bs:"副将の天明鏡"},
	"1262":{n:"張飛・夏侯氏", r:"UR", c:4, a:590, i:13, d1:500, bs:"鬼神の貫攻"},
	"1263":{n:"糜夫人", r:"SR", c:1, a:20, i:8, d1:60, bs:"全兵招致"},
	"1264":{n:"諸葛亮", r:"R", c:4, a:340, i:25, d1:570, bs:"醒龍出師"},
	"1265":{n:"諸葛鈴", r:"R", c:1, a:10, i:12, d1:60, bs:"玉雪の祝い"},
	"1266":{n:"王悦", r:"R", c:3, a:375, i:6, d1:350, bs:"春眠の攻奪"},
	"1267":{n:"趙雲・馬雲緑", r:"L", c:4, a:640, i:21, d1:530, bs:"龍神の攻奪"},
	"1268":{n:"諸葛鈴", r:"UR", c:1, a:80, i:14, d1:120, bs:"天里招来"},
	"1269":{n:"夏侯氏", r:"UR", c:4, a:510, i:15, d1:460, bs:"鬼神の攻奪"},
	"1270":{n:"張姫", r:"L", c:4, a:640, i:11, d1:565, bs:"副将の天明鏡"},
	"1271":{n:"関索", r:"L", c:4.5, a:765, i:14, d1:645, bs:"龍神突貫"},
	"1275":{n:"龐統", r:"L", c:4, a:670, i:26, d1:620, bs:"龍神の劫略令"},
	"1276":{n:"馬謖", r:"L", c:4.5, a:690, i:24, d1:790, bs:"兵器の大天撃"},
	"1277":{n:"孟達", r:"L", c:2, a:390, i:9, d1:270, bs:"剴切の天攻奪"},
	"1278":{n:"関銀屏", r:"L", c:4.5, a:700, i:16, d1:650, bs:"副将の天明鏡"},
	"1279":{n:"黄月英", r:"UR", c:4, a:440, i:26, d1:550, bs:"副将の神明鏡"},
	"1280":{n:"甘夫人", r:"UR", c:4, a:430, i:20, d1:500, bs:"全兵の飛速令"},
	"1281":{n:"王悦", r:"SR", c:4, a:505, i:8, d1:435, bs:"戦神の貫攻"},
	"1282":{n:"劉備", r:"L", c:4.5, a:535, i:23, d1:885, bs:"双兵の大天域"},
	"1283":{n:"張姫", r:"UR", c:4, a:560, i:10, d1:485, bs:"斧兵烈攻"},
	"1284":{n:"厳顔", r:"SR", c:4, a:580, i:12, d1:500, bs:"錘兵の極攻"},
	"1285":{n:"張苞", r:"R", c:3.5, a:450, i:10, d1:405, bs:"斧神降臨"},
	"1286":{n:"沙摩柯", r:"R", c:3.5, a:455, i:3, d1:465, bs:"錘兵の猛攻"},
	"1287":{n:"夏侯氏", r:"UC", c:3, a:120, i:9, d1:290, bs:"双兵舎訓練"},
	"1288":{n:"蒋琬", r:"L", c:4.5, a:500, i:27, d1:700, bs:"攻城の大檄文"},
	"1289":{n:"関平", r:"SR", c:2, a:290, i:7, d1:290, bs:"天里雷光"},
	"1290":{n:"鮑三娘", r:"UR", c:2, a:290, i:6, d1:250, bs:"聖夜の神攻奪"},
	"1291":{n:"王平", r:"L", c:4.5, a:560, i:16, d1:960, bs:"守備大聖陣"},
	"1292":{n:"関銀屏・張姫", r:"L", c:4.5, a:805, i:16, d1:665, bs:"隣地天豪撃"},
	"1293":{n:"廖化", r:"R", c:4, a:430, i:6, d1:440, bs:"闘将の叛攻令"},
	"1294":{n:"甘夫人", r:"L", c:4.5, a:600, i:23, d1:640, bs:"副将の天明鏡"},
	"1295":{n:"夏侯氏", r:"SR", c:4, a:495, i:14, d1:430, bs:"燕神破軍"},
	"1296":{n:"張姫", r:"R", c:3.5, a:405, i:11, d1:420, bs:"春眠の攻奪"},
	"1297":{n:"姜維", r:"L", c:4.5, a:810, i:25, d1:715, bs:"隣地天豪襲"},
	"1298":{n:"黄権", r:"R", c:4, a:440, i:18, d1:510, bs:"収奪の猛攻"},
	"1299":{n:"劉備・孫尚香", r:"L", c:4.5, a:665, i:22, d1:1200, bs:"覇者の大神域"},
	"1300":{n:"関索・鮑三娘", r:"L", c:4.5, a:770, i:16, d1:660, bs:"強襲天奮迅"},
	"1301":{n:"糜夫人", r:"L", c:2, a:260, i:15, d1:350, bs:"玄武の啓示"},
	"1302":{n:"姜維", r:"UR", c:4, a:585, i:23, d1:490, bs:"無双"},
	"1303":{n:"糜芳", r:"SR", c:4, a:405, i:15, d1:660, bs:"剣将の封聖域"},
	"1304":{n:"黄権", r:"R", c:4, a:445, i:17, d1:470, bs:"騎兵烈攻"},
	"1305":{n:"劉禅", r:"R", c:2.5, a:265, i:15, d1:285, bs:"劉備の大徳"},
	"1306":{n:"劉封", r:"L", c:4, a:670, i:14, d1:825, bs:"覇王の帝俊賢"},
	"1307":{n:"劉備・簡雍", r:"L", c:4.5, a:795, i:21, d1:640, bs:"龍神の劫略令"},
	"1309":{n:"関銀屏", r:"R", c:4, a:430, i:11, d1:375, bs:"軍神"},
	"1310":{n:"劉備・簡雍", r:"SR", c:4, a:500, i:18, d1:430, bs:"全軍の極攻令"},
	"1311":{n:"関平", r:"L", c:4.5, a:665, i:14, d1:875, bs:"獣神の守勢"},
	"1312":{n:"張姫", r:"SR", c:4, a:380, i:8, d1:580, bs:"鬼神の守勢"},
	"1313":{n:"夏侯氏", r:"R", c:4, a:410, i:12, d1:410, bs:"烈速雷光"},
	"1314":{n:"敬哀皇后", r:"UR", c:2, a:210, i:15, d1:280, bs:"槍神の導き"},
	"1315":{n:"王桃", r:"UC", c:4, a:330, i:3, d1:480, bs:"闘将の守勢"},
	"1316":{n:"鮑三娘", r:"L", c:1, a:165, i:9, d1:160, bs:"副将の天明鏡"},
	"1317":{n:"王桃", r:"L", c:1, a:120, i:8, d1:155, bs:"副将の天明鏡"},
	"1318":{n:"王悦", r:"L", c:1, a:215, i:10, d1:155, bs:"副将の天明鏡"},
	"1319":{n:"夏侯氏", r:"SR", c:2, a:245, i:13, d1:380, bs:"守衛聖陣"},
	"1320":{n:"穆皇后", r:"R", c:2, a:160, i:20, d1:250, bs:"兵器の大極撃"},
	"1321":{n:"劉備・張飛", r:"L", c:4.5, a:720, i:20, d1:620, bs:"全軍の鳳速令"},
	"1322":{n:"李厳", r:"R", c:3.5, a:405, i:17, d1:360, bs:"全軍の烈速令"},
	"1323":{n:"張翼", r:"L", c:2, a:390, i:12, d1:255, bs:"剴切の天攻奪"},
	"1324":{n:"馬良", r:"L", c:4.5, a:510, i:28, d1:680, bs:"双兵の義勇軍"},
	"1325":{n:"楊儀", r:"L", c:4.5, a:505, i:27, d1:815, bs:"強兵の神義勇軍"},
	"1326":{n:"糜夫人・阿斗", r:"L", c:4.5, a:760, i:16, d1:605, bs:"お菓子の療略令"},
	"1327":{n:"呉蘭", r:"UR", c:4, a:540, i:13, d1:475, bs:"鬼神の叛攻令"},
	"1328":{n:"馬超", r:"L", c:2, a:385, i:10, d1:250, bs:"双軍の神遠攻"},
	"1329":{n:"魏延", r:"L", c:2, a:405, i:14, d1:285, bs:"錘軍の神遠攻"},
	"1330":{n:"張翼", r:"R", c:4, a:455, i:9, d1:425, bs:"斧兵の極攻"},
	"1331":{n:"劉禅・蒋琬", r:"L", c:1, a:180, i:16, d1:225, bs:"剴切大聖陣"},
	"1332":{n:"穆皇后", r:"UR", c:2, a:275, i:10, d1:230, bs:"副将の神明鏡"},
	"1333":{n:"沙摩柯", r:"SL", c:4.5, a:940, i:7, d1:850, bs:"錘兵の大帝撃"},
	"1334":{n:"張姫", r:"R", c:4, a:385, i:8, d1:465, bs:"斧兵舎修練"},
	"1335":{n:"姜維", r:"UR", c:4, a:590, i:23, d1:520, bs:"鬼神の療略令"},
	"1336":{n:"張苞・関興", r:"SL", c:4.5, a:740, i:18, d1:1100, bs:"加護帝聖壁"},
	"1337":{n:"呉懿", r:"UR", c:4, a:460, i:15, d1:700, bs:"加護聖壁"},
	"1338":{n:"劉禅", r:"R", c:1.5, a:50, i:7, d1:150, bs:"仁君"},
	"2001":{n:"曹操", r:"SR", c:3.5, a:415, i:22, d1:520, bs:"魏王の号令"},
	"2002":{n:"司馬懿", r:"SR", c:3.5, a:385, i:24, d1:535, bs:"深慮遠謀"},
	"2003":{n:"荀彧", r:"R", c:2, a:25, i:15, d1:135, bs:"王佐の才"},
	"2004":{n:"夏侯惇", r:"R", c:2.5, a:275, i:6, d1:310, bs:"騎兵の進撃"},
	"2005":{n:"張遼", r:"SR", c:3, a:320, i:11, d1:390, bs:"覇王の進撃"},
	"2006":{n:"張郃", r:"R", c:2.5, a:270, i:8, d1:315, bs:"騎兵突撃"},
	"2007":{n:"曹操", r:"R", c:3, a:310, i:16, d1:370, bs:"英雄"},
	"2008":{n:"曹操", r:"UC", c:3, a:300, i:15, d1:370, bs:"英雄"},
	"2009":{n:"夏侯惇", r:"UC", c:2.5, a:260, i:6, d1:305, bs:"騎兵の進撃"},
	"2010":{n:"夏侯淵", r:"UC", c:2.5, a:275, i:5, d1:270, bs:"弓兵の進撃"},
	"2011":{n:"夏侯淵", r:"C", c:2.5, a:265, i:5, d1:270, bs:"弓兵の進撃"},
	"2012":{n:"許褚", r:"UC", c:3, a:230, i:3, d1:390, bs:"槍兵防御"},
	"2013":{n:"典韋", r:"R", c:3, a:225, i:4, d1:395, bs:"鉄壁"},
	"2014":{n:"曹仁", r:"UC", c:2.5, a:245, i:6, d1:320, bs:"騎兵の進撃"},
	"2015":{n:"曹仁", r:"C", c:2.5, a:245, i:6, d1:305, bs:"騎兵の進撃"},
	"2016":{n:"徐晃", r:"UC", c:2.5, a:275, i:9, d1:320, bs:"槍兵突撃"},
	"2017":{n:"于禁", r:"UC", c:2, a:185, i:9, d1:245, bs:"騎兵防御"},
	"2018":{n:"于禁", r:"C", c:2, a:185, i:8, d1:245, bs:"騎兵の進撃"},
	"2019":{n:"張郃", r:"UC", c:2.5, a:255, i:8, d1:310, bs:"騎兵突撃"},
	"2020":{n:"蔡瑁", r:"UC", c:2, a:180, i:9, d1:235, bs:"弓兵の進撃"},
	"2021":{n:"蔡瑁", r:"C", c:2, a:175, i:9, d1:225, bs:"弓兵防御"},
	"2022":{n:"文聘", r:"UC", c:2.5, a:225, i:8, d1:305, bs:"槍兵防御"},
	"2023":{n:"文聘", r:"C", c:2.5, a:225, i:7, d1:305, bs:"槍兵防御"},
	"2024":{n:"張魯", r:"UC", c:2, a:70, i:11, d1:170, bs:"食糧知識"},
	"2025":{n:"張魯", r:"C", c:2, a:70, i:11, d1:150, bs:"食糧知識"},
	"2026":{n:"曹真", r:"UC", c:2, a:225, i:9, d1:240, bs:"騎兵の進撃"},
	"2027":{n:"張允", r:"UC", c:1.5, a:130, i:4, d1:165, bs:"弓兵行軍"},
	"2028":{n:"張允", r:"C", c:1.5, a:135, i:4, d1:130, bs:"弓兵行軍"},
	"2029":{n:"華歆", r:"UC", c:2, a:85, i:13, d1:35, bs:"製鉄知識"},
	"2030":{n:"華歆", r:"C", c:2, a:85, i:12, d1:35, bs:"練兵訓練"},
	"2031":{n:"朱霊", r:"UC", c:2, a:205, i:7, d1:200, bs:"厩舎訓練"},
	"2032":{n:"朱霊", r:"C", c:2, a:190, i:7, d1:200, bs:"厩舎訓練"},
	"2033":{n:"曹昂", r:"UC", c:1.5, a:125, i:6, d1:180, bs:"騎兵防御"},
	"2034":{n:"曹昂", r:"C", c:1.5, a:125, i:6, d1:165, bs:"騎兵防御"},
	"2035":{n:"楽進", r:"UC", c:2.5, a:280, i:4, d1:265, bs:"騎兵突撃"},
	"2036":{n:"楽進", r:"C", c:2.5, a:265, i:4, d1:265, bs:"豪傑"},
	"2037":{n:"曹休", r:"UC", c:2, a:215, i:7, d1:225, bs:"騎兵防御"},
	"2038":{n:"曹休", r:"C", c:2, a:200, i:7, d1:225, bs:"騎兵防御"},
	"2039":{n:"曹操", r:"R", c:2.5, a:270, i:15, d1:300, bs:"覇道"},
	"2040":{n:"賈詡", r:"R", c:2, a:235, i:15, d1:200, bs:"兵器の強撃"},
	"2041":{n:"賈詡", r:"UC", c:2, a:215, i:15, d1:200, bs:"兵器強行"},
	"2042":{n:"郭嘉", r:"SR", c:2, a:250, i:17, d1:260, bs:"神算鬼謀"},
	"2043":{n:"夏侯惇", r:"R", c:3, a:330, i:9, d1:345, bs:"騎兵の猛撃"},
	"2044":{n:"夏侯淵", r:"R", c:3, a:315, i:11, d1:370, bs:"弓兵突撃"},
	"2045":{n:"曹洪", r:"UC", c:2, a:215, i:5, d1:240, bs:"騎兵の強撃"},
	"2046":{n:"曹洪", r:"C", c:2, a:210, i:5, d1:235, bs:"剣兵の強撃"},
	"2047":{n:"司馬懿", r:"R", c:2.5, a:275, i:18, d1:345, bs:"神速"},
	"2048":{n:"甄姫", r:"R", c:1, a:35, i:8, d1:155, bs:"強兵の檄文"},
	"2049":{n:"典韋", r:"R", c:3, a:355, i:4, d1:275, bs:"一騎当千"},
	"2050":{n:"典韋", r:"UC", c:3, a:325, i:4, d1:275, bs:"豪傑"},
	"2051":{n:"曹操", r:"UR", c:3.5, a:420, i:22, d1:520, bs:"魏武王"},
	"2052":{n:"張遼", r:"UR", c:3.5, a:490, i:13, d1:625, bs:"覇王の進撃"},
	"2053":{n:"程昱", r:"R", c:2, a:150, i:17, d1:260, bs:"剣兵急配"},
	"2054":{n:"董昭", r:"UR", c:4, a:450, i:26, d1:565, bs:"全軍の極攻令"},
	"2055":{n:"曹休", r:"SR", c:2.5, a:295, i:10, d1:295, bs:"弓兵突攻"},
	"2056":{n:"曹真", r:"C", c:2, a:220, i:9, d1:240, bs:"騎兵の進撃"},
	"2057":{n:"曹仁", r:"R", c:2.5, a:260, i:6, d1:385, bs:"騎兵堅守"},
	"2058":{n:"夏侯惇", r:"UR", c:3.5, a:430, i:13, d1:490, bs:"騎兵の極撃"},
	"2059":{n:"張郃", r:"SR", c:2.5, a:285, i:8, d1:330, bs:"騎兵の猛撃"},
	"2060":{n:"許褚", r:"UR", c:3.5, a:370, i:8, d1:590, bs:"王者の護り"},
	"2061":{n:"徐晃", r:"R", c:3, a:315, i:10, d1:345, bs:"槍兵突覇"},
	"2062":{n:"賈詡", r:"SR", c:2, a:235, i:17, d1:270, bs:"市場知識"},
	"2063":{n:"夏侯淵", r:"UR", c:3.5, a:430, i:11, d1:510, bs:"軍神"},
	"2064":{n:"典韋", r:"SR", c:3, a:305, i:7, d1:455, bs:"迅速援護"},
	"2065":{n:"司馬懿", r:"UR", c:3.5, a:385, i:25, d1:535, bs:"騎将の采配"},
	"2066":{n:"曹操", r:"UR", c:3.5, a:420, i:22, d1:520, bs:"魏軍の極撃"},
	"2067":{n:"張遼", r:"R", c:3, a:320, i:8, d1:385, bs:"強襲突撃"},
	"2068":{n:"郭嘉", r:"R", c:2, a:230, i:15, d1:210, bs:"剣兵突覇"},
	"2069":{n:"程昱", r:"SR", c:2, a:240, i:17, d1:260, bs:"兵器の極撃"},
	"2070":{n:"荀彧", r:"UC", c:2, a:25, i:15, d1:130, bs:"石切知識"},
	"2071":{n:"許褚", r:"UR", c:3.5, a:340, i:5, d1:640, bs:"守将の進軍"},
	"2072":{n:"甄姫", r:"SR", c:1, a:35, i:9, d1:155, bs:"農林技術"},
	"2073":{n:"曹丕", r:"UR", c:3.5, a:420, i:22, d1:540, bs:"魏軍の防衛"},
	"2074":{n:"于禁", r:"R", c:2, a:200, i:9, d1:265, bs:"騎兵の強撃"},
	"2075":{n:"郭嘉", r:"SR", c:4, a:400, i:23, d1:485, bs:"全軍の猛攻令"},
	"2076":{n:"郭嘉", r:"UC", c:2, a:225, i:15, d1:180, bs:"加工知識"},
	"2077":{n:"張遼", r:"UC", c:2.5, a:280, i:6, d1:265, bs:"騎兵の強撃"},
	"2078":{n:"李典", r:"R", c:2, a:230, i:12, d1:295, bs:"槍兵の聖域"},
	"2079":{n:"鍾会", r:"SR", c:3, a:320, i:15, d1:380, bs:"槍兵の極撃"},
	"2080":{n:"曹洪", r:"R", c:2, a:245, i:5, d1:260, bs:"騎兵突覇"},
	"2081":{n:"曹休", r:"R", c:2, a:240, i:10, d1:320, bs:"騎兵の聖域"},
	"2082":{n:"卞夫人", r:"SR", c:1, a:10, i:9, d1:95, bs:"美玉歌舞"},
	"2083":{n:"甄姫", r:"UR", c:1, a:35, i:10, d1:155, bs:"皇后の慈愛"},
	"2084":{n:"荀彧", r:"SR", c:2.5, a:220, i:18, d1:200, bs:"神算鬼謀"},
	"2085":{n:"鄧艾", r:"SR", c:3.5, a:390, i:22, d1:520, bs:"地の利堅壁"},
	"2086":{n:"楽進", r:"SR", c:3.5, a:430, i:10, d1:435, bs:"猛将の鹵獲"},
	"2087":{n:"張春華", r:"SR", c:1, a:20, i:9, d1:155, bs:"出撃見舞"},
	"2088":{n:"曹操", r:"UR", c:3.5, a:480, i:23, d1:520, bs:"魏王の覇軍"},
	"2089":{n:"許褚", r:"SR", c:3.5, a:360, i:5, d1:600, bs:"忠節不落"},
	"2090":{n:"卞夫人", r:"R", c:1, a:10, i:9, d1:85, bs:"豊穣"},
	"2091":{n:"夏侯惇", r:"UC", c:3, a:330, i:8, d1:345, bs:"騎兵の強撃"},
	"2092":{n:"典韋", r:"UR", c:4, a:500, i:8, d1:470, bs:"槍兵の大極撃"},
	"2093":{n:"夏侯淵", r:"SR", c:3.5, a:410, i:11, d1:440, bs:"弓兵の極撃"},
	"2094":{n:"賈詡", r:"UR", c:3, a:310, i:22, d1:220, bs:"奇略布陣"},
	"2095":{n:"曹彰", r:"SR", c:3.5, a:430, i:9, d1:420, bs:"天弓雨撃"},
	"2096":{n:"曹操", r:"R", c:3, a:310, i:16, d1:370, bs:"奇計百出"},
	"2097":{n:"夏侯惇", r:"UC", c:2.5, a:260, i:6, d1:305, bs:"騎兵の強撃"},
	"2098":{n:"許褚", r:"UC", c:3, a:230, i:3, d1:390, bs:"槍兵方陣"},
	"2099":{n:"華佗", r:"SR", c:2, a:90, i:18, d1:190, bs:"神医の施術"},
	"2100":{n:"司馬懿", r:"UR", c:3.5, a:320, i:27, d1:540, bs:"混元一気"},
	"2101":{n:"荀攸", r:"SR", c:2.5, a:210, i:18, d1:200, bs:"騎兵の勝鬨"},
	"2102":{n:"夏侯惇", r:"UR", c:3.5, a:435, i:14, d1:480, bs:"魏将の督戦"},
	"2103":{n:"曹植", r:"SR", c:2, a:25, i:17, d1:80, bs:"強兵の檄文"},
	"2104":{n:"張遼", r:"UR", c:3.5, a:495, i:13, d1:625, bs:"戦蹄轟撃"},
	"2105":{n:"曹操", r:"R", c:3, a:300, i:16, d1:370, bs:"騎兵突撃"},
	"2106":{n:"曹仁", r:"SR", c:3.5, a:400, i:8, d1:360, bs:"隣地猛攻"},
	"2107":{n:"司馬昭", r:"SR", c:2.5, a:280, i:18, d1:260, bs:"近衛騎兵移送"},
	"2108":{n:"張郃", r:"UR", c:3.5, a:440, i:16, d1:430, bs:"騎兵の大極撃"},
	"2109":{n:"徐晃", r:"SR", c:3.5, a:420, i:15, d1:440, bs:"騎兵の猛攻"},
	"2110":{n:"龐徳", r:"SR", c:3.5, a:435, i:13, d1:450, bs:"猛将突貫"},
	"2111":{n:"楽進", r:"R", c:3, a:325, i:9, d1:370, bs:"槍兵の猛撃"},
	"2112":{n:"辛憲英", r:"SR", c:1, a:10, i:10, d1:110, bs:"才媛献策"},
	"2113":{n:"夏侯淵", r:"R", c:3, a:295, i:9, d1:330, bs:"急襲"},
	"2114":{n:"曹操", r:"UR", c:3.5, a:430, i:22, d1:515, bs:"騎兵の極攻"},
	"2115":{n:"夏侯惇", r:"SR", c:3.5, a:425, i:11, d1:430, bs:"隻眼将の軍略"},
	"2116":{n:"許褚", r:"R", c:3, a:250, i:4, d1:435, bs:"守将の出陣"},
	"2117":{n:"曹彰", r:"R", c:3, a:340, i:7, d1:345, bs:"弓兵の強攻"},
	"2118":{n:"司馬懿", r:"UC", c:2.5, a:200, i:16, d1:240, bs:"覇道"},
	"2119":{n:"典韋・許褚",	r:"UR", c:4, a:500, i:9, d1:640, bs:"守神の進撃"},
	"2120":{n:"賈詡", r:"SR", c:1.5, a:150, i:17, d1:220, bs:"騎兵の大号令"},
	"2121":{n:"張春華", r:"R", c:1, a:15, i:9, d1:135, bs:"才女の瞳"},
	"2122":{n:"辛憲英", r:"R", c:1, a:10, i:10, d1:100, bs:"聡明叡知"},
	"2123":{n:"張遼", r:"UC", c:3, a:350, i:8, d1:335, bs:"騎兵の進攻"},
	"2124":{n:"郭嘉", r:"UR", c:3, a:300, i:22, d1:230, bs:"才略謀略"},
	"2125":{n:"鄧艾", r:"R", c:3, a:290, i:17, d1:395, bs:"擒賊擒王"},
	"2126":{n:"郭夫人", r:"UR", c:1.5, a:30, i:10, d1:180, bs:"帝妃都政"},
	"2127":{n:"鍾会", r:"R", c:2.5, a:200, i:17, d1:320, bs:"堅牢知略"},
	"2128":{n:"司馬炎", r:"SR", c:2, a:240, i:17, d1:300, bs:"混元一気"},
	"2129":{n:"関羽", r:"UR", c:4, a:515, i:15, d1:540, bs:"鬼神"},
	"2130":{n:"曹仁", r:"UR", c:3.5, a:435, i:8, d1:370, bs:"遠地猛攻"},
	"2131":{n:"王元姫", r:"SR", c:1.5, a:85, i:15, d1:110, bs:"才媛眼識"},
	"2132":{n:"郝昭", r:"SR", c:3, a:320, i:11, d1:460, bs:"堅忍不抜"},
	"2133":{n:"夏侯覇", r:"R", c:3.5, a:410, i:11, d1:460, bs:"飛将"},
	"2134":{n:"龐徳", r:"R", c:3.5, a:435, i:13, d1:450, bs:"闘将の極意"},
	"2135":{n:"夏侯淵", r:"UR", c:3.5, a:435, i:11, d1:460, bs:"刹那の奇襲"},
	"2136":{n:"荀攸", r:"R", c:2, a:150, i:17, d1:260, bs:"騎兵急配"},
	"2137":{n:"于禁", r:"UR", c:4, a:545, i:11, d1:490, bs:"戦神の極意"},
	"2138":{n:"文聘", r:"R", c:3.5, a:380, i:9, d1:700, bs:"金剛不壊"},
	"2139":{n:"王異", r:"UR", c:3.5, a:410, i:10, d1:620, bs:"守護聖陣"},
	"2140":{n:"司馬師・昭", r:"UR", c:4, a:560, i:24, d1:400, bs:"騎兵の大神撃"},
	"2141":{n:"張允", r:"R", c:2, a:200, i:8, d1:285, bs:"城壁補強"},
	"2142":{n:"楽進", r:"SR", c:4, a:480, i:10, d1:460, bs:"騎兵の大極撃"},
	"2143":{n:"丁夫人", r:"SR", c:1, a:10, i:10, d1:100, bs:"悲嘆の舞"},
	"2144":{n:"趙娥", r:"R", c:3.5, a:390, i:10, d1:400, bs:"剣神降臨"},
	"2145":{n:"郭夫人", r:"R", c:2, a:15, i:10, d1:80, bs:"麗妃都政"},
	"2146":{n:"鄧艾", r:"UC", c:1, a:160, i:9, d1:120, bs:"万夫不当"},
	"2147":{n:"司馬昭", r:"R", c:3, a:310, i:19, d1:160, bs:"麒麟児"},
	"2148":{n:"曹真", r:"R", c:3, a:345, i:11, d1:280, bs:"闘将突貫"},
	"2149":{n:"卞夫人", r:"R", c:1, a:20, i:9, d1:70, bs:"優雅な調べ"},
	"2150":{n:"司馬炎", r:"R", c:3, a:355, i:16, d1:390, bs:"覇王の強攻"},
	"2151":{n:"朱霊", r:"R", c:3.5, a:385, i:8, d1:400, bs:"神速雷光"},
	"2152":{n:"荀彧", r:"SR", c:3, a:280, i:21, d1:450, bs:"隣地攻城"},
	"2153":{n:"曹憲", r:"UR", c:2, a:240, i:11, d1:180, bs:"魏軍の極撃"},
	"2154":{n:"曹節", r:"SR", c:2, a:260, i:12, d1:245, bs:"神速劫略"},
	"2155":{n:"曹華", r:"R", c:3.5, a:390, i:9, d1:405, bs:"飛将"},
	"2156":{n:"夏侯惇", r:"SR", c:4, a:500, i:12, d1:465, bs:"神飛将"},
	"2157":{n:"徐晃", r:"UR", c:4, a:525, i:17, d1:520, bs:"隣地豪襲"},
	"2158":{n:"張魯", r:"UR", c:1.5, a:140, i:14, d1:215, bs:"師君強兵"},
	"2159":{n:"曹丕", r:"SR", c:4, a:370, i:20, d1:600, bs:"騎兵の大聖域"},
	"2160":{n:"趙娥", r:"SR", c:4, a:455, i:12, d1:510, bs:"鹵獲の猛攻"},
	"2161":{n:"甄姫", r:"R", c:1.5, a:35, i:8, d1:160, bs:"万華招来"},
	"2162":{n:"曹昂", r:"SR", c:2, a:280, i:8, d1:395, bs:"万里行"},
	"2163":{n:"曹休・曹真", r:"R", c:3.5, a:405, i:12, d1:430, bs:"千里雷光"},
	"2164":{n:"蔡瑁", r:"R", c:3, a:370, i:10, d1:355, bs:"弓兵の猛攻"},
	"2165":{n:"甄姫", r:"SR", c:1.5, a:35, i:9, d1:150, bs:"洛神の舞"},
	"2166":{n:"華佗", r:"R", c:1, a:80, i:11, d1:160, bs:"神医の術式"},
	"2167":{n:"司馬師", r:"UR", c:4, a:410, i:26, d1:555, bs:"兵器の極攻"},
	"2168":{n:"趙娥", r:"UR", c:3.5, a:410, i:12, d1:580, bs:"守衛聖陣"},
	"2169":{n:"許褚", r:"UC", c:1.5, a:195, i:3, d1:270, bs:"守護防陣"},
	"2170":{n:"典韋", r:"UC", c:1.5, a:230, i:4, d1:220, bs:"闘将突貫"},
	"2171":{n:"曹丕", r:"R", c:3.5, a:315, i:18, d1:405, bs:"全軍の強攻令"},
	"2172":{n:"司馬炎", r:"UC", c:2.5, a:265, i:15, d1:315, bs:"全軍の進攻令"},
	"2173":{n:"程昱・荀彧", r:"UR", c:4, a:430, i:26, d1:550, bs:"智将砕器"},
	"2174":{n:"夏侯恩", r:"R", c:3.5, a:415, i:11, d1:460, bs:"剣兵突攻"},
	"2175":{n:"鍾会", r:"UC", c:2.5, a:190, i:13, d1:295, bs:"兵器突覇"},
	"2176":{n:"閻圃", r:"UR", c:2, a:140, i:20, d1:315, bs:"食糧革命"},
	"2177":{n:"丁夫人", r:"R", c:1, a:10, i:9, d1:90, bs:"剣兵増強"},
	"2178":{n:"夏侯惇", r:"L", c:4, a:620, i:14, d1:600, bs:"騎兵の大天撃"},
	"2179":{n:"程昱", r:"UR", c:2, a:250, i:19, d1:435, bs:"拠点統制"},
	"2180":{n:"司馬昭", r:"UR", c:4, a:360, i:22, d1:765, bs:"騎兵の大神域"},
	"2181":{n:"曹節", r:"SR", c:3.5, a:335, i:13, d1:640, bs:"守衛方陣"},
	"2182":{n:"郭夫人", r:"SR", c:4, a:360, i:20, d1:685, bs:"護君"},
	"2183":{n:"許褚", r:"L", c:4.5, a:550, i:10, d1:810, bs:"覇者の大神域"},
	"2184":{n:"曹憲", r:"SR", c:4, a:440, i:13, d1:485, bs:"魏王の覇軍"},
	"2185":{n:"張郃", r:"SR", c:4, a:445, i:15, d1:415, bs:"鹵獲の猛攻"},
	"2186":{n:"夏侯淵", r:"L", c:4, a:680, i:13, d1:595, bs:"弓兵の大極攻"},
	"2187":{n:"曹休", r:"UR", c:4, a:400, i:13, d1:740, bs:"弓将の封聖域"},
	"2188":{n:"許褚", r:"UR", c:4, a:540, i:7, d1:410, bs:"隣地豪撃"},
	"2189":{n:"賈南風", r:"R", c:3, a:300, i:13, d1:580, bs:"守護方陣"},
	"2190":{n:"満寵", r:"SR", c:4, a:425, i:23, d1:555, bs:"全軍の烈速令"},
	"2191":{n:"曹仁", r:"L", c:4, a:450, i:10, d1:815, bs:"騎兵の大天域"},
	"2192":{n:"陳羣", r:"UR", c:4, a:435, i:24, d1:605, bs:"攻城の義勇軍"},
	"2193":{n:"荀彧", r:"R", c:3, a:200, i:17, d1:300, bs:"知略謀略"},
	"2194":{n:"賈詡", r:"R", c:3, a:320, i:17, d1:330, bs:"全軍の神速令"},
	"2195":{n:"徐晃", r:"R", c:3, a:350, i:10, d1:325, bs:"万里行"},
	"2196":{n:"曹華", r:"UR", c:4, a:540, i:12, d1:455, bs:"鬼神突貫"},
	"2197":{n:"楽進", r:"UR", c:4, a:510, i:11, d1:455, bs:"騎兵烈攻"},
	"2198":{n:"司馬懿", r:"L", c:4, a:540, i:29, d1:525, bs:"智将の大天撃"},
	"2199":{n:"曹洪", r:"UR", c:4, a:550, i:10, d1:445, bs:"鬼神突貫"},
	"2200":{n:"曹操", r:"L", c:4, a:540, i:25, d1:555, bs:"全軍の大極攻令"},
	"2201":{n:"于禁", r:"SR", c:3.5, a:415, i:10, d1:410, bs:"鬼神の鹵獲"},
	"2202":{n:"張遼・郭嘉", r:"L", c:4.5, a:800, i:22, d1:670, bs:"龍神"},
	"2203":{n:"張春華", r:"UR", c:1, a:30, i:9, d1:160, bs:"豪撃招来"},
	"2204":{n:"郭淮", r:"SR", c:4, a:510, i:21, d1:440, bs:"騎兵の大神撃"},
	"2205":{n:"王双", r:"R", c:4, a:450, i:7, d1:470, bs:"弓兵の極攻"},
	"2206":{n:"王元姫", r:"R", c:1, a:15, i:9, d1:90, bs:"騎兵増勢"},
	"2207":{n:"張魯", r:"R", c:1.5, a:80, i:12, d1:180, bs:"食糧革命"},
	"2208":{n:"董昭", r:"L", c:4, a:550, i:26, d1:680, bs:"攻城戦の大神砕"},
	"2209":{n:"李典", r:"UC", c:3, a:240, i:12, d1:380, bs:"忠節不落"},
	"2210":{n:"司馬炎", r:"UR", c:4, a:350, i:21, d1:910, bs:"守護聖陣"},
	"2211":{n:"曹真", r:"UR", c:4, a:495, i:15, d1:510, bs:"飛軍雷光"},
	"2212":{n:"華佗", r:"UR", c:1, a:90, i:12, d1:165, bs:"神医の術式"},
	"2213":{n:"張遼", r:"L", c:4.5, a:750, i:17, d1:650, bs:"無双"},
	"2214":{n:"曹仁", r:"SR", c:1, a:190, i:3, d1:120, bs:"万里雷光"},
	"2215":{n:"司馬師", r:"SR", c:1, a:175, i:12, d1:115, bs:"兵器突攻"},
	"2216":{n:"鍾繇", r:"UR", c:4, a:410, i:25, d1:570, bs:"機略徴発"},
	"2217":{n:"辛憲英", r:"UR", c:4, a:355, i:23, d1:505, bs:"市場栄華"},
	"2218":{n:"賈詡", r:"L", c:4, a:595, i:26, d1:640, bs:"剣兵大烈攻"},
	"2219":{n:"曹昂", r:"R", c:3.5, a:350, i:10, d1:565, bs:"闘将の守勢"},
	"2220":{n:"荀彧", r:"L", c:4, a:520, i:25, d1:605, bs:"全軍の鳳速令"},
	"2221":{n:"文欽", r:"SR", c:4, a:490, i:6, d1:575, bs:"猛将の叛乱"},
	"2222":{n:"郝昭", r:"UR", c:4, a:365, i:14, d1:660, bs:"鬼神の守勢"},
	"2223":{n:"夏侯恩", r:"UC", c:2.5, a:250, i:8, d1:385, bs:"武将の守勢"},
	"2224":{n:"曹植", r:"R", c:1.5, a:100, i:15, d1:260, bs:"王者の護り"},
	"2225":{n:"甄姫", r:"L", c:1, a:55, i:11, d1:170, bs:"護国真心"},
	"2226":{n:"諸葛誕", r:"UR", c:4, a:415, i:25, d1:845, bs:"援軍の大神域"},
	"2227":{n:"董昭", r:"SR", c:3.5, a:370, i:24, d1:570, bs:"援軍の大聖域"},
	"2228":{n:"典韋",  r:"L", c:4, a:530, i:9, d1:865, bs:"天衣無縫"},
	"2229":{n:"司馬炎",  r:"L", c:4.5, a:540, i:22, d1:650, bs:"全軍の大極攻令"},
	"2230":{n:"毌丘倹",  r:"UR", c:4, a:405, i:22, d1:560, bs:"隣地神攻城"},
	"2231":{n:"曹操・荀彧",  r:"L", c:4.5, a:740, i:24, d1:665, bs:"強襲大烈覇"},
	"2232":{n:"王元姫",  r:"UR", c:1, a:30, i:11, d1:125, bs:"青藍の祈り"},
	"2233":{n:"楽進", r:"L", c:4, a:630, i:12, d1:565, bs:"騎兵の大天撃"},
	"2234":{n:"張郃", r:"R", c:2, a:240, i:8, d1:225, bs:"千里雷光"},
	"2235":{n:"満寵", r:"R", c:2, a:190, i:16, d1:335, bs:"忠節不落"},
	"2236":{n:"趙娥", r:"UR", c:4, a:555, i:13, d1:550, bs:"隣地猛攻"},
	"2237":{n:"曹操", r:"L", c:4.5, a:780, i:24, d1:590, bs:"龍神"},
	"2238":{n:"于禁", r:"UR", c:4, a:570, i:11, d1:505, bs:"騎兵の大神撃"},
	"2239":{n:"曹丕", r:"UR", c:4, a:545, i:22, d1:495, bs:"魏王の覇軍"},
	"2240":{n:"羊祜", r:"SR", c:4, a:515, i:20, d1:430, bs:"騎兵烈攻"},
	"2241":{n:"司馬昭", r:"R", c:4, a:365, i:20, d1:445, bs:"兵器の猛攻"},
	"2242":{n:"夏侯淵", r:"UR", c:4, a:515, i:12, d1:480, bs:"飛速雷光"},
	"2243":{n:"毌丘倹", r:"SR", c:4, a:385, i:21, d1:550, bs:"才略謀略"},
	"2244":{n:"夏侯惇", r:"UR", c:4, a:550, i:13, d1:475, bs:"戦神の貫攻"},
	"2245":{n:"董昭", r:"L", c:4.5, a:520, i:24, d1:750, bs:"報恩の天恵沢"},
	"2246":{n:"荀彧", r:"L", c:4, a:550, i:25, d1:735, bs:"守衛大聖陣"},
	"2247":{n:"趙娥", r:"SR", c:4, a:460, i:12, d1:550, bs:"飛軍雷光"},
	"2248":{n:"曹昂", r:"UR", c:3, a:360, i:11, d1:400, bs:"天里迅雷"},
	"2249":{n:"辛憲英", r:"R", c:1, a:20, i:9, d1:85, bs:"青春の祝い"},
	"2250":{n:"曹昂", r:"L", c:2, a:310, i:11, d1:440, bs:"剴切の鳳雷光"},
	"2251":{n:"王双", r:"UC", c:3, a:250, i:3, d1:390, bs:"守護堅陣"},
	"2252":{n:"曹休", r:"R", c:3.5, a:340, i:13, d1:560, bs:"守衛方陣"},
	"2253":{n:"郭嘉", r:"L", c:4, a:510, i:25, d1:670, bs:"全軍の鳳速令"},
	"2254":{n:"曹休", r:"L", c:4, a:635, i:15, d1:640, bs:"凰里雷光"},
	"2255":{n:"趙娥", r:"L", c:4, a:670, i:14, d1:650, bs:"お菓子の劫略令"},
	"2256":{n:"賈南風", r:"UR", c:4, a:550, i:17, d1:580, bs:"南瓜の劫略令"},
	"2257":{n:"鄧艾", r:"SR", c:4, a:480, i:23, d1:555, bs:"全軍の極攻令"},
	"2258":{n:"曹洪", r:"SR", c:4, a:490, i:12, d1:435, bs:"猛士の貫攻"},
	"2259":{n:"張遼", r:"R", c:3.5, a:430, i:8, d1:400, bs:"猛将の貫攻"},
	"2260":{n:"丁夫人", r:"L", c:4, a:460, i:18, d1:805, bs:"副将の天明鏡"},
	"2261":{n:"曹丕・甄姫", r:"UR", c:4, a:420, i:19, d1:780, bs:"守備聖陣"},
	"2262":{n:"王元姫", r:"SR", c:1, a:20, i:14, d1:70, bs:"守衛招致"},
	"2263":{n:"張春華", r:"R", c:1, a:15, i:9, d1:135, bs:"絹雲の祝い"},
	"2264":{n:"夏侯惇", r:"R", c:4, a:435, i:11, d1:460, bs:"鬼神"},
	"2265":{n:"曹操・典韋・許褚", r:"L", c:1, a:220, i:20, d1:200, bs:"剴切大聖陣"},
	"2266":{n:"于禁", r:"R", c:3.5, a:415, i:9, d1:405, bs:"隣地猛襲"},
	"2272":{n:"夏侯恩", r:"UR", c:4, a:530, i:14, d1:530, bs:"鬼神の攻奪"},
	"2273":{n:"張魯", r:"L", c:4.5, a:510, i:27, d1:655, bs:"豊穣の天恵沢"},
	"2274":{n:"鮑信", r:"UR", c:4, a:520, i:18, d1:520, bs:"鬼神の攻奪"},
	"2275":{n:"卞夫人", r:"UR", c:1, a:20, i:10, d1:120, bs:"貫攻招来"},
	"2276":{n:"王元姫", r:"UR", c:1, a:20, i:16, d1:130, bs:"守勢招来"},
	"2277":{n:"曹憲", r:"R", c:4, a:415, i:12, d1:350, bs:"闘将の劫略令"},
	"2278":{n:"郭夫人", r:"L", c:4, a:440, i:27, d1:810, bs:"副将の天明鏡"},
	"2279":{n:"于禁", r:"L", c:4.5, a:660, i:13, d1:870, bs:"龍神の守勢"},
	"2282":{n:"朱霊", r:"SR", c:4, a:485, i:10, d1:465, bs:"猛将の劫略令"},
	"2283":{n:"曹操", r:"UC", c:4, a:380, i:16, d1:445, bs:"騎兵の大極撃"},
	"2284":{n:"張魯", r:"UC", c:4, a:260, i:15, d1:440, bs:"徳義為政"},
	"2285":{n:"李典", r:"SR", c:4, a:495, i:14, d1:430, bs:"猛将の劫略令"},
	"2286":{n:"張春華", r:"UR", c:4, a:520, i:12, d1:580, bs:"副将の神明鏡"},
	"2287":{n:"曹洪", r:"L", c:4.5, a:550, i:14, d1:930, bs:"守衛大聖陣"},
	"2288":{n:"曹丕", r:"L", c:4, a:675, i:23, d1:610, bs:"双兵の大天撃"},
	"2289":{n:"許褚", r:"L", c:4, a:735, i:10, d1:670, bs:"錘兵の大天撃"},
	"2290":{n:"趙娥", r:"UR", c:4, a:525, i:13, d1:490, bs:"双兵烈攻"},
	"2291":{n:"牛金", r:"SR", c:4, a:410, i:9, d1:565, bs:"斧兵の大聖域"},
	"2292":{n:"文聘", r:"SR", c:4, a:420, i:11, d1:625, bs:"錘兵の大聖域"},
	"2293":{n:"張郃", r:"UC", c:3, a:345, i:8, d1:335, bs:"双兵の強撃"},
	"2294":{n:"曹操・関羽", r:"L", c:4.5, a:810, i:21, d1:600, bs:"龍神突貫"},
	"2295":{n:"曹丕・孟達", r:"L", c:4.5, a:805, i:16, d1:700, bs:"龍神"},
	"2296":{n:"卞夫人", r:"L", c:2, a:250, i:13, d1:340, bs:"青龍の啓示"},
	"2297":{n:"曹植", r:"UR", c:4, a:415, i:22, d1:820, bs:"天衣無縫"},
	"2298":{n:"文聘", r:"SR", c:4, a:425, i:10, d1:660, bs:"槍将の封聖域"},
	"2299":{n:"牛金", r:"R", c:4, a:455, i:8, d1:450, bs:"槍兵烈攻"},
	"2300":{n:"賈南風", r:"R", c:3, a:375, i:13, d1:410, bs:"戦女の行軍"},
	"2301":{n:"毛玠", r:"L", c:4.5, a:505, i:27, d1:660, bs:"強兵の大檄文"},
	"2302":{n:"曹洪", r:"R", c:1.5, a:160, i:4, d1:180, bs:"万里雷光"},
	"2303":{n:"龐徳", r:"UR", c:4, a:530, i:15, d1:500, bs:"鬼神の叛攻令"},
	"2304":{n:"張允", r:"SR", c:4, a:505, i:13, d1:500, bs:"猛将の叛攻令"},
	"2305":{n:"文欽・毌丘倹", r:"R", c:3.5, a:400, i:13, d1:510, bs:"覇者の護り"},
	"2306":{n:"夏侯淵", r:"SR", c:4, a:470, i:11, d1:450, bs:"猛将の攻奪"},
	"2307":{n:"程昱", r:"R", c:4, a:330, i:17, d1:455, bs:"全兵の猛攻令"},
	"2308":{n:"曹憲・曹華", r:"L", c:4.5, a:655, i:15, d1:990, bs:"龍神の守勢"},
	"2309":{n:"甄姫", r:"L", c:4.5, a:510, i:11, d1:885, bs:"副将の天明鏡"},
	"2310":{n:"郭嘉", r:"L", c:4.5, a:805, i:25, d1:660, bs:"龍神の極意"},
	"2311":{n:"曹洪", r:"L", c:4, a:700, i:14, d1:565, bs:"龍神の劫略令"},
	"2312":{n:"典韋", r:"L", c:4, a:715, i:9, d1:840, bs:"覇王の帝俊護"},
	"2313":{n:"曹丕・曹植・曹沖", r:"L", c:4.5, a:800, i:25, d1:705, bs:"魏帝の叛攻令"},
	"2314":{n:"曹節", r:"R", c:3.5, a:335, i:11, d1:545, bs:"覇者の護り"},
	"2315":{n:"荀彧", r:"SR", c:4, a:365, i:21, d1:685, bs:"守護聖陣"},
	"2316":{n:"鄧艾", r:"L", c:4, a:630, i:26, d1:620, bs:"全軍の鳳速令"},
	"2317":{n:"鍾繇", r:"SR", c:4, a:395, i:24, d1:545, bs:"全兵の烈速令"},
	"2318":{n:"曹節", r:"UR", c:2, a:225, i:14, d1:290, bs:"騎神の導き"},
	"2319":{n:"徐晃", r:"L", c:4.5, a:815, i:18, d1:630, bs:"隣地極攻"},
	"2320":{n:"諸葛誕", r:"SR", c:4, a:380, i:24, d1:600, bs:"智将砕器"},
	"2321":{n:"曹華", r:"R", c:2, a:180, i:10, d1:355, bs:"猛士の守勢"},
	"2322":{n:"典韋", r:"L", c:4.5, a:785, i:9, d1:645, bs:"龍神の療略令"},
	"2323":{n:"楽進", r:"UR", c:4, a:515, i:11, d1:500, bs:"鬼神の攻奪"},
	"2324":{n:"荀攸", r:"L", c:4.5, a:500, i:27, d1:675, bs:"攻城の神義勇軍"},
	"2325":{n:"王元姫", r:"R", c:1, a:15, i:10, d1:65, bs:"旋風の恵み"},
	"2326":{n:"曹節", r:"UR", c:4, a:575, i:14, d1:470, bs:"南瓜の療略令"},
	"2327":{n:"司馬懿・牛金", r:"L", c:4.5, a:800, i:20, d1:655, bs:"龍神の叛攻令"},
	"2328":{n:"賈南風", r:"R", c:4, a:435, i:13, d1:415, bs:"双兵の極攻"},
	"2329":{n:"牛金", r:"R", c:4, a:410, i:8, d1:445, bs:"猛将の叛攻令"},
	"2330":{n:"文聘", r:"R", c:4, a:420, i:9, d1:465, bs:"無双"},
	"2331":{n:"鄧艾・郭淮", r:"SR", c:4, a:440, i:21, d1:440, bs:"猛将の療略令"},
	"2332":{n:"曹沖・華佗", r:"L", c:2, a:410, i:19, d1:315, bs:"剴切の天療令"},
	"2333":{n:"曹華", r:"L", c:1, a:175, i:14, d1:170, bs:"副将の天明鏡"},
	"2334":{n:"曹憲", r:"L", c:1, a:170, i:16, d1:175, bs:"副将の天明鏡"},
	"2335":{n:"夏侯恩", r:"UC", c:4, a:370, i:8, d1:380, bs:"武将の攻奪"},
	"2336":{n:"趙娥", r:"SL", c:4, a:800, i:15, d1:665, bs:"無双"},
	"2337":{n:"夏侯惇", r:"SL", c:4.5, a:890, i:9, d1:755, bs:"双兵の大帝撃"},
	"2338":{n:"曹仁・牛金", r:"L", c:4.5, a:800, i:11, d1:620, bs:"龍神の療略令"},
	"2339":{n:"趙娥", r:"R", c:4, a:380, i:10, d1:460, bs:"双兵舎修練"},
	"2340":{n:"曹丕・曹昂", r:"L", c:2, a:395, i:21, d1:310, bs:"剴切の天砕攻"},
	"2341":{n:"郭嘉", r:"R", c:4, a:450, i:15, d1:395, bs:"猛将の攻奪"},
	"2342":{n:"辛憲英", r:"UR", c:2, a:230, i:17, d1:205, bs:"双神の導き"},
	"2343":{n:"楽進", r:"R", c:1, a:110, i:9, d1:110, bs:"飛蹄進軍"},
	"3001":{n:"孫権", r:"R", c:2.5, a:180, i:14, d1:295, bs:"呉の治世"},
	"3002":{n:"周瑜", r:"SR", c:3.5, a:340, i:23, d1:570, bs:"弓将の采配"},
	"3003":{n:"陸遜", r:"SR", c:3, a:305, i:19, d1:450, bs:"王者の護り"},
	"3004":{n:"孫策", r:"SR", c:3, a:350, i:9, d1:385, bs:"覇王の進撃"},
	"3005":{n:"甘寧", r:"R", c:3, a:345, i:6, d1:275, bs:"弓兵の猛撃"},
	"3006":{n:"孫尚香", r:"SR", c:2, a:210, i:8, d1:255, bs:"弓腰姫の愛"},
	"3007":{n:"呂蒙", r:"R", c:3, a:320, i:15, d1:370, bs:"深慮遠謀"},
	"3008":{n:"孫権", r:"UC", c:2.5, a:180, i:12, d1:295, bs:"呉の治世"},
	"3009":{n:"周瑜", r:"UC", c:2.5, a:200, i:14, d1:365, bs:"弓兵の進撃"},
	"3010":{n:"周瑜", r:"R", c:2.5, a:200, i:15, d1:385, bs:"弓兵防御"},
	"3011":{n:"程普", r:"UC", c:2.5, a:220, i:10, d1:295, bs:"兵舎訓練"},
	"3012":{n:"程普", r:"C", c:2.5, a:220, i:10, d1:280, bs:"槍兵防御"},
	"3013":{n:"黄蓋", r:"UC", c:2.5, a:250, i:7, d1:230, bs:"兵器行軍"},
	"3014":{n:"諸葛瑾", r:"UC", c:2, a:135, i:11, d1:220, bs:"伐採知識"},
	"3015":{n:"朱治", r:"UC", c:2, a:135, i:9, d1:230, bs:"弓兵防御"},
	"3016":{n:"朱治", r:"C", c:2, a:135, i:9, d1:220, bs:"弓兵防御"},
	"3017":{n:"韓当", r:"UC", c:2, a:220, i:5, d1:190, bs:"騎兵の進撃"},
	"3018":{n:"韓当", r:"C", c:2, a:210, i:5, d1:185, bs:"騎兵行軍"},
	"3019":{n:"蘇飛", r:"UC", c:2, a:160, i:8, d1:235, bs:"弓兵の進撃"},
	"3020":{n:"蘇飛", r:"C", c:2, a:170, i:8, d1:225, bs:"剣兵の進撃"},
	"3021":{n:"闞沢", r:"UC", c:2, a:125, i:12, d1:105, bs:"石切知識"},
	"3022":{n:"闞沢", r:"C", c:2, a:115, i:12, d1:100, bs:"石切知識"},
	"3023":{n:"蒋欽", r:"UC", c:2, a:195, i:6, d1:195, bs:"弓兵の進撃"},
	"3024":{n:"蒋欽", r:"C", c:2, a:185, i:6, d1:190, bs:"弓兵の進撃"},
	"3025":{n:"孫翊", r:"UC", c:1.5, a:150, i:2, d1:100, bs:"豪傑"},
	"3026":{n:"孫翊", r:"C", c:1.5, a:140, i:2, d1:100, bs:"豪傑"},
	"3027":{n:"孫匡", r:"UC", c:1.5, a:110, i:5, d1:160, bs:"弓兵訓練"},
	"3028":{n:"孫匡", r:"C", c:1.5, a:110, i:5, d1:150, bs:"弓兵防御"},
	"3029":{n:"祖茂", r:"UC", c:1.5, a:160, i:4, d1:160, bs:"豪傑"},
	"3030":{n:"祖茂", r:"C", c:1.5, a:150, i:4, d1:160, bs:"槍兵の進撃"},
	"3031":{n:"大喬", r:"R", c:1, a:20, i:8, d1:140, bs:"富国"},
	"3032":{n:"小喬", r:"R", c:1, a:25, i:7, d1:170, bs:"強兵の檄文"},
	"3033":{n:"孫堅", r:"R", c:3, a:320, i:11, d1:395, bs:"剣兵の極撃"},
	"3034":{n:"魯粛", r:"R", c:2, a:230, i:14, d1:300, bs:"兵器修練"},
	"3035":{n:"魯粛", r:"UC", c:2, a:230, i:13, d1:280, bs:"槍兵堅守"},
	"3036":{n:"太史慈", r:"R", c:3.5, a:390, i:8, d1:480, bs:"守護神"},
	"3037":{n:"陸遜", r:"R", c:2.5, a:260, i:18, d1:355, bs:"八卦の陣"},
	"3038":{n:"周泰", r:"UC", c:2.5, a:210, i:5, d1:310, bs:"鉄壁"},
	"3039":{n:"周泰", r:"C", c:2.5, a:210, i:5, d1:300, bs:"槍兵堅守"},
	"3040":{n:"孫権", r:"SR", c:2.5, a:250, i:13, d1:365, bs:"大皇帝"},
	"3041":{n:"孫策", r:"R", c:2.5, a:280, i:8, d1:330, bs:"騎兵突撃"},
	"3042":{n:"甘寧", r:"R", c:2.5, a:290, i:3, d1:225, bs:"一騎当千"},
	"3043":{n:"甘寧", r:"UC", c:2.5, a:280, i:3, d1:205, bs:"槍兵強行"},
	"3044":{n:"孫堅", r:"UR", c:3.5, a:420, i:18, d1:520, bs:"大皇帝"},
	"3045":{n:"周瑜", r:"UR", c:3.5, a:395, i:24, d1:510, bs:"弓兵の極撃"},
	"3046":{n:"孫尚香", r:"UR", c:2.5, a:240, i:16, d1:320, bs:"市場繁栄"},
	"3047":{n:"孫策", r:"UR", c:4, a:505, i:10, d1:600, bs:"飛将"},
	"3048":{n:"太史慈", r:"SR", c:3.5, a:420, i:6, d1:445, bs:"弓兵の猛撃"},
	"3049":{n:"孫策", r:"UC", c:2.5, a:290, i:6, d1:230, bs:"豪傑"},
	"3050":{n:"孫翊", r:"R", c:1.5, a:170, i:4, d1:110, bs:"一騎当千"},
	"3051":{n:"黄蓋", r:"R", c:2.5, a:275, i:9, d1:245, bs:"兵器強行"},
	"3052":{n:"陸遜", r:"UC", c:2.5, a:210, i:13, d1:340, bs:"弓兵方陣"},
	"3053":{n:"呂蒙", r:"UR", c:3, a:330, i:4, d1:380, bs:"覇王の進撃"},
	"3054":{n:"甘寧", r:"SR", c:3, a:340, i:5, d1:310, bs:"強襲突覇"},
	"3055":{n:"魯粛", r:"UR", c:2, a:150, i:19, d1:455, bs:"緊急帰還"},
	"3056":{n:"大喬", r:"UR", c:1, a:30, i:9, d1:150, bs:"喬姫の祈り"},
	"3057":{n:"小喬", r:"UR", c:1, a:35, i:8, d1:180, bs:"喬姫の激励"},
	"3058":{n:"陸遜", r:"UR", c:3.5, a:395, i:24, d1:540, bs:"王者の護り"},
	"3059":{n:"太史慈", r:"UC", c:3, a:310, i:8, d1:300, bs:"豪傑"},
	"3060":{n:"孫堅", r:"UR", c:3.5, a:420, i:20, d1:520, bs:"呉軍の極撃"},
	"3061":{n:"諸葛瑾", r:"R", c:2, a:180, i:15, d1:260, bs:"伐採技術"},
	"3062":{n:"呂蒙", r:"UC", c:2.5, a:280, i:11, d1:260, bs:"槍兵突撃"},
	"3063":{n:"程普", r:"R", c:2.5, a:235, i:11, d1:310, bs:"兵舎修練"},
	"3064":{n:"周泰", r:"R", c:2.5, a:265, i:5, d1:385, bs:"槍兵の聖域"},
	"3065":{n:"甘寧", r:"UR", c:3.5, a:440, i:7, d1:505, bs:"隣地猛攻"},
	"3066":{n:"孫権", r:"UR", c:3.5, a:440, i:20, d1:505, bs:"皇帝の勅令"},
	"3067":{n:"黄蓋", r:"UR", c:3, a:350, i:11, d1:345, bs:"苦肉の計"},
	"3068":{n:"太史慈", r:"UR", c:3.5, a:450, i:7, d1:430, bs:"槍戟鬼神"},
	"3069":{n:"魯粛", r:"SR", c:2.5, a:265, i:18, d1:285, bs:"弓兵の勝鬨"},
	"3070":{n:"孫堅", r:"UC", c:2.5, a:250, i:13, d1:375, bs:"弓兵の強撃"},
	"3071":{n:"凌統", r:"R", c:2.5, a:280, i:9, d1:270, bs:"槍兵突覇"},
	"3072":{n:"祖茂", r:"R", c:1.5, a:180, i:4, d1:175, bs:"覇道"},
	"3073":{n:"徐盛", r:"R", c:2.5, a:270, i:14, d1:310, bs:"弓兵突覇"},
	"3074":{n:"張昭", r:"SR", c:2.5, a:170, i:19, d1:280, bs:"富国論"},
	"3075":{n:"丁奉", r:"R", c:2.5, a:275, i:12, d1:310, bs:"弓兵の聖域"},
	"3076":{n:"張紘", r:"SR", c:2.5, a:180, i:19, d1:210, bs:"迅速斥候"},
	"3077":{n:"呉夫人", r:"SR", c:1, a:20, i:9, d1:85, bs:"弓兵増強"},
	"3078":{n:"孫策", r:"R", c:2.5, a:280, i:8, d1:330, bs:"騎兵突覇"},
	"3079":{n:"周瑜", r:"UC", c:2.5, a:200, i:14, d1:365, bs:"弓兵修練"},
	"3080":{n:"甘寧", r:"UC", c:2.5, a:280, i:3, d1:205, bs:"弓兵突撃"},
	"3081":{n:"周瑜", r:"UR", c:3.5, a:460, i:26, d1:510, bs:"神謀風如"},
	"3082":{n:"甘寧", r:"SR", c:3.5, a:430, i:12, d1:505, bs:"奇襲"},
	"3083":{n:"孫尚香", r:"R", c:2, a:240, i:10, d1:255, bs:"弓将の采配"},
	"3084":{n:"太史慈", r:"UC", c:3, a:340, i:9, d1:300, bs:"弓兵の強撃"},
	"3085":{n:"程普", r:"SR", c:3, a:330, i:16, d1:380, bs:"槍兵の猛撃"},
	"3086":{n:"諸葛恪", r:"R", c:2.5, a:180, i:17, d1:265, bs:"弓兵の聖域"},
	"3087":{n:"孫策", r:"UR", c:4, a:510, i:13, d1:600, bs:"国士無双"},
	"3088":{n:"呉国太", r:"SR", c:2, a:10, i:9, d1:110, bs:"市場繁栄"},
	"3089":{n:"呂蒙", r:"UR", c:3.5, a:420, i:23, d1:505, bs:"呉将の督戦"},
	"3090":{n:"孫尚香", r:"SR", c:2, a:215, i:9, d1:265, bs:"弓兵の勝鬨"},
	"3091":{n:"陸遜", r:"UR", c:3.5, a:390, i:24, d1:585, bs:"呉軍の防衛"},
	"3092":{n:"朱桓", r:"UC", c:2.5, a:275, i:9, d1:265, bs:"槍兵の進攻"},
	"3093":{n:"張紘", r:"R", c:2.5, a:160, i:18, d1:190, bs:"密偵精鋭"},
	"3094":{n:"蒋欽", r:"R", c:2, a:270, i:7, d1:235, bs:"弓兵の強攻"},
	"3095":{n:"孫堅", r:"SR", c:3.5, a:410, i:17, d1:505, bs:"拠点襲撃"},
	"3096":{n:"徐盛", r:"UC", c:2.5, a:280, i:9, d1:275, bs:"弓兵の進攻"},
	"3097":{n:"黄蓋", r:"SR", c:3.5, a:330, i:15, d1:460, bs:"防将戦略"},
	"3098":{n:"呉国太", r:"R", c:1, a:10, i:7, d1:80, bs:"市場知識"},
	"3099":{n:"丁奉", r:"UC", c:2.5, a:270, i:11, d1:345, bs:"弓兵方陣"},
	"3100":{n:"張昭", r:"UR", c:3, a:230, i:21, d1:230, bs:"老巧の政令"},
	"3101":{n:"凌統", r:"SR", c:3.5, a:400, i:11, d1:435, bs:"槍戟鬼神"},
	"3102":{n:"陸抗", r:"R", c:2, a:145, i:17, d1:325, bs:"堅牢知略"},
	"3103":{n:"呉夫人", r:"R", c:1, a:15, i:8, d1:75, bs:"弓兵増強"},
	"3104":{n:"孫権", r:"UR", c:3.5, a:440, i:18, d1:460, bs:"弓兵の極攻"},
	"3105":{n:"小喬", r:"SR", c:1, a:30, i:7, d1:175, bs:"小華の舞"},
	"3106":{n:"虎姉妹", r:"UR", c:2.5, a:280, i:6, d1:230, bs:"弓兵の強撃"},
	"3107":{n:"大喬", r:"SR", c:1, a:25, i:8, d1:145, bs:"大華の舞"},
	"3108":{n:"闞沢", r:"R", c:1.5, a:115, i:13, d1:95, bs:"石切技術"},
	"3109":{n:"凌操", r:"SR", c:3.5, a:415, i:8, d1:425, bs:"弓兵の猛攻"},
	"3110":{n:"諸葛恪", r:"UC", c:1.5, a:85, i:12, d1:100, bs:"加工知識"},
	"3111":{n:"太史慈", r:"UR", c:4, a:500, i:8, d1:540, bs:"弓兵の大極撃"},
	"3112":{n:"周泰", r:"SR", c:3.5, a:410, i:6, d1:505, bs:"不撓不屈"},
	"3113":{n:"虞翻", r:"SR", c:2.5, a:250, i:18, d1:280, bs:"弩兵移送"},
	"3114":{n:"陸遜・呂蒙", r:"UR", c:4, a:450, i:27, d1:600, bs:"孫呉の烈火"},
	"3115":{n:"周瑜", r:"SR", c:1.5, a:150, i:17, d1:255, bs:"弓兵の大号令"},
	"3116":{n:"大喬", r:"R", c:1, a:10, i:9, d1:110, bs:"優姫の敬愛"},
	"3117":{n:"甘寧", r:"R", c:3.5, a:420, i:10, d1:400, bs:"飛将"},
	"3118":{n:"孫堅", r:"UC", c:3, a:345, i:8, d1:325, bs:"槍兵の進攻"},
	"3119":{n:"張紘", r:"UC", c:1.5, a:75, i:12, d1:110, bs:"弓兵訓練"},
	"3120":{n:"蘇飛", r:"R", c:2.5, a:285, i:8, d1:275, bs:"弓兵速攻"},
	"3121":{n:"歩隲", r:"R", c:2, a:150, i:17, d1:260, bs:"弓兵急配"},
	"3122":{n:"歩夫人", r:"SR", c:1, a:15, i:9, d1:100, bs:"石切技巧"},
	"3123":{n:"孫翊", r:"SR", c:3, a:340, i:4, d1:345, bs:"騎兵の猛攻"},
	"3124":{n:"韓当", r:"SR", c:3.5, a:405, i:10, d1:575, bs:"槍兵の猛攻"},
	"3125":{n:"孫権", r:"SR", c:3.5, a:435, i:15, d1:410, bs:"若獅子の軍略"},
	"3126":{n:"徐氏", r:"SR", c:1, a:20, i:9, d1:100, bs:"製鉄技巧"},
	"3127":{n:"蒋欽", r:"SR", c:3, a:340, i:8, d1:345, bs:"弓兵の猛攻"},
	"3128":{n:"孫尚香", r:"UC", c:3, a:300, i:12, d1:330, bs:"弓兵速攻"},
	"3129":{n:"大虎", r:"UR", c:2, a:240, i:10, d1:190, bs:"呉軍の極撃"},
	"3130":{n:"小虎", r:"SR", c:3.5, a:335, i:11, d1:560, bs:"護君"},
	"3131":{n:"孫尚香", r:"R", c:2.5, a:290, i:10, d1:335, bs:"飛蹄進軍"},
	"3132":{n:"凌操", r:"UC", c:2.5, a:285, i:8, d1:300, bs:"武将突貫"},
	"3133":{n:"陸抗", r:"UC", c:2, a:120, i:16, d1:240, bs:"呉の治世"},
	"3134":{n:"朱桓", r:"SR", c:2.5, a:295, i:9, d1:300, bs:"槍兵突攻"},
	"3135":{n:"凌操", r:"R", c:3, a:340, i:8, d1:390, bs:"謀反の強攻"},
	"3136":{n:"韓当", r:"R", c:2.5, a:270, i:7, d1:310, bs:"兵器の進攻"},
	"3137":{n:"徐盛", r:"SR", c:3.5, a:430, i:12, d1:380, bs:"猛将の鹵獲"},
	"3138":{n:"孫策・周瑜", r:"UR", c:4, a:580, i:24, d1:500, bs:"弓兵の大神撃"},
	"3139":{n:"潘璋", r:"SR", c:4, a:490, i:14, d1:480, bs:"弓兵の大極撃"},
	"3140":{n:"大虎", r:"SR", c:1, a:100, i:9, d1:90, bs:"苛政虎舞"},
	"3141":{n:"孫氏", r:"R", c:3.5, a:400, i:15, d1:390, bs:"弓神降臨"},
	"3142":{n:"小虎", r:"R", c:2, a:50, i:10, d1:60, bs:"孫家の恵み"},
	"3143":{n:"虞翻", r:"UC", c:3, a:230, i:19, d1:250, bs:"食糧技術"},
	"3144":{n:"潘璋", r:"R", c:3.5, a:430, i:12, d1:405, bs:"猛士の極意"},
	"3145":{n:"張昭", r:"R", c:3, a:150, i:20, d1:200, bs:"加工技術"},
	"3146":{n:"孫桓", r:"UR", c:4, a:520, i:15, d1:520, bs:"鬼神の鹵獲"},
	"3147":{n:"歩隲", r:"UC", c:2, a:130, i:16, d1:250, bs:"剣兵堅守"},
	"3148":{n:"呂蒙", r:"SR", c:3.5, a:420, i:18, d1:440, bs:"覇王の猛攻"},
	"3149":{n:"周泰", r:"R", c:3, a:370, i:5, d1:350, bs:"隣地猛攻"},
	"3150":{n:"孫堅", r:"R", c:3, a:360, i:12, d1:365, bs:"槍兵の猛攻"},
	"3151":{n:"凌統", r:"UC", c:3, a:340, i:7, d1:340, bs:"万夫不当"},
	"3152":{n:"張昭・張紘", r:"UR", c:4, a:400, i:27, d1:520, bs:"呉の治世"},
	"3153":{n:"丁奉", r:"SR", c:4, a:390, i:14, d1:615, bs:"弓兵の大聖域"},
	"3154":{n:"大喬・小喬", r:"UR", c:4, a:360, i:21, d1:550, bs:"万華招来"},
	"3155":{n:"呉夫人", r:"UC", c:3, a:150, i:18, d1:390, bs:"弓兵修練"},
	"3156":{n:"小虎", r:"UR", c:3.5, a:480, i:12, d1:515, bs:"弓襲遠攻"},
	"3157":{n:"孫氏", r:"SR", c:4, a:490, i:18, d1:465, bs:"孫呉の烈火"},
	"3158":{n:"朱治", r:"R", c:3.5, a:420, i:10, d1:395, bs:"弓兵突攻"},
	"3159":{n:"太史慈", r:"SR", c:4, a:510, i:7, d1:565, bs:"鬼神の極意"},
	"3160":{n:"孫策", r:"R", c:3.5, a:405, i:9, d1:430, bs:"猛将の極意"},
	"3161":{n:"諸葛瑾", r:"SR", c:1.5, a:200, i:16, d1:190, bs:"徳義為政"},
	"3162":{n:"虞翻", r:"R", c:3.5, a:275, i:20, d1:420, bs:"全軍の強攻令"},
	"3163":{n:"周姫", r:"UR", c:4, a:355, i:23, d1:660, bs:"守衛聖陣"},
	"3164":{n:"大喬", r:"SR", c:3, a:100, i:16, d1:320, bs:"千華招来"},
	"3165":{n:"孫堅", r:"L", c:4, a:670, i:22, d1:615, bs:"弓兵の大天撃"},
	"3166":{n:"孫尚香", r:"L", c:3.5, a:530, i:18, d1:505, bs:"無双"},
	"3167":{n:"陸抗", r:"UR", c:4, a:505, i:25, d1:465, bs:"全軍の極攻令"},
	"3168":{n:"陳武", r:"SR", c:4, a:485, i:14, d1:400, bs:"全軍の猛攻令"},
	"3169":{n:"虞翻", r:"UR", c:4, a:380, i:24, d1:800, bs:"弓兵の大神域"},
	"3170":{n:"大虎", r:"R", c:3, a:310, i:10, d1:430, bs:"剣将の采配"},
	"3171":{n:"甘寧", r:"L", c:4, a:610, i:14, d1:565, bs:"凰里雷光"},
	"3172":{n:"周姫", r:"SR", c:4, a:330, i:27, d1:605, bs:"呉の治世"},
	"3173":{n:"孫策", r:"L", c:4, a:615, i:14, d1:590, bs:"鳳速雷光"},
	"3174":{n:"呂蒙", r:"L", c:4, a:650, i:24, d1:595, bs:"槍兵の大極攻"},
	"3175":{n:"周姫", r:"UR", c:3.5, a:340, i:25, d1:585, bs:"万華招来"},
	"3176":{n:"周瑜", r:"SR", c:3.5, a:440, i:22, d1:420, bs:"弓兵の猛攻"},
	"3177":{n:"魯粛", r:"R", c:4, a:255, i:18, d1:660, bs:"守護防陣"},
	"3178":{n:"陳武", r:"UR", c:4, a:390, i:15, d1:735, bs:"槍将の封聖域"},
	"3179":{n:"周瑜", r:"SR", c:3.5, a:405, i:23, d1:480, bs:"全軍の極攻令"},
	"3180":{n:"周瑜", r:"R", c:3, a:300, i:17, d1:355, bs:"智将器撃"},
	"3181":{n:"孫桓", r:"R", c:3, a:340, i:11, d1:375, bs:"全軍の神速令"},
	"3182":{n:"程普", r:"L", c:4, a:470, i:18, d1:850, bs:"槍兵の大天域"},
	"3183":{n:"張紘", r:"UR", c:4, a:375, i:25, d1:500, bs:"強兵の義勇軍"},
	"3184":{n:"陸遜", r:"SR", c:4, a:395, i:21, d1:620, bs:"覇者の護り"},
	"3185":{n:"丁奉", r:"UR", c:4, a:490, i:15, d1:555, bs:"全軍の飛速令"},
	"3186":{n:"蒋欽", r:"UR", c:4, a:530, i:10, d1:490, bs:"弓兵烈攻"},
	"3187":{n:"歩隲", r:"C", c:2, a:125, i:13, d1:245, bs:"剣兵防御"},
	"3188":{n:"周瑜・黄蓋", r:"L", c:4.5, a:700, i:21, d1:755, bs:"兵器の縮地天撃"},
	"3189":{n:"歩夫人", r:"UR", c:1, a:20, i:10, d1:120, bs:"迅雷招来"},
	"3190":{n:"孫皎", r:"SR", c:4, a:520, i:16, d1:510, bs:"剣兵の大神撃"},
	"3191":{n:"全琮", r:"R", c:4, a:430, i:13, d1:415, bs:"騎兵の極攻"},
	"3192":{n:"徐氏", r:"R", c:1, a:15, i:8, d1:90, bs:"弓兵増勢"},
	"3193":{n:"孫匡", r:"R", c:3, a:375, i:8, d1:395, bs:"隣地豪襲"},
	"3194":{n:"孫氏", r:"UR", c:4, a:500, i:19, d1:520, bs:"収奪の極攻"},
	"3195":{n:"張昭", r:"L", c:4, a:470, i:25, d1:670, bs:"食糧天革命"},
	"3196":{n:"孫権", r:"L", c:4, a:565, i:24, d1:590, bs:"全軍の大極攻令"},
	"3197":{n:"凌統", r:"UR", c:4, a:535, i:13, d1:475, bs:"攻城戦の極攻"},
	"3198":{n:"大喬", r:"L", c:4.5, a:490, i:22, d1:720, bs:"攻城の義勇軍"},
	"3199":{n:"朱皇后", r:"UR", c:1, a:30, i:10, d1:140, bs:"守城誠心"},
	"3200":{n:"周姫", r:"R", c:4, a:300, i:22, d1:700, bs:"護君"},
	"3201":{n:"小喬", r:"L", c:4, a:510, i:16, d1:670, bs:"春遊の呼集"},
	"3202":{n:"太史慈", r:"L", c:4, a:645, i:10, d1:600, bs:"槍兵大烈攻"},
	"3203":{n:"朱桓", r:"R", c:3, a:350, i:11, d1:340, bs:"闘将の貫攻"},
	"3204":{n:"蘇飛", r:"SR", c:3.5, a:420, i:11, d1:410, bs:"鹵獲の極攻"},
	"3205":{n:"徐盛", r:"R", c:3, a:360, i:10, d1:310, bs:"闘将の極意"},
	"3206":{n:"陳武", r:"R", c:1, a:160, i:7, d1:90, bs:"一騎当千"},
	"3207":{n:"潘璋", r:"UC", c:3, a:285, i:10, d1:370, bs:"武将の守勢"},
	"3208":{n:"小喬", r:"UR", c:1, a:30, i:8, d1:160, bs:"全兵招来"},
	"3209":{n:"陸抗", r:"SR", c:4, a:445, i:24, d1:440, bs:"攻城戦の猛砕"},
	"3210":{n:"大虎",  r:"L", c:4.5, a:755, i:15, d1:710, bs:"龍神突貫"},
	"3211":{n:"周姫",  r:"L", c:4, a:545, i:27, d1:820, bs:"呉の大治世"},
	"3212":{n:"徐氏",  r:"UR", c:1, a:30, i:10, d1:120, bs:"紅梅の祈り"},
	"3213":{n:"陸遜",  r:"L", c:4, a:555, i:29, d1:580, bs:"呉の大治世"},
	"3214":{n:"孫権・陸遜",  r:"L", c:4.5, a:800, i:22, d1:655, bs:"攻城戦の大極攻"},
	"3215":{n:"黄蓋", r:"L", c:4, a:680, i:12, d1:605, bs:"弓兵の大天撃"},
	"3216":{n:"孫氏", r:"L", c:4.5, a:505, i:20, d1:915, bs:"覇者の大神域"},
	"3217":{n:"孫権", r:"L", c:4.5, a:720, i:24, d1:735, bs:"兵器の縮地天撃"},
	"3218":{n:"陸抗", r:"UR", c:4, a:590, i:25, d1:555, bs:"弓兵の大神撃"},
	"3219":{n:"虞翻", r:"UR", c:4, a:500, i:24, d1:555, bs:"全軍の飛速令"},
	"3220":{n:"文鴦", r:"SR", c:4, a:530, i:12, d1:490, bs:"弓兵烈攻"},
	"3221":{n:"陳武", r:"R", c:4, a:360, i:13, d1:635, bs:"守護方陣"},
	"3222":{n:"周瑜", r:"L", c:4, a:640, i:26, d1:600, bs:"龍神の攻奪"},
	"3223":{n:"孫尚香", r:"L", c:4, a:610, i:19, d1:590, bs:"全兵の大極攻令"},
	"3224":{n:"甘寧", r:"L", c:4, a:670, i:14, d1:580, bs:"強襲天奮迅"},
	"3225":{n:"諸葛瑾", r:"UR", c:3, a:290, i:26, d1:350, bs:"智将の大神撃"},
	"3226":{n:"韓当", r:"UR", c:3, a:410, i:10, d1:355, bs:"鬼神の縮地劫略"},
	"3227":{n:"朱皇后", r:"R", c:1, a:20, i:8, d1:90, bs:"朱夏の祝い"},
	"3228":{n:"歩隲", r:"SR", c:4, a:335, i:27, d1:580, bs:"呉の治世"},
	"3229":{n:"魯粛", r:"L", c:4, a:520, i:24, d1:880, bs:"守護大聖陣"},
	"3230":{n:"凌統", r:"L", c:2, a:380, i:10, d1:285, bs:"剴切の天砕器"},
	"3231":{n:"全琮", r:"UC", c:3, a:340, i:10, d1:325, bs:"闘将の極意"},
	"3232":{n:"黄蓋", r:"R", c:1, a:120, i:6, d1:125, bs:"苦肉の計"},
	"3233":{n:"大喬", r:"L", c:4, a:550, i:21, d1:555, bs:"副将の天明鏡"},
	"3234":{n:"周瑜・小喬", r:"UR", c:4, a:550, i:19, d1:570, bs:"天里雷鳴"},
	"3235":{n:"歩夫人", r:"SR", c:1, a:15, i:9, d1:105, bs:"江河の祝福"},
	"3236":{n:"徐氏", r:"R", c:1, a:15, i:8, d1:95, bs:"飛泉の祝い"},
	"3237":{n:"孫策", r:"R", c:4, a:450, i:10, d1:450, bs:"麒麟慈心"},
	"3238":{n:"呉夫人", r:"L", c:4, a:610, i:24, d1:660, bs:"樅樹の劫略令"},
	"3239":{n:"呉国太", r:"UR", c:4, a:500, i:21, d1:595, bs:"雪華の劫略令"},
	"3242":{n:"周瑜・張昭・張紘", r:"L", c:4.5, a:640, i:28, d1:820, bs:"王佐の天資"},
	"3243":{n:"孫氏", r:"SR", c:4, a:490, i:18, d1:460, bs:"玩具の劫略令"},
	"3244":{n:"孫登", r:"UR", c:2, a:260, i:18, d1:415, bs:"剴切の極攻令"},
	"3246":{n:"孫策", r:"L", c:4.5, a:710, i:13, d1:590, bs:"全軍の鳳速令"},
	"3247":{n:"小虎", r:"L", c:4, a:630, i:14, d1:580, bs:"副将の天明鏡"},
	"3248":{n:"小喬", r:"L", c:4.5, a:575, i:17, d1:810, bs:"全兵の大極攻令"},
	"3252":{n:"丁奉", r:"UR", c:4, a:560, i:15, d1:510, bs:"鬼神の劫略令"},
	"3253":{n:"孫権", r:"UC", c:4, a:385, i:15, d1:450, bs:"兵器突覇"},
	"3254":{n:"陸遜", r:"L", c:4, a:680, i:27, d1:600, bs:"龍神の劫略令"},
	"3255":{n:"朱皇后", r:"UR", c:4, a:395, i:13, d1:870, bs:"副将の神明鏡"},
	"3256":{n:"周泰", r:"L", c:4.5, a:775, i:9, d1:655, bs:"獣神の極意"},
	"3257":{n:"大虎・小虎", r:"L", c:4.5, a:590, i:11, d1:620, bs:"全軍の鳳速令"},
	"3258":{n:"朱治", r:"L", c:2, a:395, i:13, d1:245, bs:"剴切の天攻奪"},
	"3259":{n:"孫堅", r:"L", c:4.5, a:560, i:22, d1:880, bs:"斧兵の大天域"},
	"3260":{n:"周瑜", r:"SR", c:4, a:385, i:24, d1:570, bs:"双兵の大聖域"},
	"3261":{n:"凌統", r:"R", c:3.5, a:425, i:10, d1:335, bs:"双神降臨"},
	"3262":{n:"祖茂", r:"R", c:3.5, a:420, i:8, d1:425, bs:"双兵の猛攻"},
	"3263":{n:"黄蓋", r:"UC", c:3, a:375, i:8, d1:365, bs:"錘兵の強撃"},
	"3264":{n:"歩夫人", r:"UC", c:3, a:125, i:9, d1:325, bs:"斧兵舎訓練"},
	"3265":{n:"徐氏", r:"UC", c:3, a:135, i:9, d1:310, bs:"錘兵舎訓練"},
	"3266":{n:"孫尚香", r:"R", c:4, a:380, i:18, d1:420, bs:"帝妃都政"},
	"3267":{n:"大喬・小喬", r:"L", c:4.5, a:760, i:22, d1:750, bs:"樅樹の劫略令"},
	"3268":{n:"孫登・周姫", r:"L", c:4.5, a:780, i:24, d1:670, bs:"樅樹の叛攻令"},
	"3269":{n:"孫氏", r:"R", c:4, a:440, i:16, d1:420, bs:"飛将"},
	"3270":{n:"孫策・程普", r:"L", c:4.5, a:810, i:16, d1:640, bs:"無双"},
	"3271":{n:"陸遜・孫桓", r:"L", c:4.5, a:730, i:27, d1:650, bs:"兵器の大極攻"},
	"3272":{n:"孫権・諸葛亮", r:"L", c:4.5, a:740, i:29, d1:700, bs:"智将の大天撃"},
	"3273":{n:"周瑜・龐統", r:"L", c:4.5, a:730, i:29, d1:710, bs:"智将天砕器"},
	"3274":{n:"小喬", r:"L", c:2, a:255, i:15, d1:360, bs:"白虎の啓示"},
	"3275":{n:"大虎", r:"UR", c:4, a:590, i:12, d1:560, bs:"龍神の極意"},
	"3276":{n:"朱然", r:"SR", c:4, a:415, i:14, d1:590, bs:"騎将の封聖域"},
	"3277":{n:"孫翊", r:"R", c:4, a:450, i:4, d1:435, bs:"剣兵烈攻"},
	"3278":{n:"孫匡", r:"R", c:4, a:455, i:8, d1:465, bs:"猛将の叛乱"},
	"3279":{n:"太史慈", r:"SR", c:4, a:500, i:9, d1:440, bs:"猛将の叛攻令"},
	"3280":{n:"周姫", r:"L", c:4.5, a:520, i:27, d1:780, bs:"副将の天明鏡"},
	"3281":{n:"小喬", r:"UR", c:1, a:145, i:8, d1:160, bs:"飛隠雷光"},
	"3282":{n:"陸抗", r:"L", c:4.5, a:765, i:26, d1:645, bs:"龍神の貫攻"},
	"3283":{n:"程普", r:"L", c:4, a:740, i:18, d1:625, bs:"龍神突貫"},
	"3284":{n:"呂岱", r:"R", c:4, a:440, i:12, d1:430, bs:"双兵突攻"},
	"3285":{n:"孫尚香", r:"R", c:4, a:480, i:16, d1:480, bs:"錘兵突攻"},
	"3286":{n:"孫策・孫権・孫尚香", r:"L", c:1, a:210, i:19, d1:220, bs:"孫呉大聖陣"},
	"3287":{n:"徐氏", r:"L", c:1, a:115, i:10, d1:180, bs:"副将の天明鏡"},
	"3288":{n:"甘寧", r:"UR", c:4, a:520, i:11, d1:530, bs:"強襲奮迅"},
	"3289":{n:"全琮", r:"SR", c:4, a:490, i:15, d1:495, bs:"猛将の叛攻令"},
	"3290":{n:"周瑜", r:"UR", c:4, a:500, i:27, d1:535, bs:"呉の大治世"},
	"3291":{n:"呉夫人", r:"UR", c:2, a:220, i:17, d1:270, bs:"弓神の導き"},
	"3292":{n:"周泰", r:"L", c:4.5, a:830, i:9, d1:695, bs:"獣神の貫攻"},
	"3293":{n:"甘寧", r:"L", c:4.5, a:770, i:14, d1:600, bs:"龍神の叛攻令"},
	"3294":{n:"歩夫人", r:"R", c:1, a:10, i:8, d1:90, bs:"朔風の恵み"},
	"3295":{n:"朱皇后", r:"SR", c:1, a:25, i:9, d1:95, bs:"劫略招致"},
	"3296":{n:"大虎・小虎", r:"UR", c:4, a:455, i:10, d1:615, bs:"覇者の護り"},
	"3297":{n:"太史慈", r:"R", c:4, a:440, i:6, d1:470, bs:"闘将の攻奪"},
	"3298":{n:"張紘", r:"L", c:4.5, a:515, i:28, d1:675, bs:"斧兵の義勇軍"},
	"3299":{n:"朱皇后", r:"UR", c:4, a:535, i:13, d1:490, bs:"南瓜の療略令"},
	"3300":{n:"徐盛", r:"SR", c:4, a:530, i:12, d1:505, bs:"猛将の叛攻令"},
	"3301":{n:"陳武", r:"L", c:2, a:400, i:18, d1:260, bs:"斧軍の神遠攻"},
	"3302":{n:"孫氏", r:"R", c:4, a:475, i:15, d1:470, bs:"錘兵の極攻"},
	"3303":{n:"大喬", r:"UR", c:2, a:300, i:10, d1:245, bs:"聖夜の神攻奪"},
	"3304":{n:"呉夫人・呉国太", r:"SR", c:4, a:495, i:18, d1:430, bs:"玩具の劫略令"},
	"3305":{n:"小喬", r:"R", c:3.5, a:410, i:14, d1:395, bs:"雪華の攻奪"},
	"3306":{n:"周姫", r:"L", c:4.5, a:530, i:27, d1:635, bs:"聖夜の天恵沢"},
	"3307":{n:"周姫", r:"UR", c:4, a:415, i:27, d1:465, bs:"聖夜の神恵沢"},
	"3308":{n:"周姫", r:"SR", c:4, a:360, i:27, d1:440, bs:"聖夜の聖恵沢"},
	"3309":{n:"周瑜", r:"R", c:4, a:350, i:17, d1:495, bs:"覇者の護り"},
	"3310":{n:"孫翊", r:"UC", c:4, a:385, i:3, d1:395, bs:"猛将の貫攻"},
	"3311":{n:"呂蒙", r:"UC", c:4, a:410, i:11, d1:405, bs:"隣地猛襲"},
	"3312":{n:"周姫・周瑜", r:"L", c:2, a:360, i:24, d1:355, bs:"剴切の鳳速令"},
	"3314":{n:"陸遜・潘璋", r:"L", c:4.5, a:750, i:27, d1:710, bs:"兵器大烈攻"},
	"3315":{n:"黄蓋", r:"SL", c:4.5, a:930, i:13, d1:770, bs:"斧兵の大帝撃"},
	"3316":{n:"歩夫人・大虎・小虎", r:"L", c:4.5, a:620, i:13, d1:680, bs:"全軍の鳳速令"},
	"3317":{n:"歩夫人", r:"L", c:4.5, a:530, i:11, d1:795, bs:"官女大転聖"},
	"3318":{n:"大虎", r:"L", c:4.5, a:570, i:15, d1:910, bs:"官女大転聖"},
	"3319":{n:"小虎", r:"L", c:4.5, a:560, i:14, d1:830, bs:"官女大転聖"},
	"3320":{n:"孫翊・徐氏", r:"L", c:2, a:290, i:10, d1:440, bs:"剴切の大神域"},
	"3321":{n:"陸抗", r:"SR", c:4, a:515, i:20, d1:420, bs:"鬼神の貫攻"},
	"3322":{n:"呉国太", r:"UR", c:2, a:235, i:16, d1:230, bs:"斧神の導き"},
	"3323":{n:"孫桓・朱然", r:"L", c:4.5, a:675, i:17, d1:1020, bs:"加護大聖壁"},
	"3324":{n:"蒋欽", r:"R", c:4, a:320, i:7, d1:550, bs:"加護堅壁"},
	"4001":{n:"呂布", r:"SR", c:4, a:500, i:3, d1:545, bs:"飛将"},
	"4002":{n:"董卓", r:"R", c:3, a:325, i:6, d1:300, bs:"剣兵の進撃"},
	"4003":{n:"袁紹", r:"R", c:2.5, a:220, i:10, d1:350, bs:"弓兵突撃"},
	"4004":{n:"公孫瓚",	r:"R", c:3, a:315, i:8, d1:345, bs:"騎兵突撃"},
	"4005":{n:"袁術", r:"R", c:2.5, a:235, i:8, d1:210, bs:"練兵訓練"},
	"4006":{n:"孟獲", r:"UC", c:2.5, a:270, i:4, d1:240, bs:"蛮族の襲撃"},
	"4007":{n:"孟獲", r:"C", c:2.5, a:255, i:4, d1:240, bs:"蛮族の襲撃"},
	"4008":{n:"劉焉", r:"UC", c:2, a:110, i:13, d1:215, bs:"兵器訓練"},
	"4009":{n:"劉焉", r:"C", c:2, a:110, i:13, d1:195, bs:"弓兵防御"},
	"4010":{n:"劉表", r:"UC", c:2, a:95, i:14, d1:175, bs:"石切知識"},
	"4011":{n:"劉表", r:"C", c:2, a:95, i:13, d1:175, bs:"石切知識"},
	"4012":{n:"孔融", r:"UC", c:2, a:35, i:14, d1:95, bs:"製鉄知識"},
	"4013":{n:"孔融", r:"C", c:2, a:35, i:13, d1:95, bs:"製鉄知識"},
	"4014":{n:"黄祖", r:"UC", c:1.5, a:105, i:3, d1:155, bs:"弓兵行軍"},
	"4015":{n:"黄祖", r:"C", c:1.5, a:105, i:3, d1:145, bs:"弓兵防御"},
	"4016":{n:"牛輔", r:"UC", c:1.5, a:120, i:2, d1:80, bs:"槍兵行軍"},
	"4017":{n:"牛輔", r:"C", c:1.5, a:110, i:2, d1:80, bs:"槍兵行軍"},
	"4018":{n:"郭汜", r:"UC", c:2, a:215, i:1, d1:185, bs:"騎兵行軍"},
	"4019":{n:"郭汜", r:"C", c:2, a:205, i:1, d1:180, bs:"剣兵の進撃"},
	"4020":{n:"祝融", r:"UC", c:2.5, a:275, i:3, d1:155, bs:"火神の攻勢"},
	"4021":{n:"祝融", r:"C", c:2.5, a:260, i:3, d1:155, bs:"火神の攻勢"},
	"4022":{n:"李傕", r:"UC", c:2, a:200, i:2, d1:205, bs:"騎兵の進撃"},
	"4023":{n:"李傕", r:"C", c:2, a:195, i:2, d1:195, bs:"騎兵防御"},
	"4024":{n:"貂蝉", r:"SR", c:1, a:10, i:10, d1:100, bs:"傾国"},
	"4025":{n:"張角", r:"SR", c:3.5, a:380, i:19, d1:135, bs:"太平要術"},
	"4026":{n:"袁紹", r:"R", c:2.5, a:220, i:10, d1:350, bs:"騎兵の聖域"},
	"4027":{n:"袁紹", r:"UC", c:2.5, a:205, i:9, d1:350, bs:"剣兵の強撃"},
	"4028":{n:"祝融", r:"R", c:1.5, a:175, i:3, d1:110, bs:"弓兵突覇"},
	"4029":{n:"孟獲", r:"R", c:3, a:335, i:5, d1:310, bs:"蛮王の襲撃"},
	"4030":{n:"蔡琰", r:"R", c:1, a:10, i:10, d1:125, bs:"攻城の檄文"},
	"4031":{n:"呂布", r:"UR", c:4, a:510, i:5, d1:545, bs:"飛将"},
	"4032":{n:"公孫瓚",	r:"SR", c:3, a:340, i:10, d1:350, bs:"騎兵の猛撃"},
	"4033":{n:"孟獲", r:"SR", c:3, a:365, i:2, d1:290, bs:"蛮王の襲撃"},
	"4034":{n:"董卓", r:"SR", c:3, a:350, i:8, d1:305, bs:"強襲突覇"},
	"4035":{n:"祝融", r:"SR", c:2.5, a:280, i:3, d1:190, bs:"弓兵の極撃"},
	"4036":{n:"劉焉", r:"R", c:2, a:125, i:14, d1:230, bs:"農林知識"},
	"4037":{n:"劉表", r:"R", c:2, a:110, i:14, d1:195, bs:"加工知識"},
	"4038":{n:"袁術", r:"UC", c:2, a:185, i:6, d1:190, bs:"練兵修練"},
	"4039":{n:"張角", r:"UR", c:3.5, a:380, i:22, d1:120, bs:"群雄の極撃"},
	"4040":{n:"貂蝉", r:"UR", c:1, a:10, i:10, d1:110, bs:"速撃の舞"},
	"4041":{n:"龐徳・馬超",	r:"UR", c:4, a:510, i:13, d1:575, bs:"鬼神の鹵獲"},
	"4042":{n:"于吉", r:"SR", c:2, a:200, i:17, d1:270, bs:"農林技術"},
	"4043":{n:"王異", r:"SR", c:2.5, a:290, i:9, d1:240, bs:"剣兵の極撃"},
	"4044":{n:"馬騰", r:"SR", c:3.5, a:410, i:10, d1:425, bs:"騎兵の極撃"},
	"4045":{n:"献帝", r:"UR", c:1, a:10, i:10, d1:130, bs:"富国強兵"},
	"4046":{n:"公孫瓚",	r:"UC", c:2.5, a:275, i:8, d1:265, bs:"騎兵強行"},
	"4047":{n:"顔良", r:"SR", c:3.5, a:435, i:5, d1:350, bs:"飛将"},
	"4048":{n:"文醜", r:"SR", c:3.5, a:425, i:5, d1:350, bs:"猛将の鹵獲"},
	"4049":{n:"蔡琰", r:"SR", c:1, a:10, i:10, d1:130, bs:"優雅な調べ"},
	"4050":{n:"袁紹", r:"SR", c:3.5, a:410, i:18, d1:520, bs:"軍神"},
	"4051":{n:"董卓", r:"UR", c:3.5, a:425, i:18, d1:520, bs:"暴虐外道"},
	"4052":{n:"張角", r:"R", c:2.5, a:180, i:17, d1:310, bs:"八卦の陣"},
	"4053":{n:"呂布", r:"UR", c:4, a:520, i:5, d1:545, bs:"神飛将"},
	"4054":{n:"劉表", r:"SR", c:2.5, a:180, i:18, d1:325, bs:"強兵の檄文"},
	"4055":{n:"公孫瓚",	r:"UR", c:3.5, a:420, i:11, d1:460, bs:"飛蹄進軍"},
	"4056":{n:"華雄", r:"SR", c:3.5, a:420, i:8, d1:390, bs:"槍兵の極撃"},
	"4057":{n:"紀霊", r:"R", c:2.5, a:270, i:6, d1:300, bs:"騎兵の猛撃"},
	"4058":{n:"祝融", r:"UR", c:3, a:360, i:4, d1:320, bs:"隣地猛攻"},
	"4059":{n:"陳宮", r:"R", c:2, a:240, i:14, d1:230, bs:"密偵召集"},
	"4060":{n:"袁紹", r:"UR", c:3.5, a:410, i:20, d1:630, bs:"群雄の防衛"},
	"4061":{n:"厳氏", r:"SR", c:1, a:20, i:8, d1:120, bs:"憂姫護国"},
	"4062":{n:"郭汜", r:"R", c:2.5, a:280, i:2, d1:270, bs:"強襲突撃"},
	"4063":{n:"張梁", r:"R", c:2.5, a:270, i:12, d1:310, bs:"弓兵の強撃"},
	"4064":{n:"張宝", r:"R", c:2.5, a:200, i:16, d1:230, bs:"兵器強行"},
	"4065":{n:"水鏡娘", r:"UR", c:1, a:20, i:10, d1:150, bs:"人選眼力"},
	"4066":{n:"貂蝉", r:"UR", c:1, a:10, i:10, d1:110, bs:"月下羽衣"},
	"4067":{n:"何皇后", r:"SR", c:1, a:15, i:8, d1:90, bs:"傾国"},
	"4068":{n:"呂布", r:"R", c:3.5, a:410, i:3, d1:445, bs:"万夫不当"},
	"4069":{n:"張任", r:"UC", c:2.5, a:280, i:10, d1:300, bs:"槍兵の強撃"},
	"4070":{n:"袁術", r:"SR", c:3, a:330, i:13, d1:350, bs:"虐帝の号令"},
	"4071":{n:"兀突骨", r:"UC", c:3, a:300, i:1, d1:355, bs:"守護防陣"},
	"4072":{n:"孟獲", r:"UR", c:3.5, a:450, i:5, d1:470, bs:"蛮勇の一撃"},
	"4073":{n:"何進", r:"R", c:2.5, a:255, i:2, d1:265, bs:"槍兵の強攻"},
	"4074":{n:"張宝", r:"UC", c:2.5, a:190, i:15, d1:220, bs:"趁火打劫"},
	"4075":{n:"高順", r:"SR", c:3.5, a:410, i:10, d1:450, bs:"陥陣営の侵攻"},
	"4076":{n:"董白", r:"SR", c:1.5, a:60, i:9, d1:60, bs:"暴姫の命"},
	"4077":{n:"顔良", r:"R", c:3, a:350, i:5, d1:280, bs:"剛将の畏怖"},
	"4078":{n:"王異", r:"R", c:2, a:150, i:7, d1:320, bs:"守護堅陣"},
	"4079":{n:"水鏡", r:"UR", c:1, a:50, i:17, d1:90, bs:"水鏡鑑定"},
	"4080":{n:"水鏡", r:"SR", c:1, a:30, i:15, d1:80, bs:"水鏡眼識"},
	"4081":{n:"水鏡", r:"R", c:1, a:15, i:12, d1:70, bs:"水鏡評論"},
	"4082":{n:"水鏡", r:"UC", c:1, a:10, i:10, d1:60, bs:"水鏡指南"},
	"4083":{n:"文醜", r:"R", c:3, a:340, i:5, d1:290, bs:"趁火打劫"},
	"4084":{n:"紀霊", r:"UC", c:2.5, a:270, i:5, d1:245, bs:"騎兵の進攻"},
	"4085":{n:"霊帝", r:"SR", c:1, a:10, i:5, d1:110, bs:"天子勅令"},
	"4086":{n:"華雄", r:"R", c:3, a:345, i:4, d1:330, bs:"槍兵の強攻"},
	"4087":{n:"呂布・貂蝉", r:"UR", c:4, a:540, i:18, d1:490, bs:"鬼神の極意"},
	"4088":{n:"董卓", r:"SR", c:3.5, a:380, i:21, d1:450, bs:"暴君の覇道"},
	"4089":{n:"蔡琰", r:"R", c:1, a:10, i:10, d1:110, bs:"才女の音律"},
	"4090":{n:"張角", r:"UC", c:3, a:150, i:16, d1:250, bs:"守護防陣"},
	"4091":{n:"呂氏", r:"SR", c:3.5, a:410, i:4, d1:345, bs:"弓神降臨"},
	"4092":{n:"馬騰", r:"R", c:3, a:320, i:9, d1:295, bs:"騎兵の強攻"},
	"4093":{n:"陳宮", r:"UC", c:2, a:230, i:13, d1:190, bs:"兵器突撃"},
	"4094":{n:"鄒氏", r:"R", c:1, a:15, i:7, d1:75, bs:"桃色吐息"},
	"4095":{n:"水鏡娘(自分用)", r:"UR", c:1, a:50, i:17, d1:90, bs:"娘々祈祷"},
	"4096":{n:"水鏡娘(自分用)", r:"SR", c:1, a:30, i:15, d1:80, bs:"娘々祈祷"},
	"4097":{n:"水鏡娘(自分用)", r:"R", c:1, a:15, i:12, d1:70, bs:"娘々祈祷"},
	"4098":{n:"水鏡娘(自分用)", r:"UC", c:1, a:10, i:10, d1:60, bs:"娘々祈祷"},
	"4099":{n:"水鏡娘(自分用)", r:"UR", c:1, a:50, i:17, d1:90, bs:"娘々訓練"},
	"4100":{n:"水鏡娘(自分用)", r:"SR", c:1, a:30, i:15, d1:80, bs:"娘々訓練"},
	"4101":{n:"水鏡娘(自分用)", r:"R", c:1, a:15, i:12, d1:70, bs:"娘々訓練"},
	"4102":{n:"水鏡娘(自分用)", r:"UC", c:1, a:10, i:10, d1:60, bs:"娘々訓練"},
	"4103":{n:"于吉", r:"R", c:1.5, a:130, i:13, d1:100, bs:"豊潤祈祷"},
	"4104":{n:"郭汜", r:"UC", c:1.5, a:200, i:1, d1:175, bs:"趁火打劫"},
	"4105":{n:"馬超", r:"UR", c:4, a:510, i:5, d1:440, bs:"騎兵の大極撃"},
	"4106":{n:"李傕", r:"R", c:2.5, a:280, i:3, d1:240, bs:"騎兵速攻"},
	"4107":{n:"黄祖", r:"R", c:1.5, a:200, i:3, d1:195, bs:"密偵精鋭"},
	"4108":{n:"貂蝉", r:"R", c:1.5, a:10, i:9, d1:90, bs:"桃色吐息"},
	"4109":{n:"張任", r:"R", c:2.5, a:270, i:10, d1:390, bs:"防将戦法"},
	"4110":{n:"公孫瓚",	r:"SR", c:3, a:360, i:9, d1:430, bs:"白馬将"},
	"4111":{n:"袁紹", r:"UR", c:3.5, a:425, i:20, d1:545, bs:"群将の督戦"},
	"4112":{n:"高順", r:"R", c:3, a:330, i:10, d1:340, bs:"兵器突撃"},
	"4113":{n:"何進", r:"UC", c:2.5, a:245, i:2, d1:200, bs:"剣兵の進攻"},
	"4114":{n:"祝融", r:"R", c:3, a:330, i:3, d1:300, bs:"剣兵の強攻"},
	"4115":{n:"郭汜", r:"SR", c:1, a:190, i:4, d1:125, bs:"天里行"},
	"4116":{n:"董卓", r:"UR", c:3.5, a:435, i:19, d1:500, bs:"剣兵の極攻"},
	"4117":{n:"花鬘", r:"SR", c:3.5, a:410, i:6, d1:450, bs:"剣神降臨"},
	"4118":{n:"何皇后", r:"R", c:1, a:10, i:8, d1:110, bs:"護国献心"},
	"4119":{n:"呂氏", r:"UC", c:3, a:290, i:3, d1:320, bs:"騎兵速攻"},
	"4120":{n:"兀突骨", r:"UR", c:4, a:440, i:4, d1:680, bs:"覇者の護り"},
	"4121":{n:"田豊", r:"SR", c:4, a:380, i:22, d1:710, bs:"剣兵の大聖域"},
	"4122":{n:"貂蝉", r:"UR", c:1, a:10, i:8, d1:110, bs:"素材知識"},
	"4123":{n:"孟獲", r:"R", c:3.5, a:395, i:4, d1:550, bs:"王者の護り"},
	"4124":{n:"パンダ(仮)", r:"UR", c:1.5, a:85, i:13, d1:160, bs:"熊猫の麺匠"},
	"4125":{n:"張梁", r:"UC", c:3, a:350, i:11, d1:310, bs:"弓兵突覇"},
	"4126":{n:"パンダ(仮)", r:"R", c:1.5, a:100, i:10, d1:100, bs:"熊猫の猛襲"},
	"4127":{n:"左慈娘・鶴爺", r:"SR", c:3.5, a:380, i:18, d1:400, bs:"娘々武神"},
	"4128":{n:"公孫瓚", r:"R", c:3.5, a:390, i:9, d1:375, bs:"千里雷光"},
	"4129":{n:"孟優", r:"UR", c:4, a:520, i:10, d1:550, bs:"剣兵烈攻"},
	"4130":{n:"文醜", r:"UC", c:3, a:320, i:4, d1:315, bs:"神速"},
	"4131":{n:"韓遂", r:"SR", c:2.5, a:280, i:11, d1:290, bs:"騎兵突攻"},
	"4132":{n:"孟優", r:"R", c:2.5, a:285, i:8, d1:250, bs:"剣兵速攻"},
	"4133":{n:"関羽", r:"UR", c:3.5, a:285, i:8, d1:250, bs:"一騎湯戦"},
	"4134":{n:"呂布・貂蝉", r:"UR", c:3.5, a:395, i:15, d1:360, bs:"湯将の極意"},
	"4135":{n:"呂布", r:"R", c:3.5, a:410, i:3, d1:445, bs:"武将の極意"},
	"4136":{n:"水鏡娘(自分用)", r:"C", c:1, a:10, i:10, d1:50, bs:"娘々祈祷"},
	"4137":{n:"劉璋", r:"SR", c:2.5, a:260, i:14, d1:340, bs:"守護聖陣"},
	"4138":{n:"李傕", r:"SR", c:3.5, a:360, i:4, d1:575, bs:"守護方陣"},
	"4139":{n:"文醜・顔良", r:"UR", c:4, a:570, i:7, d1:390, bs:"鬼神の極意"},
	"4140":{n:"水鏡娘", r:"UR", c:3, a:335, i:13, d1:355, bs:"烈速雷光"},
	"4141":{n:"陳宮・高順", r:"UR", c:4, a:420, i:26, d1:520, bs:"兵器の猛攻"},
	"4142":{n:"劉表", r:"SR", c:1.5, a:100, i:17, d1:255, bs:"兵器の大号令"},
	"4143":{n:"董白", r:"R", c:1, a:30, i:8, d1:60, bs:"暴姫の威令"},
	"4144":{n:"華雄", r:"R", c:3.5, a:420, i:8, d1:390, bs:"猛将の極意"},
	"4145":{n:"花鬘", r:"R", c:3.5, a:410, i:6, d1:430, bs:"飛将"},
	"4146":{n:"袁紹", r:"UC", c:3, a:150, i:19, d1:350, bs:"奇計百出"},
	"4147":{n:"劉璋", r:"UR", c:3.5, a:410, i:20, d1:600, bs:"護君"},
	"4148":{n:"牛輔", r:"SR", c:3, a:330, i:10, d1:385, bs:"兵器の強攻"},
	"4149":{n:"馬騰", r:"UC", c:2.5, a:250, i:8, d1:350, bs:"騎兵方陣"},
	"4150":{n:"馬超", r:"SR", c:3.5, a:430, i:6, d1:420, bs:"若獅子の軍略"},
	"4151":{n:"韓遂", r:"UC", c:2.5, a:270, i:10, d1:265, bs:"謀反の進攻"},
	"4152":{n:"水鏡娘(自分用)", r:"C", c:1, a:10, i:10, d1:50, bs:"娘々訓練"},
	"4153":{n:"水鏡家族", r:"UR", c:2, a:90, i:18, d1:190, bs:"水鏡の方術"},
	"4154":{n:"水鏡娘", r:"UR", c:1, a:20, i:10, d1:150, bs:"娘々強兵"},
	"4155":{n:"パンダ（仮）", r:"SR", c:3.5, a:390, i:13, d1:380, bs:"龍鱗猛攻"},
	"4156":{n:"鄒氏", r:"SR", c:1, a:15, i:8, d1:85, bs:"酔吟吐息"},
	"4157":{n:"貂蝉", r:"R", c:1, a:10, i:9, d1:90, bs:"傾国"},
	"4158":{n:"呂氏", r:"R", c:3.5, a:370, i:4, d1:350, bs:"闘将の極意"},
	"4159":{n:"お江", r:"UR", c:1, a:10, i:10, d1:100, bs:"傾国"},
	"4160":{n:"劉焉", r:"SR", c:2.5, a:230, i:19, d1:250, bs:"兵器の勝鬨"},
	"4161":{n:"水鏡娘", r:"UR", c:3, a:325, i:10, d1:250, bs:"娘々軍神"},
	"4162":{n:"水鏡娘", r:"UR", c:2, a:240, i:15, d1:200, bs:"群雄の極撃"},
	"4163":{n:"献帝", r:"SR", c:3, a:100, i:23, d1:100, bs:"陳留王政"},
	"4164":{n:"祝融", r:"R", c:3.5, a:200, i:4, d1:670, bs:"金剛不壊"},
	"4165":{n:"張遼", r:"UR", c:4, a:530, i:15, d1:540, bs:"戦神の極意"},
	"4166":{n:"のぶニャが", r:"UR", c:3, a:330, i:10, d1:300, bs:"万夫必殺"},
	"4167":{n:"織田信長夢", r:"UR", c:3.5, a:425, i:6, d1:455, bs:"一閃の極意"},
	"4168":{n:"袁譚・袁尚", r:"UR", c:4, a:570, i:21, d1:550, bs:"剣兵の大神撃"},
	"4169":{n:"鄒氏", r:"SR", c:1.5, a:15, i:8, d1:80, bs:"哀艶の舞"},
	"4170":{n:"孟優", r:"SR", c:4, a:470, i:9, d1:505, bs:"剣兵の大極撃"},
	"4171":{n:"王悦", r:"R", c:3.5, a:395, i:6, d1:400, bs:"槍神降臨"},
	"4172":{n:"韓遂", r:"R", c:3.5, a:410, i:10, d1:420, bs:"猛士の極意"},
	"4173":{n:"董白", r:"UC", c:3, a:295, i:7, d1:360, bs:"剣兵速攻"},
	"4174":{n:"貂蝉", r:"SR", c:1.5, a:10, i:13, d1:150, bs:"月華の舞"},
	"4175":{n:"呂氏", r:"R", c:3, a:310, i:4, d1:340, bs:"暴姫の命"},
	"4176":{n:"沮授", r:"UR", c:3.5, a:350, i:27, d1:495, bs:"攻城戦の極砕"},
	"4177":{n:"織田信長", r:"UR", c:3.5, a:405, i:8, d1:430, bs:"天下布武"},
//	"4178":{n:"上杉謙信", r:"SR", c:3.5, a:0, i:14, d1:425, bs:"一騎当千"},
//	"4179":{n:"武田信玄", r:"R", c:3, a:0, i:12, d1:355, bs:"騎兵の極撃"},
	"4180":{n:"真田幸村", r:"R", c:3, a:325, i:8, d1:375, bs:"六文銭の雷光"},
	"4181":{n:"王美人", r:"UR", c:1.5, a:50, i:15, d1:210, bs:"佳人薄命"},
	"4182":{n:"蔡琰", r:"SR", c:1, a:100, i:12, d1:120, bs:"文姫の慈愛"},
	"4183":{n:"鄒氏", r:"R", c:3.5, a:425, i:10, d1:420, bs:"猛暑の極意"},
	"4184":{n:"水鏡娘", r:"SR", c:3.5, a:405, i:20, d1:505, bs:"鹵獲の猛攻"},
	"4186":{n:"胡車児", r:"UR", c:2, a:280, i:8, d1:205, bs:"強襲奮迅"},
	"4187":{n:"水鏡娘", r:"SR", c:3.5, a:395, i:18, d1:395, bs:"猛将の鹵獲"},
	"4191":{n:"李傕・郭汜", r:"UR", c:4, a:515, i:6, d1:445, bs:"鬼神の鹵獲"},
	"4192":{n:"木鹿大王", r:"UR", c:4, a:525, i:10, d1:510, bs:"飛将"},
	"4193":{n:"張宝", r:"SR", c:3.5, a:405, i:22, d1:405, bs:"兵器の猛攻"},
	"4194":{n:"李儒", r:"SR", c:2.5, a:220, i:18, d1:250, bs:"大剣兵移送"},
	"4195":{n:"左慈娘(自分用)", r:"UR", c:1, a:50, i:17, d1:90, bs:"娘々練磨"},
	"4196":{n:"左慈娘(自分用)", r:"SR", c:1, a:30, i:15, d1:80, bs:"娘々練磨"},
	"4197":{n:"左慈娘(自分用)", r:"R", c:1, a:15, i:12, d1:70, bs:"娘々練磨"},
	"4198":{n:"左慈娘(自分用)", r:"UC", c:1, a:10, i:10, d1:60, bs:"娘々練磨"},
	"4199":{n:"左慈娘(自分用)", r:"C", c:1, a:10, i:10, d1:50, bs:"娘々練磨"},
	"4200":{n:"貂蝉", r:"SR", c:1, a:10, i:8, d1:100, bs:"素材知識"},
	"4201":{n:"貂蝉", r:"R", c:1, a:10, i:8, d1:90, bs:"素材知識"},
	"4202":{n:"祝融", r:"L", c:4, a:650, i:6, d1:740, bs:"剣兵の大天撃"},
	"4203":{n:"霊帝", r:"R", c:1, a:10, i:5, d1:100, bs:"富国強兵"},
	"4204":{n:"水鏡娘", r:"L", c:4, a:580, i:22, d1:420, bs:"鹵獲の極攻"},
	"4205":{n:"董白", r:"UR", c:4, a:510, i:11, d1:450, bs:"飛速迅雷"},
	"4206":{n:"劉璋", r:"SR", c:1, a:80, i:10, d1:230, bs:"守衛聖陣"},
	"4207":{n:"董白", r:"SR", c:3.5, a:420, i:10, d1:425, bs:"収奪の猛攻"},
	"4208":{n:"劉璋", r:"R", c:1, a:50, i:9, d1:135, bs:"領地統制"},
	"4209":{n:"審配", r:"SR", c:4, a:410, i:22, d1:465, bs:"全軍の猛攻令"},
	"4210":{n:"呂布", r:"L", c:4, a:700, i:6, d1:640, bs:"無双"},
	"4211":{n:"劉焉", r:"UR", c:1, a:130, i:11, d1:160, bs:"皇叔の号令"},
	"4212":{n:"皇甫嵩", r:"UR", c:4, a:520, i:23, d1:505, bs:"全軍の極攻令"},
	"4213":{n:"李儒", r:"UR", c:4, a:370, i:22, d1:720, bs:"剣兵の大神域"},
	"4214":{n:"王悦", r:"UR", c:3.5, a:415, i:8, d1:460, bs:"王将の心意気"},
	"4215":{n:"盧氏", r:"UR", c:4, a:385, i:25, d1:485, bs:"食糧革命"},
	"4216":{n:"王美人", r:"SR", c:1, a:20, i:10, d1:150, bs:"食糧技巧"},
	"4217":{n:"呂氏", r:"R", c:3.5, a:400, i:4, d1:395, bs:"神飛将"},
	"4218":{n:"献帝", r:"R", c:2.5, a:80, i:17, d1:80, bs:"富国"},
	"4219":{n:"龐徳", r:"UR", c:4, a:515, i:15, d1:555, bs:"飛将"},
	"4220":{n:"張角", r:"L", c:4, a:500, i:24, d1:520, bs:"守護大聖陣"},
	"4221":{n:"袁紹", r:"L", c:4, a:660, i:22, d1:710, bs:"剣兵の大極攻"},
	"4222":{n:"趙雲", r:"UR", c:3.5, a:395, i:15, d1:410, bs:"軍神"},
	"4227":{n:"水鏡(自分用)", r:"L", c:1, a:80, i:19, d1:100, bs:"水鏡透察"},
	"4228":{n:"水鏡(自分用)", r:"UR", c:1, a:50, i:17, d1:90, bs:"水鏡鑑定"},
	"4229":{n:"水鏡(自分用)", r:"SR", c:1, a:30, i:15, d1:80, bs:"水鏡眼識"},
	"4230":{n:"水鏡(自分用)", r:"R", c:1, a:15, i:12, d1:70, bs:"水鏡評論"},
	"4231":{n:"水鏡(自分用)", r:"UC", c:1, a:10, i:10, d1:60, bs:"水鏡指南"},
	"4232":{n:"水鏡", r:"L", c:1, a:80, i:19, d1:100, bs:"水鏡透察"},
	"4233":{n:"左慈娘", r:"L", c:4, a:490, i:22, d1:830, bs:"覇者の大神域"},
	"4234":{n:"袁姫", r:"UC", c:2, a:20, i:11, d1:300, bs:"護国望心"},
	"4235":{n:"厳白虎", r:"R", c:3, a:355, i:5, d1:335, bs:"謀反の猛攻"},
	"4236":{n:"馬鉄", r:"SR", c:2.5, a:315, i:9, d1:315, bs:"謀反の突攻"},
	"4237":{n:"王異", r:"R", c:2, a:270, i:7, d1:275, bs:"謀反の速攻"},
	"4238":{n:"張郃・高覧", r:"UR", c:4, a:550, i:16, d1:375, bs:"飛速迅雷"},
	"4239":{n:"盧植", r:"UR", c:4, a:380, i:24, d1:715, bs:"剣将の封聖域"},
	"4240":{n:"于吉娘",  r:"UR", c:1, a:50, i:10, d1:140, bs:"娘々聖水"},
	"4241":{n:"王允", r:"UR", c:4, a:400, i:26, d1:505, bs:"全軍の飛速令"},
	"4242":{n:"張松", r:"SR", c:4, a:370, i:24, d1:555, bs:"兵器の大極撃"},
	"4243":{n:"劉曼", r:"SR", c:4, a:350, i:19, d1:710, bs:"守衛方陣"},
	"4244":{n:"孟獲", r:"L", c:4, a:470, i:6, d1:770, bs:"剣兵の大天域"},
	"4245":{n:"波才", r:"UC", c:3, a:315, i:6, d1:305, bs:"鹵獲の進攻"},
	"4246":{n:"馬元義", r:"R", c:3.5, a:425, i:14, d1:415, bs:"鹵獲の強攻"},
	"4247":{n:"張任", r:"UR", c:4, a:495, i:15, d1:505, bs:"飛隠雷光"},
	"4248":{n:"張繍", r:"SR", c:3.5, a:420, i:13, d1:385, bs:"烈隠雷光"},
	"4249":{n:"張角", r:"UR", c:3.5, a:350, i:20, d1:320, bs:"守護堅陣"},
	"4250":{n:"ルー", r:"UR", c:2, a:100, i:12, d1:250, bs:"神官の加護"},
	"4251":{n:"メアリ・ドレーク", r:"SR", c:3.5, a:340, i:13, d1:350, bs:"覇王の強撃"},
	"4252":{n:"ギルガメッシュ", r:"R", c:3, a:300, i:8, d1:390, bs:"エクスカリパー"},
	"4253":{n:"董卓", r:"L", c:4, a:590, i:23, d1:630, bs:"龍神の縮地劫略"},
	"4254":{n:"何進", r:"UR", c:4, a:495, i:9, d1:530, bs:"鬼神の縮地劫略"},
	"4255":{n:"袁紹", r:"SR", c:3.5, a:405, i:18, d1:365, bs:"猛将の縮地劫略"},
	"4256":{n:"左慈娘・鶴爺", r:"UR", c:3.5, a:420, i:19, d1:430, bs:"娘々武神"},
	"4257":{n:"于吉娘", r:"L", c:4, a:495, i:22, d1:830, bs:"天衣無縫"},
	"4258":{n:"左慈娘", r:"UR", c:4, a:515, i:21, d1:420, bs:"飛速迅雷"},
	"4259":{n:"花鬘", r:"SR", c:4, a:505, i:7, d1:440, bs:"夏神の極意"},
	"4260":{n:"盧氏", r:"R", c:3, a:255, i:18, d1:380, bs:"千華招来"},
	"4261":{n:"左慈(自分用)", r:"C", c:1, a:10, i:10, d1:50, bs:"爺々回帰"},
	"4262":{n:"左慈(自分用)", r:"C", c:1, a:10, i:10, d1:50, bs:"爺々継承"},
	"4263":{n:"龐徳", r:"R", c:3.5, a:405, i:13, d1:400, bs:"騎兵突攻"},
	"4264":{n:"馬騰・馬超", r:"L", c:4.5, a:800, i:15, d1:630, bs:"隣地極攻"},
	"4265":{n:"劉曼", r:"UR", c:1, a:100, i:8, d1:175, bs:"戦神招来"},
	"4266":{n:"郭図", r:"SR", c:4, a:530, i:23, d1:480, bs:"弓兵の大神撃"},
	"4267":{n:"徐栄", r:"R", c:4, a:435, i:10, d1:440, bs:"槍兵の極攻"},
	"4268":{n:"厳氏", r:"R", c:1, a:10, i:7, d1:110, bs:"剣兵増勢"},
	"4269":{n:"袁姫", r:"R", c:1, a:30, i:7, d1:115, bs:"食糧技巧"},
	"4270":{n:"馬鉄", r:"R", c:3.5, a:400, i:8, d1:395, bs:"猛将の鹵獲"},
	"4272":{n:"田豊", r:"UR", c:4, a:410, i:24, d1:740, bs:"智将の大神撃"},
	"4273":{n:"李傕", r:"UR", c:4, a:505, i:6, d1:410, bs:"万里雷光"},
	"4274":{n:"郭汜", r:"SR", c:3.5, a:420, i:5, d1:405, bs:"戦蹄轟撃"},
	"4275":{n:"牛輔", r:"R", c:3, a:300, i:9, d1:300, bs:"強襲速撃"},
	"4276":{n:"華雄", r:"UC", c:3, a:320, i:4, d1:315, bs:"精鋭の進撃"},
	"4277":{n:"李儒", r:"UC", c:2.5, a:220, i:14, d1:370, bs:"剣兵方陣"},
	"4278":{n:"董卓", r:"UC", c:2.5, a:275, i:5, d1:395, bs:"剣兵突撃"},
	"4285":{n:"孫策伯符", r:"UR", c:3.5, a:420, i:7, d1:500, bs:"武神"},
	"4286":{n:"左慈娘", r:"UR", c:4, a:510, i:21, d1:505, bs:"鬼神の鹵獲"},
	"4288":{n:"織田信長", r:"SR", c:3.5, a:365, i:7, d1:420, bs:"天下布武"},
	"4289":{n:"鄒氏", r:"UR", c:1, a:20, i:9, d1:105, bs:"白菊の祈り"},
	"4290":{n:"陶謙", r:"R", c:3, a:265, i:18, d1:385, bs:"智将器撃"},
	"4311":{n:"王異", r:"SR", c:4, a:475, i:10, d1:490, bs:"剣兵の猛攻"},
	"4312":{n:"管輅", r:"SR", c:2, a:70, i:18, d1:165, bs:"神卜の方術"},
	"4313":{n:"于吉娘", r:"SR", c:2, a:50, i:13, d1:150, bs:"娘々敬慕"},
	"4314":{n:"韓遂", r:"UR", c:4, a:510, i:14, d1:420, bs:"鬼神の叛乱"},
	"4315":{n:"沮授", r:"R", c:4, a:245, i:24, d1:590, bs:"剣兵の聖域"},
	"4316":{n:"于吉娘", r:"UR", c:4, a:350, i:21, d1:555, bs:"天華招来"},
	"4317":{n:"貂蝉", r:"L", c:1, a:50, i:11, d1:180, bs:"守城真心"},
	"4318":{n:"左慈娘", r:"UR", c:4, a:370, i:21, d1:730, bs:"娘々護君"},
	"4320":{n:"アトランティス", r:"SR", c:3, a:340, i:16, d1:350, bs:"槍神降臨"},
	"4321":{n:"ダイアナ", r:"R", c:3, a:305, i:16, d1:300, bs:"一騎当千"},
	"4322":{n:"献帝", r:"L", c:4.5, a:500, i:21, d1:610, bs:"全軍の大極攻令"},
	"4323":{n:"高順", r:"UR", c:4, a:480, i:12, d1:495, bs:"兵器烈攻"},
	"4324":{n:"審配", r:"R", c:3, a:290, i:18, d1:365, bs:"兵器速攻"},
	"4325":{n:"李儒", r:"R", c:3.5, a:295, i:19, d1:405, bs:"知略徴発"},
	"4326":{n:"公孫淵", r:"UR", c:4, a:475, i:15, d1:460, bs:"謀反の烈攻"},
	"4327":{n:"管輅", r:"R", c:1, a:50, i:10, d1:140, bs:"神卜の術式"},
	"4337":{n:"曹操・袁紹", r:"UR", c:4, a:495, i:22, d1:520, bs:"全軍の飛速令"},
	"4338":{n:"公孫瓚", r:"L", c:4, a:625, i:13, d1:580, bs:"騎兵大烈攻"},
	"4339":{n:"兀突骨", r:"SR", c:4, a:410, i:3, d1:670, bs:"猛将の守勢"},
	"4340":{n:"馬騰", r:"UR", c:4, a:510, i:12, d1:485, bs:"鬼神の貫攻"},
	"4341":{n:"胡車児", r:"SR", c:3.5, a:425, i:10, d1:415, bs:"猛将の貫攻"},
	"4342":{n:"厳白虎", r:"SR", c:3.5, a:360, i:7, d1:610, bs:"猛将の守勢"},
	"4343":{n:"張繍", r:"R", c:3, a:320, i:11, d1:425, bs:"闘将の守勢"},
	"4344":{n:"顔良", r:"UC", c:2.5, a:270, i:4, d1:270, bs:"武将の極意"},
	"4345":{n:"何皇后", r:"UR", c:1, a:25, i:9, d1:105, bs:"収奪招来"},
	"4346":{n:"郭汜", r:"R", c:1.5, a:210, i:2, d1:180, bs:"千里行"},
	"4347":{n:"于吉娘・亀", r:"SR", c:3.5, a:360, i:18, d1:475, bs:"千里雷光"},
	"4348":{n:"于吉娘・亀", r:"UR", c:1, a:50, i:10, d1:90, bs:"感謝の聖恵沢"},
	"4349":{n:"孟優",  r:"UC", c:2.5, a:270, i:7, d1:235, bs:"武将の貫攻"},
	"4350":{n:"厳氏",  r:"SR", c:3.5, a:290, i:13, d1:520, bs:"万華招来"},
	"4351":{n:"于吉娘",  r:"L", c:4.5, a:635, i:23, d1:685, bs:"市場繁栄"},
	"4352":{n:"于吉娘", r:"SR", c:2.5, a:120, i:14, d1:220, bs:"千華招来"},
	"4353":{n:"劉璋",  r:"R", c:3, a:265, i:14, d1:430, bs:"援軍の聖域"},
	"4354":{n:"董卓・呂布",  r:"L", c:4.5, a:800, i:11, d1:690, bs:"龍神突貫"},
	"4356":{n:"夏侯惇", r:"UR", c:3.5, a:420, i:12, d1:440, bs:"騎兵の極撃"},
	"4357":{n:"おミーちの方", r:"SR", c:2, a:220, i:17, d1:250, bs:"恵風"},
	"4358":{n:"さニャだ幸村",  r:"R", c:3, a:330, i:8, d1:320, bs:"一騎当千"},
	"4359":{n:"花鬘",  r:"UR", c:4, a:510, i:8, d1:550, bs:"鬼神"},
	"4360":{n:"パンダ（仮）",  r:"L", c:2, a:270, i:15, d1:300, bs:"南瓜の天恵沢"},
	"4361":{n:"于吉娘",  r:"L", c:4, a:620, i:22, d1:600, bs:"お菓子の攻奪"},
	"4362":{n:"黄祖",  r:"UR", c:4, a:550, i:10, d1:500, bs:"隣地豪撃"},
	"4363":{n:"パンダ（仮）", r:"UR", c:2, a:220, i:14, d1:250, bs:"南瓜の神恵沢"},
	"4365":{n:"南華老仙(自分用)", r:"UC", c:1, a:10, i:10, d1:60, bs:"南華指南"},
	"4366":{n:"南華老仙(自分用)", r:"R", c:1, a:15, i:12, d1:70, bs:"南華評論"},
	"4367":{n:"南華老仙(自分用)", r:"SR", c:1, a:30, i:15, d1:80, bs:"南華眼識"},
	"4368":{n:"南華老仙(自分用)", r:"UR", c:1, a:50, i:17, d1:90, bs:"南華鑑定"},
	"4369":{n:"南華老仙(自分用)", r:"L", c:1, a:80, i:19, d1:100, bs:"南華透察"},
	"4376":{n:"劉曼", r:"R", c:1, a:20, i:7, d1:110, bs:"暁風の恵み"},
	"4377":{n:"何進", r:"SR", c:4, a:490, i:8, d1:500, bs:"鬼神の貫攻"},
	"4378":{n:"何進", r:"L", c:4, a:660, i:10, d1:675, bs:"剣兵の大天撃"},
	"4379":{n:"董卓", r:"L", c:4, a:625, i:23, d1:640, bs:"龍神の攻奪"},
	"4380":{n:"董卓", r:"UR", c:4, a:510, i:22, d1:510, bs:"収奪の極攻"},
	"4381":{n:"水鏡娘・左慈娘", r:"L", c:4.5, a:530, i:24, d1:675, bs:"全軍の大極攻令"},
	"4382":{n:"呂布", r:"L", c:4.5, a:810, i:7, d1:700, bs:"隣地極攻"},
	"4383":{n:"兀突骨", r:"UR", c:4, a:575, i:4, d1:530, bs:"剣兵の大神撃"},
	"4384":{n:"郭汜", r:"UR", c:4, a:535, i:7, d1:445, bs:"鹵獲の極攻"},
	"4385":{n:"朱儁", r:"SR", c:4, a:520, i:14, d1:455, bs:"槍兵烈攻"},
	"4386":{n:"木鹿大王", r:"R", c:4, a:430, i:8, d1:480, bs:"鹵獲の猛攻"},
	"4387":{n:"馬鉄", r:"R", c:2, a:275, i:7, d1:225, bs:"飛将"},
	"4388":{n:"劉曼", r:"L", c:4.5, a:480, i:21, d1:730, bs:"食糧天革命"},
	"4389":{n:"劉曼", r:"UR", c:4, a:370, i:20, d1:525, bs:"食糧革命"},
	"4390":{n:"呂氏", r:"UR", c:4, a:460, i:6, d1:575, bs:"兵器の大神撃"},
	"4394":{n:"南華娘(自分用)", r:"UR", c:1, a:50, i:17, d1:90, bs:"娘々祈念"},
	"4395":{n:"南華娘(自分用)", r:"SR", c:1, a:30, i:15, d1:80, bs:"娘々祈念"},
	"4396":{n:"南華娘(自分用)", r:"R", c:1, a:15, i:12, d1:70, bs:"娘々祈念"},
	"4397":{n:"南華娘(自分用)", r:"UC", c:1, a:10, i:10, d1:60, bs:"娘々祈念"},
	"4398":{n:"南華娘(自分用)", r:"C", c:1, a:10, i:10, d1:50, bs:"娘々祈念"},
	"4405":{n:"孟優", r:"UR", c:4, a:490, i:10, d1:440, bs:"猛将の攻奪"},
	"4406":{n:"袁術", r:"L", c:4, a:620, i:17, d1:555, bs:"全軍の大極攻令"},
	"4407":{n:"韓遂", r:"L", c:4, a:605, i:15, d1:540, bs:"謀反の大烈攻"},
	"4408":{n:"王美人", r:"R", c:1, a:15, i:7, d1:85, bs:"玄冬の祝い"},
	"4409":{n:"南華娘", r:"L", c:4.5, a:485, i:24, d1:720, bs:"全兵の大極攻令"},
	"4410":{n:"鄒氏", r:"L", c:4, a:460, i:14, d1:650, bs:"緊急呼集"},
	"4411":{n:"胡車児", r:"L", c:4, a:620, i:13, d1:570, bs:"鳳隠雷光"},
	"4412":{n:"文醜", r:"L", c:4, a:610, i:8, d1:545, bs:"龍神の攻奪"},
	"4413":{n:"張角娘・虎", r:"L", c:4, a:700, i:22, d1:600, bs:"龍神の貫攻"},
	"4414":{n:"張角娘・虎", r:"UR", c:4, a:510, i:21, d1:500, bs:"鬼神の貫攻"},
	"4415":{n:"張角娘・虎", r:"SR", c:3.5, a:410, i:19, d1:385, bs:"猛将の貫攻"},
	"4416":{n:"水鏡娘・パンダ", r:"L", c:4, a:500, i:21, d1:800, bs:"守衛大聖陣"},
	"4417":{n:"左慈娘・鶴爺", r:"L", c:4, a:620, i:21, d1:580, bs:"鳳速雷光"},
	"4418":{n:"于吉娘・亀", r:"L", c:4, a:490, i:21, d1:820, bs:"龍神の守勢"},
	"4419":{n:"張角娘", r:"L", c:4.5, a:510, i:22, d1:990, bs:"娘々無縫"},
	"4420":{n:"水鏡娘", r:"L", c:4.5, a:580, i:21, d1:900, bs:"娘々天恵沢"},
	"4421":{n:"南華娘", r:"L", c:4.5, a:650, i:24, d1:680, bs:"娘々大神砕"},
	"4422":{n:"袁譚", r:"L", c:2, a:380, i:15, d1:320, bs:"剴切の天叛攻"},
	"4423":{n:"袁尚", r:"UR", c:2, a:300, i:14, d1:220, bs:"剴切の神叛攻"},
	"4424":{n:"王異", r:"L", c:4.5, a:555, i:13, d1:910, bs:"守護大聖陣"},
	"4425":{n:"張角娘", r:"L", c:4.5, a:720, i:22, d1:700, bs:"兵器の娘天撃"},
	"4426":{n:"南華娘・于吉娘", r:"L", c:4.5, a:755, i:24, d1:660, bs:"娘々龍神"},
	"4427":{n:"左慈娘", r:"L", c:4.5, a:740, i:22, d1:600, bs:"娘々無双"},
	"4428":{n:"于吉娘", r:"SR", c:1, a:45, i:9, d1:130, bs:"娘々聖水"},
	"4429":{n:"南華娘", r:"UR", c:4, a:390, i:21, d1:750, bs:"娘々護君"},
	"4430":{n:"顔良", r:"L", c:4, a:600, i:8, d1:535, bs:"龍神の攻奪"},
	"4431":{n:"華雄", r:"SR", c:4, a:475, i:9, d1:555, bs:"鹵獲の極攻"},
	"4432":{n:"袁紹", r:"R", c:4, a:435, i:10, d1:450, bs:"全軍の猛攻令"},
	"4433":{n:"淳于瓊", r:"UR", c:4, a:555, i:11, d1:495, bs:"戦神の貫攻"},
	"4434":{n:"黄祖", r:"SR", c:4, a:400, i:7, d1:570, bs:"猛士の守勢"},
	"4439":{n:"水鏡娘", r:"L", c:4, a:490, i:22, d1:600, bs:"全軍の大極攻令"},
	"4440":{n:"張任", r:"SR", c:4, a:420, i:14, d1:480, bs:"護君"},
	"4441":{n:"孟獲・祝融", r:"UR", c:4, a:540, i:5, d1:405, bs:"略取の極攻"},
	"4442":{n:"兀突骨", r:"L", c:4, a:700, i:5, d1:585, bs:"隣地天豪襲"},
	"4443":{n:"木鹿大王", r:"SR", c:4, a:505, i:9, d1:495, bs:"隣地豪襲"},
	"4444":{n:"徐栄", r:"UC", c:3, a:255, i:7, d1:385, bs:"闘将の守勢"},
	"4445":{n:"何皇后", r:"L", c:4, a:540, i:11, d1:550, bs:"副将の天明鏡"},
	"4446":{n:"盧氏", r:"SR", c:1, a:15, i:10, d1:90, bs:"鳴鳥の祝福"},
	"4447":{n:"兀突骨", r:"R", c:4, a:445, i:2, d1:470, bs:"国士無双"},
	"4448":{n:"鄒氏", r:"R", c:1, a:15, i:7, d1:70, bs:"鳴鳥の祝い"},
	"4452":{n:"花鬘・祝融", r:"L", c:4.5, a:800, i:8, d1:705, bs:"隣地極攻"},
	"4453":{n:"王異", r:"UR", c:4, a:415, i:11, d1:710, bs:"戦神の守勢"},
	"4454":{n:"于吉娘", r:"SR", c:4, a:330, i:20, d1:540, bs:"感謝の聖恵沢"},
	"4455":{n:"張角娘・左慈娘", r:"L", c:4.5, a:525, i:22, d1:915, bs:"娘々大神域"},
	"4456":{n:"南華娘", r:"L", c:4.5, a:515, i:24, d1:870, bs:"娘々大神域"},
	"4457":{n:"貂蝉", r:"L", c:2, a:100, i:12, d1:360, bs:"護国真心"},
	"4458":{n:"董白", r:"UR", c:4, a:390, i:11, d1:520, bs:"豊穣の神恵沢"},
	"4459":{n:"蔡琰", r:"SR", c:4, a:350, i:14, d1:450, bs:"豊穣の聖恵沢"},
	"4460":{n:"呂布", r:"SR", c:3.5, a:435, i:3, d1:475, bs:"鬼神突貫"},
	"4461":{n:"呂布・貂蝉", r:"L", c:4, a:690, i:19, d1:650, bs:"龍神の劫略令"},
	"4462":{n:"蔡夫人", r:"L", c:2, a:250, i:14, d1:300, bs:"副将の天明鏡"},
	"4463":{n:"厳氏", r:"L", c:4, a:450, i:16, d1:970, bs:"副将の天明鏡"},
	"4466":{n:"南華娘・龍", r:"L", c:4.5, a:570, i:24, d1:615, bs:"全軍の大極攻令"},
	"4467":{n:"南華娘・龍", r:"R", c:3, a:365, i:18, d1:375, bs:"闘将の劫略令"},
	"4468":{n:"袁紹", r:"UC", c:4, a:375, i:16, d1:410, bs:"鹵獲の猛攻"},
	"4469":{n:"馬騰", r:"UC", c:4, a:390, i:9, d1:405, bs:"飛将"},
	"4470":{n:"劉璋", r:"UC", c:4, a:270, i:14, d1:530, bs:"守護堅陣"},
	"4471":{n:"劉表", r:"UC", c:4, a:260, i:18, d1:425, bs:"全軍の強攻令"},
	"4472":{n:"董白", r:"L", c:4.5, a:510, i:13, d1:880, bs:"副将の天明鏡"},
	"4473":{n:"呂氏", r:"L", c:4.5, a:750, i:10, d1:670, bs:"副将の天明鏡"},
	"4474":{n:"劉曼", r:"L", c:4.5, a:520, i:27, d1:680, bs:"副将の天明鏡"},
	"4475":{n:"丁原", r:"UR", c:4, a:530, i:6, d1:510, bs:"鬼神の劫略令"},
	"4476":{n:"王美人", r:"UR", c:4, a:500, i:16, d1:550, bs:"副将の神明鏡"},
	"4477":{n:"馬騰", r:"L", c:4.5, a:755, i:14, d1:640, bs:"獣神の貫攻"},
	"4478":{n:"鄒氏", r:"L", c:1, a:170, i:12, d1:120, bs:"剴切の天砕器"},
	"4479":{n:"左慈娘", r:"L", c:4, a:600, i:22, d1:550, bs:"お菓子の攻奪"},
	"4480":{n:"張角娘", r:"L", c:4.5, a:770, i:22, d1:665, bs:"お菓子の劫略令"},
	"4481":{n:"南華娘・水鏡娘", r:"UR", c:4, a:565, i:16, d1:520, bs:"南瓜の劫略令"},
	"4482":{n:"王允", r:"L", c:4.5, a:500, i:28, d1:690, bs:"王佐の天資"},
	"4483":{n:"祝融", r:"R", c:3.5, a:430, i:4, d1:450, bs:"闘将の劫略令"},
	"4484":{n:"左慈娘", r:"R", c:3.5, a:390, i:20, d1:450, bs:"南瓜の攻奪"},
	"4485":{n:"董卓", r:"L", c:4.5, a:585, i:23, d1:970, bs:"錘兵の大天域"},
	"4486":{n:"孟獲", r:"L", c:4, a:720, i:6, d1:605, bs:"斧兵の大天撃"},
	"4487":{n:"王異", r:"UR", c:4, a:570, i:11, d1:535, bs:"錘兵烈攻"},
	"4488":{n:"胡車児", r:"SR", c:4, a:530, i:11, d1:455, bs:"双兵の極攻"},
	"4489":{n:"孟優", r:"SR", c:4, a:565, i:9, d1:450, bs:"斧兵の極攻"},
	"4490":{n:"紀霊", r:"R", c:3.5, a:460, i:7, d1:445, bs:"錘神降臨"},
	"4491":{n:"阿会喃", r:"R", c:3.5, a:445, i:7, d1:420, bs:"斧兵の猛攻"},
	"4492":{n:"韓遂", r:"UC", c:3, a:360, i:10, d1:330, bs:"斧兵の強撃"},
	"4493":{n:"陳宮", r:"UR", c:2, a:260, i:24, d1:290, bs:"剴切の神砕器"},
	"4494":{n:"馬元義", r:"R", c:4, a:440, i:15, d1:480, bs:"闘将の叛攻令"},
	"4495":{n:"董白", r:"R", c:3.5, a:400, i:9, d1:415, bs:"雪華の攻奪"},
	"4496":{n:"董卓・呂布・貂蝉", r:"L", c:1, a:250, i:11, d1:230, bs:"剴切の大極攻令"},
	"4497":{n:"水鏡娘・左慈娘", r:"UR", c:4, a:570, i:22, d1:530, bs:"鬼神の劫略令"},
	"4498":{n:"馬超・馬岱", r:"SR", c:4, a:500, i:7, d1:510, bs:"猛将の劫略令"},
	"4499":{n:"張角・張宝・張梁", r:"L", c:1, a:240, i:11, d1:210, bs:"剴切の天劫令"},
	"4500":{n:"公孫淵", r:"SR", c:4, a:445, i:14, d1:450, bs:"全兵の極攻令"},
	"4501":{n:"于吉娘", r:"L", c:4.5, a:470, i:23, d1:1110, bs:"娘々無縫"},
	"4502":{n:"呂氏", r:"R", c:3.5, a:420, i:4, d1:450, bs:"隣地豪襲"},
	"4503":{n:"于吉娘", r:"R", c:3, a:325, i:13, d1:340, bs:"万華招来"},
	"4504":{n:"張宝", r:"L", c:4, a:620, i:21, d1:605, bs:"龍神の叛攻令"},
	"4505":{n:"牛輔", r:"UR", c:4, a:545, i:11, d1:500, bs:"鬼神の叛攻令"},
	"4506":{n:"呂布・張遼", r:"L", c:4.5, a:835, i:15, d1:640, bs:"隣地極攻"},
	"4507":{n:"袁術・孫堅", r:"L", c:4.5, a:755, i:22, d1:685, bs:"龍神の攻奪"},
	"4508":{n:"盧氏", r:"L", c:2, a:255, i:18, d1:345, bs:"朱雀の啓示"},
	"4509":{n:"華雄", r:"UR", c:4, a:400, i:9, d1:760, bs:"龍神の守勢"},
	"4510":{n:"花鬘", r:"SR", c:4, a:395, i:8, d1:620, bs:"弓将の封聖域"},
	"4511":{n:"黄祖", r:"R", c:4, a:460, i:6, d1:460, bs:"弓兵烈攻"},
	"4512":{n:"紀霊", r:"R", c:2, a:245, i:7, d1:230, bs:"神速劫略"},
	"4513":{n:"何進", r:"R", c:4, a:305, i:6, d1:590, bs:"猛将の守勢"},
	"4514":{n:"王異", r:"L", c:4.5, a:755, i:13, d1:680, bs:"副将の天明鏡"},
	"4523":{n:"公孫瓚", r:"L", c:4, a:650, i:13, d1:530, bs:"龍神の攻奪"},
	"4524":{n:"厳白虎", r:"R", c:4, a:460, i:5, d1:435, bs:"斧兵突攻"},
	"4525":{n:"呂氏", r:"L", c:4, a:730, i:10, d1:930, bs:"覇王の帝護斬"},
	"4526":{n:"袁姫", r:"L", c:1, a:120, i:23, d1:145, bs:"副将の天明鏡"},
	"4527":{n:"郭図", r:"UR", c:4, a:550, i:24, d1:505, bs:"鬼神の叛攻令"},
	"4528":{n:"李傕", r:"R", c:4, a:420, i:3, d1:475, bs:"闘将の叛攻令"},
	"4530":{n:"水鏡娘・水鏡", r:"L", c:2, a:425, i:20, d1:350, bs:"剴切の天劫令"},
	"4531":{n:"パンダ（仮）", r:"L", c:1, a:160, i:15, d1:145, bs:"副将の天明鏡"},
	"4532":{n:"鄒氏", r:"UR", c:2, a:190, i:11, d1:285, bs:"副将の神明鏡"},
	"4533":{n:"鄒氏", r:"UR", c:2, a:215, i:11, d1:260, bs:"剣神の導き"},
	"4542":{n:"水鏡娘・パンダ", r:"L", c:4.5, a:760, i:21, d1:640, bs:"鋭兵の神遠征軍"},
	"4543":{n:"左慈娘・南華娘", r:"L", c:4, a:560, i:23, d1:565, bs:"攻城の神義勇軍"},
	"4544":{n:"于吉娘・張角娘", r:"L", c:4.5, a:800, i:27, d1:705, bs:"龍神の療略令"},
	"4547":{n:"娘々応援団★青", r:"L", c:4.5, a:765, i:21, d1:730, bs:"龍神の劫略令"},
	"4548":{n:"娘々応援団★紫", r:"L", c:4.5, a:670, i:22, d1:595, bs:"全軍の鳳速令"},
	"4549":{n:"娘々応援団★桃", r:"L", c:4.5, a:690, i:23, d1:1040, bs:"守護大聖陣"},
	"4550":{n:"娘々応援団★黄", r:"L", c:4.5, a:790, i:22, d1:665, bs:"龍神の叛攻令"},
	"4551":{n:"娘々応援団★緑", r:"L", c:4.5, a:680, i:21, d1:715, bs:"全軍の大極攻令"},
	"4552":{n:"水鏡娘・パンダ", r:"SR", c:3, a:400, i:19, d1:310, bs:"鬼神の攻奪"},
	"4553":{n:"左慈娘・南華娘", r:"SR", c:4, a:350, i:21, d1:415, bs:"攻城の檄文"},
	"4554":{n:"于吉娘・張角娘", r:"SR", c:4, a:365, i:27, d1:485, bs:"呉の治世"},
	"4560":{n:"花鬘", r:"UC", c:4, a:395, i:5, d1:400, bs:"武将の劫略令"},
	"4561":{n:"董白", r:"UC", c:4, a:400, i:7, d1:415, bs:"武将の叛攻令"},
	"4562":{n:"蔡琰", r:"UR", c:2, a:260, i:15, d1:290, bs:"副将の神明鏡"},
	"4563":{n:"左慈娘", r:"R", c:2, a:245, i:16, d1:240, bs:"猛将の貫攻"},
	"4564":{n:"蔡琰", r:"UR", c:1, a:30, i:15, d1:125, bs:"守備招来"},
	"4566":{n:"王美人・何皇后", r:"L", c:4.5, a:700, i:14, d1:680, bs:"鳳速雷光"},
	"4567":{n:"于吉娘", r:"R", c:1, a:40, i:8, d1:120, bs:"娘々敬慕"},
	"4568":{n:"沮授", r:"L", c:4.5, a:520, i:28, d1:815, bs:"錘兵の義勇軍"},
	"4569":{n:"パンダ（仮）", r:"SR", c:4, a:515, i:13, d1:455, bs:"仮装の療略令"},
	"4570":{n:"南華娘・水鏡娘", r:"SR", c:3.5, a:405, i:15, d1:445, bs:"南瓜の攻奪"},
	"4571":{n:"娘々聖歌隊", r:"L", c:4.5, a:810, i:23, d1:690, bs:"樅樹の劫略令"},
	"4572":{n:"南華娘", r:"L", c:4.5, a:530, i:24, d1:650, bs:"聖夜大転聖"},
	"4573":{n:"左慈娘", r:"L", c:4.5, a:555, i:22, d1:665, bs:"聖夜大転聖"},
	"4574":{n:"張角娘", r:"L", c:4.5, a:580, i:22, d1:730, bs:"聖夜大転聖"},
	"4575":{n:"水鏡娘・左慈娘", r:"SR", c:3, a:335, i:19, d1:445, bs:"守護大聖陣"},
	"4576":{n:"袁姫・袁術", r:"L", c:1, a:200, i:15, d1:170, bs:"剴切の大極攻令"},
	"4577":{n:"文醜", r:"UR", c:4, a:565, i:7, d1:475, bs:"龍神"},
	"4578":{n:"花鬘", r:"L", c:1, a:205, i:6, d1:140, bs:"副将の天明鏡"},
	"4579":{n:"呂布", r:"SL", c:4.5, a:950, i:7, d1:780, bs:"神威"},
	"4581":{n:"貂蝉・厳氏", r:"L", c:2, a:400, i:14, d1:260, bs:"剴切の天叛令"},
	"4582":{n:"盧氏", r:"L", c:1, a:120, i:17, d1:200, bs:"護国真心"},
	"4583":{n:"花鬘・祝融", r:"UR", c:2.5, a:270, i:7, d1:430, bs:"剴切聖陣"},
	"4584":{n:"張角娘・左慈娘", r:"UR", c:4, a:585, i:21, d1:525, bs:"鬼神の叛攻令"},
	"4585":{n:"張角娘", r:"SR", c:4, a:525, i:20, d1:470, bs:"猛将の療略令"},
	"4586":{n:"左慈娘", r:"R", c:3.5, a:380, i:19, d1:420, bs:"全軍の烈速令"},
	"4587":{n:"張角・波才", r:"L", c:4.5, a:670, i:23, d1:650, bs:"全軍の大極攻令"},
	"4588":{n:"王異", r:"R", c:4, a:390, i:7, d1:510, bs:"錘兵舎修練"},
	"4589":{n:"水鏡娘・パンダ", r:"L", c:4.5, a:745, i:21, d1:655, bs:"娘々大烈覇"},
	"4590":{n:"于吉娘・亀", r:"L", c:4.5, a:765, i:21, d1:670, bs:"斧兵の娘天撃"},
	"4591":{n:"左慈娘・鶴爺", r:"L", c:4.5, a:750, i:21, d1:660, bs:"双兵の娘天撃"},
	"4592":{n:"張角娘・虎", r:"L", c:4.5, a:770, i:22, d1:745, bs:"錘兵の娘天撃"},
	"4593":{n:"南華娘・龍", r:"L", c:4.5, a:740, i:24, d1:695, bs:"兵器の娘天撃"},
	"4594":{n:"盧氏", r:"UR", c:2, a:240, i:18, d1:225, bs:"錘神の導き"},
	"4595":{n:"水鏡", r:"SL", c:1, a:120, i:21, d1:140, bs:"水鏡達識"},
	"4596":{n:"曹操・袁紹", r:"SR", c:4, a:430, i:20, d1:565, bs:"加護方壁"},
	"4597":{n:"皇甫嵩", r:"SR", c:4, a:405, i:21, d1:665, bs:"加護方壁"},
	"5001":{n:"呂布×董卓", r:"UR", c:4, a:425, i:11, d1:420, bs:""},
	"5002":{n:"張角×何進", r:"SR", c:3.5, a:310, i:19, d1:400, bs:""},
	"5003":{n:"孫堅×黄祖", r:"R", c:3, a:270, i:9, d1:385, bs:""},
	"5004":{n:"水鏡娘×パンダ", r:"R", c:3, a:250, i:13, d1:355, bs:""},
	"5005":{n:"王允×張譲", r:"UR", c:4, a:365, i:25, d1:520, bs:""},
	"5006":{n:"張繍×典韋", r:"SR", c:3.5, a:335, i:11, d1:505, bs:""},
	"5007":{n:"公孫瓚×袁紹", r:"R", c:3, a:255, i:8, d1:340, bs:""},
	"5008":{n:"曹操×呂布", r:"UR", c:4, a:480, i:13, d1:385, bs:""},
	"5009":{n:"姜維×鄧艾", r:"SR", c:3.5, a:330, i:18, d1:500, bs:""},
	"5010":{n:"荀攸×陳宮", r:"R", c:3, a:200, i:19, d1:430, bs:""},
	"5011":{n:"水鏡娘×パンダ", r:"SR", c:3, a:270, i:13, d1:365, bs:""},
	"5012":{n:"孫策×太史慈", r:"UR", c:4, a:500, i:9, d1:480, bs:""},
	"5013":{n:"関羽×顔良", r:"SR", c:3.5, a:390, i:10, d1:425, bs:""},
	"5014":{n:"楽進×淳于瓊", r:"R", c:3, a:310, i:7, d1:325, bs:""},
	"5015":{n:"水鏡娘×パンダ", r:"UR", c:3.5, a:300, i:13, d1:420, bs:""},
	"5016":{n:"王異×馬超", r:"UR", c:4, a:490, i:10, d1:455, bs:"剣兵の大神撃"},
	"5017":{n:"賈詡×韓遂",	r:"SR", c:3.5, a:350, i:20, d1:390, bs:"才略謀略"},
	"5018":{n:"魏延×楊儀", r:"R", c:3, a:300, i:16, d1:310, bs:"謀反の猛攻"},
	"5019":{n:"大虎×小虎", r:"UR", c:4, a:440, i:10, d1:550, bs:""},
	"5020":{n:"貂蝉×董卓", r:"UR", c:2, a:220, i:17, d1:335, bs:""},
	"5021":{n:"左慈娘×水鏡娘", r:"UR", c:4, a:340, i:24, d1:605, bs:"守衛聖陣"},
	"5022":{n:"龐統×張任", r:"UR", c:4, a:405, i:24, d1:395, bs:"全軍の極攻令"},
	"5023":{n:"呂蒙×関羽", r:"SR", c:4, a:415, i:16, d1:415, bs:""},
	"5024":{n:"少帝×献帝", r:"UR", c:4, a:485, i:21, d1:490, bs:"皇帝の覇軍"},
	"5025":{n:"諸葛恪×諸葛誕", r:"UR", c:4, a:425, i:23, d1:555, bs:"鹵獲の極攻"},
	"5026":{n:"張飛×馬超", r:"UR", c:4, a:490, i:9, d1:390, bs:"鬼神突貫"},
	"5027":{n:"諸葛亮×孟獲", r:"R", c:3.5, a:325, i:19, d1:505, bs:""},
	"5028":{n:"王允×張譲", r:"R", c:3, a:235, i:15, d1:385, bs:""},
	"5029":{n:"徐庶×曹仁", r:"UR", c:4, a:480, i:23, d1:445, bs:"全軍の飛速令"},
	"5030":{n:"凌操×甘寧", r:"SR", c:3.5, a:405, i:10, d1:405, bs:"弓襲遠攻"},
	"5031":{n:"張郃×馬謖", r:"R", c:3, a:290, i:13, d1:320, bs:"烈速雷光"},
	"5032":{n:"曹丕×徐盛", r:"SR", c:4, a:320, i:19, d1:440, bs:"守衛方陣"},
	"5033":{n:"曹洪×呉蘭", r:"R", c:3, a:275, i:6, d1:335, bs:"鹵獲の猛攻"},
	"5034":{n:"水鏡娘×于吉娘", r:"UR", c:4, a:395, i:24, d1:570, bs:"全兵の極攻令"},
	"5035":{n:"曹彰×劉封", r:"UC", c:2.5, a:190, i:7, d1:240, bs:""},
	"5037":{n:"黄忠×夏侯淵", r:"UR", c:4, a:495, i:13, d1:500, bs:"戦神の極意"},
	"5038":{n:"馬岱×魏延", r:"SR", c:3.5, a:395, i:9, d1:450, bs:"謀反の極攻"},
	"5039":{n:"趙雲×夏侯恩", r:"R", c:3, a:310, i:12, d1:395, bs:"猛士の極意"},
	"5040":{n:"張郃×馬謖", r:"UR", c:4, a:420, i:17, d1:400, bs:"天里雷光"},
	"5041":{n:"盧植×張角", r:"UR", c:4, a:400, i:24, d1:540, bs:"鬼神の守勢"},
	"5042":{n:"張遼×凌統", r:"UR", c:4, a:510, i:14, d1:410, bs:"鬼神の貫攻"},
	"5043":{n:"朱儁×張宝", r:"SR", c:4, a:355, i:19, d1:540, bs:"猛将の守勢"},
	"5044":{n:"周瑜×曹仁", r:"SR", c:4, a:400, i:22, d1:455, bs:"攻城戦の猛砕"},
	"5045":{n:"皇甫嵩×張梁", r:"R", c:3.5, a:360, i:15, d1:420, bs:"飛将"},
	"5046":{n:"司馬昭×劉禅", r:"R", c:3.5, a:310, i:14, d1:485, bs:"闘将の守勢"},
	"5047":{n:"呂蒙×関羽",  r:"UR", c:4, a:500, i:17, d1:420, bs:"覇王の神賢斬"},
	"5048":{n:"大虎×小虎", r:"UR", c:4, a:485, i:10, d1:515, bs:"強襲奮迅"},
	"5049":{n:"沙摩柯×周泰", r:"SR", c:4, a:430, i:5, d1:460, bs:"猛将の貫攻"},
	"5050":{n:"関興×孫桓", r:"SR", c:4, a:410, i:13, d1:415, bs:"万里雷光"},
	"5051":{n:"張飛×張郃", r:"R", c:3.5, a:315, i:7, d1:350, bs:""},
	"5052":{n:"朱霊×公孫瓚", r:"UC", c:2.5, a:180, i:8, d1:180, bs:""},
	"5053":{n:"張繍×曹操", r:"SR", c:4, a:420, i:18, d1:465, bs:"謀反の烈攻"},
	"5054":{n:"張繍×曹操", r:"UR", c:4, a:470, i:19, d1:480, bs:"猛将の攻奪"},
	"5055":{n:"趙雲×夏侯恩", r:"UR", c:4, a:480, i:16, d1:420, bs:"覇王の神賢斬"},
	"5056":{n:"劉備×陸遜", r:"UR", c:4, a:450, i:25, d1:450, bs:"猛将の攻奪"},
	"5057":{n:"姜維×趙雲", r:"UR", c:4, a:440, i:21, d1:600, bs:"戦神の守勢"},
	"5058":{n:"諸葛亮×夏侯惇", r:"UR", c:4, a:445, i:24, d1:395, bs:"猛将の攻奪"},
	"5059":{n:"大虎×小虎", r:"R", c:3.5, a:310, i:8, d1:380, bs:"闘将の劫略令"},
	"5060":{n:"左慈娘×水鏡娘", r:"R", c:3.5, a:300, i:20, d1:535, bs:"守護方陣"},
	"5061":{n:"孫堅×劉表", r:"UR", c:4, a:400, i:27, d1:490, bs:"智将砕器"},
	"5062":{n:"甘寧×関羽", r:"UR", c:4, a:475, i:14, d1:410, bs:"猛将の攻奪"},
	"5063":{n:"馬忠×祝融", r:"UR", c:4, a:460, i:10, d1:420, bs:"猛将の攻奪"},
	"5064":{n:"張繍×曹操", r:"R", c:3.5, a:375, i:14, d1:330, bs:"弓襲遠攻"},
	"5065":{n:"大虎×小虎", r:"SR", c:4, a:425, i:9, d1:475, bs:"玩具の叛攻令"},
	"5066":{n:"左慈娘×水鏡娘", r:"SR", c:4, a:330, i:23, d1:575, bs:"戦神の守勢"},
	"5067":{n:"典韋×呂布", r:"UR", c:4, a:520, i:7, d1:450, bs:"斧兵の大神撃"},
	"5068":{n:"趙雲×文醜", r:"UR", c:4, a:500, i:14, d1:445, bs:"双兵の大神撃"},
	"5069":{n:"許褚×徐晃", r:"UR", c:4, a:540, i:13, d1:500, bs:"錘兵の大神撃"},
	"5070":{n:"孟獲×関索", r:"SR", c:4, a:445, i:7, d1:435, bs:"鬼神突貫"},
	"5071":{n:"張飛×厳顔", r:"SR", c:4, a:420, i:12, d1:430, bs:"猛将の劫略令"},
	"5072":{n:"兀突骨×魏延", r:"SR", c:4, a:460, i:9, d1:480, bs:"鬼神の極意"},
	"5073":{n:"華雄×祖茂", r:"R", c:3, a:340, i:7, d1:315, bs:"猛将の貫攻"},
	"5074":{n:"甘寧×曹仁", r:"R", c:3, a:315, i:8, d1:310, bs:"飛将"},
	"5075":{n:"韓当×黄祖", r:"R", c:3, a:350, i:6, d1:350, bs:"猛将の極意"},
	"5076":{n:"少帝×献帝", r:"SR", c:3.5, a:400, i:19, d1:425, bs:"鬼神の攻奪"},
	"5077":{n:"李典×劉備", r:"UR", c:4, a:500, i:18, d1:400, bs:"鬼神の攻奪"},
	"5078":{n:"関平×龐徳", r:"UR", c:4, a:480, i:13, d1:475, bs:"鬼神の攻奪"},
	"5079":{n:"郭淮×廖化", r:"R", c:3.5, a:390, i:13, d1:385, bs:"猛将の攻奪"},
	"5080":{n:"孫堅×董卓", r:"UR", c:4, a:510, i:20, d1:545, bs:"鬼神の攻奪"},
	"5081":{n:"文醜×公孫瓚", r:"R", c:3.5, a:380, i:7, d1:350, bs:"猛将の攻奪"},
	"5082":{n:"劉備×劉璋", r:"R", c:3.5, a:355, i:16, d1:335, bs:"猛将の攻奪"},
	"5083":{n:"大虎×小虎", r:"SR", c:4, a:380, i:9, d1:435, bs:"全兵の極攻令"},
	"5084":{n:"水鏡娘×于吉娘", r:"SR", c:4, a:390, i:22, d1:480, bs:"兵器の極攻"},
	"5085":{n:"関羽×夏侯惇", r:"UR", c:4, a:500, i:14, d1:430, bs:"龍神の貫攻"},
	"5086":{n:"諸葛誕×文欽", r:"SR", c:2, a:200, i:21, d1:210, bs:"智将砕器"},
	"8001":{n:"小指南パンダ", r:"UC", c:1, a:10, i:10, d1:45, bs:"覇王の猛攻"}
};

//--------------------------------------------------------
// カード表示拡張
//--------------------------------------------------------
jQuery.noConflict();
j$ = jQuery;

const SukesukeFlg = 1;

const lv_list = [
	[	14	 ,	 80  ,	 16  ,	 03  ,	1 ] ,
	[	40	 ,	 71  ,	 21  ,	 06  ,	2 ] ,
	[	66	 ,	 62  ,	 25  ,	 10  ,	3 ] ,
	[	92	 ,	 53  ,	 27  ,	 13  ,	7 ] ,
	[	118	 ,	 46  ,	 28  ,	 17  ,	9 ] ,
	[	144	 ,	 39  ,	 28  ,	 19  ,	 14 ] ,
	[	170	 ,	 34  ,	 27  ,	 22  ,	 17 ] ,
	[	196	 ,	 29  ,	 26  ,	 24  ,	 21 ] ,
	[	224	 ,	 26  ,	 24  ,	 21  ,	 29 ] ,
	[	250	 ,	 24  ,	 22  ,	 20  ,	 34 ] ,
	[	276	 ,	 23  ,	 20  ,	 18  ,	 39 ] ,
	[	302	 ,	 20  ,	 18  ,	 16  ,	 46 ] ,
	[	328	 ,	 17  ,	 16  ,	 14  ,	 53 ] ,
	[	999	 ,	 17  ,	 16  ,	 14  ,	 53 ] ,
];

const scr_list = [
	[		25 , 66.67	 ],  [	   225 , 66.56	 ],  [	   625 , 66.44	 ],  [	  1225 , 66.33	 ],  [	  2025 , 66.22	 ],  [	  3025 , 66.10	 ],  [	  4225 , 65.99	 ],  [	  5625 , 65.87	 ],
	[	  7225 , 65.75	 ],  [	  9025 , 65.64	 ],  [	 11025 , 65.52	 ],  [	 13225 , 65.40	 ],  [	 15625 , 65.28	 ],  [	 18225 , 65.16	 ],  [	 21025 , 65.03	 ],  [	 24025 , 64.91	 ],
	[	 27225 , 64.79	 ],  [	 30625 , 64.66	 ],  [	 34225 , 64.54	 ],  [	 38025 , 64.41	 ],  [	 42025 , 64.29	 ],  [	 46225 , 64.16	 ],  [	 50625 , 64.03	 ],  [	 55225 , 63.90	 ],
	[	 60025 , 63.77	 ],  [	 65025 , 63.64	 ],  [	 70225 , 63.50	 ],  [	 75625 , 63.37	 ],  [	 81225 , 63.24	 ],  [	 87025 , 63.10	 ],  [	 93025 , 62.96	 ],  [	 99225 , 62.83	 ],
	[	105625 , 62.69	 ],  [	112225 , 62.55	 ],  [	119025 , 62.41	 ],  [	126025 , 62.26	 ],  [	133225 , 62.12	 ],  [	140625 , 61.98	 ],  [	148225 , 61.83	 ],  [	156025 , 61.69	 ],
	[	164025 , 61.54	 ],  [	172225 , 61.39	 ],  [	180625 , 61.24	 ],  [	189225 , 61.09	 ],  [	198025 , 60.94	 ],  [	207025 , 60.78	 ],  [	216225 , 60.63	 ],  [	225625 , 60.48	 ],
	[	235225 , 60.32	 ],  [	245025 , 60.16	 ],  [	255025 , 60.00	 ],  [	265225 , 59.84	 ],  [	275625 , 59.68	 ],  [	286225 , 59.51	 ],  [	297025 , 59.35	 ],  [	308025 , 59.19	 ],
	[	319225 , 59.02	 ],  [	330625 , 58.85	 ],  [	342225 , 58.68	 ],  [	354025 , 58.51	 ],  [	366025 , 58.33	 ],  [	378225 , 58.16	 ],  [	390625 , 57.98	 ],  [	403225 , 57.81	 ],
	[	416025 , 57.63	 ],  [	429025 , 57.45	 ],  [	442225 , 57.26	 ],  [	455625 , 57.08	 ],  [	469225 , 56.90	 ],  [	483025 , 56.71	 ],  [	497025 , 56.52	 ],  [	511225 , 56.33	 ],
	[	525625 , 56.14	 ],  [	540225 , 55.95	 ],  [	555025 , 55.76	 ],  [	570025 , 55.56	 ],  [	585225 , 55.36	 ],  [	600625 , 55.16	 ],  [	616225 , 54.95	 ],  [	632025 , 54.75	 ],
	[	648025 , 54.55	 ],  [	664225 , 54.34	 ],  [	680625 , 54.13	 ],  [	697225 , 53.92	 ],  [	714025 , 53.71	 ],  [	731025 , 53.49	 ],  [	748225 , 53.27	 ],  [	765625 , 53.05	 ],
	[	783225 , 52.83	 ],  [	801025 , 52.61	 ],  [	819025 , 52.38	 ],  [	837225 , 52.15	 ],  [	855625 , 51.92	 ],  [	874225 , 51.69	 ],  [	893025 , 51.46	 ],  [	912025 , 51.22	 ],
	[	931225 , 50.98	 ],  [	950625 , 50.74	 ],  [	970225 , 50.50	 ],  [	990025 , 50.25	 ],  [	 1010025 , 50.00	 ],  [	 1030225 , 49.75	 ],  [	 1050625 , 49.49	 ],  [	 1071225 , 49.23	 ],
	[	1092025 , 48.97	 ],  [	 1113025 , 48.71	 ],  [	 1134225 , 48.45	 ],  [	 1155625 , 48.18	 ],  [	 1177225 , 47.91	 ],  [	 1199025 , 47.64	 ],  [	 1221025 , 47.36	 ],  [	 1243225 , 47.08	 ],
	[	1265625 , 46.80	 ],  [	 1288225 , 46.52	 ],  [	 1311025 , 46.23	 ],  [	 1334025 , 45.93	 ],  [	 1357225 , 45.62	 ],  [	 1380625 , 45.31	 ],  [	 1404225 , 45.00	 ],  [	 1428025 , 44.68	 ],
	[	1452025 , 44.36	 ],  [	 1476225 , 44.03	 ],  [	 1500625 , 43.70	 ],  [	 1525225 , 43.36	 ],  [	 1550025 , 43.02	 ],  [	 1575025 , 42.68	 ],  [	 1600225 , 42.33	 ],  [	 1625625 , 41.98	 ],
	[	1651225 , 41.62	 ],  [	 1677025 , 41.26	 ],  [	 1703025 , 40.89	 ],  [	 1729225 , 40.52	 ],  [	 1755625 , 40.14	 ],  [	 1782225 , 39.76	 ],  [	 1809025 , 39.37	 ],  [	 1836025 , 38.98	 ],
	[	1863225 , 38.58	 ],  [	 1890625 , 38.18	 ],  [	 1918225 , 37.77	 ],  [	 1946025 , 37.36	 ],  [	 1974025 , 36.94	 ],  [	 2002225 , 36.51	 ],  [	 2030625 , 36.09	 ],  [	 2059225 , 35.65	 ],
	[	2088025 , 35.21	 ],  [	 2117025 , 34.77	 ],  [	 2146225 , 34.32	 ],  [	 2175625 , 33.86	 ],  [	 2205225 , 33.40	 ],  [	 2235025 , 32.93	 ],  [	 2265025 , 32.46	 ],  [	 2295225 , 31.98	 ],
	[	2325625 , 31.50	 ],  [	 2356225 , 31.01	 ],  [	 2387025 , 30.52	 ],  [	 2418025 , 30.02	 ],  [	 2449225 , 29.52	 ],  [	 2480625 , 29.01	 ],  [	 2512225 , 28.49	 ],  [	 2544025 , 27.97	 ],
	[	2576025 , 27.45	 ],  [	 2608225 , 26.92	 ],  [	 2640625 , 26.38	 ],  [	 2673225 , 25.84	 ],  [	 2706025 , 25.30	 ],  [	 2739025 , 24.74	 ],  [	 2772225 , 24.19	 ],  [	 2805625 , 23.63	 ],
	[	2839225 , 23.07	 ],  [	 2873025 , 22.50	 ],  [	 2907025 , 21.92	 ],  [	 2941225 , 21.35	 ],  [	 2975625 , 20.77	 ],  [	 3010225 , 20.18	 ],  [	 3045025 , 19.59	 ],  [	 3080025 , 19.00	 ],
	[	3115225 , 18.40	 ],  [	 3150625 , 17.80	 ],  [	 3186225 , 17.20	 ],  [	 3222025 , 16.60	 ],  [	 3258025 , 15.99	 ],  [	 3294225 , 15.38	 ],  [	 3330625 , 14.76	 ],  [	 3367225 , 14.15	 ],
	[	3404025 , 13.53	 ],  [	 3441025 , 12.91	 ],  [	 3478225 , 12.28	 ],  [	 3515625 , 11.65	 ],  [	 3553225 , 11.01	 ],  [	 3591025 , 10.37	 ],  [	 3629025 ,	9.73	 ],  [	 3667225 ,	9.09	 ],
	[	3705625 ,  9.09	 ],  [	 3744225 ,	9.09	 ],  [	 3783025 ,	9.09	 ],  [	 3822025 ,	9.09	 ],  [	 3861225 ,	9.09	 ],  [	 3900625 ,	9.09	 ],  [	 3940225 ,	9.09	 ],  [	 3980025 ,	9.09	 ],
	[	4020025 ,  9.09	 ],  [	 4060225 ,	9.09	 ],  [	 4100625 ,	9.09	 ],  [	 4141225 ,	9.09	 ],  [	 4182025 ,	9.09	 ],  [	 4223025 ,	9.09	 ],  [	 4264225 ,	9.09	 ],  [	 4305625 ,	9.09	 ],
	[	4347225 ,  9.09	 ],  [	 4389025 ,	9.09	 ],  [	 4431025 ,	9.09	 ],  [	 4473225 ,	9.09	 ],  [	 4515625 ,	9.09	 ],  [	 4558225 ,	9.09	 ],  [	 4601025 ,	9.09	 ],  [	 4644025 ,	9.09	 ],
	[	4687225 ,  9.09	 ],  [	 4730625 ,	9.09	 ],  [	 4774225 ,	9.09	 ],  [	 4818025 ,	9.09	 ],  [	 4862025 ,	9.09	 ],  [	 4906225 ,	9.09	 ],  [	 4950625 ,	9.09	 ],  [	 4995225 ,	9.09	 ],
	[	5040025 ,  9.09	 ],  [	 999999999 ,  9.09	 ]
];

const isFirefox = window.navigator.userAgent.toLowerCase().indexOf('firefox') != -1;

( function() {

	// 所持スキル表示
	if(    location.pathname == "/user/"
		|| location.pathname == "/card/deck.php"
		|| location.pathname == "/card/trade_card.php"
		|| location.pathname.match(/union/)
		|| location.pathname == "/busyodas/busyodas_lineup.php"
	) {
		bro3dasuDrawSkill();
	}

	// 合成スキル表示
	if(    location.pathname == "/union/learn.php"
		|| location.pathname == "/busyodas/busyodas_continuty_result.php"
		|| location.pathname == "/busyodas/busyodas_result.php"
		|| location.pathname == "/card/trade.php"
		|| location.pathname == "/card/trade_card.php"
		|| location.pathname == "/card/exhibit_list.php"
		|| location.pathname == "/card/deck.php"
		|| location.pathname == "/card/set_subgeneral.php"
		|| location.pathname.match(/union/)
	){
		bro3dasuDrawCompositionSkill();
	}

})();

//--------------------------------------
// 武将スキル表示
//--------------------------------------
function bro3dasuDrawSkill() {

	if (location.pathname.match(/union/) == null) {
		// スキル枠
		if (location.pathname == "/busyodas/busyodas_lineup.php") {
			// ブショーダス ラインナップ画面
			j$("[id^=lineupCardFront]").each(function(){
				var frontId = j$(this).attr("id");

				// 指南パンダの場合、元画像にスキル名が載っているので枠自体を描画しない
				var aId = frontId.replace(/lineupCardFront/, 'lineupCardBack') + "_link_cover";
				var alt = ""
				j$("img.lazy", j$("#" + aId)).each(function(){
					alt += j$(this).attr("alt");
				});

				if (alt.indexOf('指南パンダ') === -1) {
					var frameId = "skillFrame_" + frontId;
					j$(this).after(
						'<span style="position:relative; top:-90px;">' +
							'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; left:0px; z-index:1; width:75%;">' +
							'<ul id="' + frameId + '" style="position:relative; top:-62px; left:4px; font-size:10px; z-index:2; color:white;"></ul>' +
						'</span>'
					);
				}
			});
		}
		else
		if (location.pathname == "/card/trade_card.php") {
			j$("div[class*=card rarerity] span[class=status_frontback]").parent().after(
				'<span style="position:relative; top:-297px;">' +
					'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; left:6px; z-index:-1">' +
					'<ul id="skillFrame" style="position:relative; top:-82px; left:14px; font-size:12px;"></ul>' +
				'</span>'
			);
		} else if (location.pathname == "/card/deck.php") {
			if (! isFirefox) {
				j$("div[class*=card rarerity] span[class=status_frontback]").parent().prepend(
					'<span style="position:relative; top:24px;">' +
						'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; left:6px; z-index:-1">' +
						'<ul id="skillFrame" style="position:relative; top:-82px; left:14px; font-size:12px;"></ul>' +
					'</span>'
				);
			} else {
				j$("div[class*=card rarerity] span[class=status_frontback]").parent().after(
					'<span style="position:relative; top:-293px;">' +
						'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; left:6px; z-index:-1">' +
						'<ul id="skillFrame" style="position:relative; top:-82px; left:14px; font-size:12px;"></ul>' +
					'</span>'
				);
			}
		} else {
			j$("div[class*=card rarerity] span[class=status_frontback]").parent().after(
				'<span style="position:relative; top:-125px;">' +
					'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; left:6px; z-index:-1">' +
					'<ul id="skillFrame" style="position:relative; top:-82px; left:14px; font-size:12px;"></ul>' +
				'</span>'
			);
		}
	}

	// デッキ特別処理
	if( location.pathname == "/card/deck.php"){
		// ステータス強化ボタンの位置を移動
		j$("img[class=levelup]").css({"position":"relative", "top":"-160px", "left":"6px"});

		// スケスケ指定時はステータス強化ボタンを透明化
		if(SukesukeFlg == 1){
			j$("img[class=levelup]").css("opacity", "0.5");
		}
	}

	// 所持スキルを表示
	if( location.pathname == "/user/" || location.pathname == "/card/deck.php"	|| location.pathname == "/card/trade_card.php"){
		// プロフィール画面、デッキ画面
		j$("div[class*=cardWrapper]").each(
			function(){
				var skillText = "";
				j$("ul[class^=back_skill] span[class*=skillName]", this).each(
					function(){
						skillText = skillText + "<li style='margin-top:1px !important;'>" + j$(this).text() + "</li>";
					}
				);
				j$("#skillFrame", this).html(skillText);
			}
		);
	} else if(location.pathname.match(/union/)) {
		// 合成画面
		if(location.pathname == "/union/learn.php") {
			j$("div[class*=cardWrapper] ul[class=front_skill]").remove();
		}
	}
	else if (location.pathname == "/busyodas/busyodas_lineup.php") {
		// ブショーダス ラインナップ画面
		j$("[id^=lineupCardFront]").each(function(){
			var frontId = j$(this).attr("id");
			var backId = frontId.replace(/lineupCardFront/, 'lineupCardBack');
			var backObj = j$("#" + backId);
			var skillText = "";
			j$(".back_skill .skill_name", backObj).each(function(){
				skillText += "<li>" + j$(this).text() + "</li>";
			});

			// ダイヤモンドEXラインナップ画面の表示崩れ対策でセレクターを分ける
			if (skillText.length === 0) {
				// SL対応
				j$(".back_skill5 .skill_name", backObj).each(function(){
					skillText += "<li>" + j$(this).text() + "</li>";
				});
			}

			j$("#skillFrame_" + frontId).html(skillText);
		});
	}
}

//----------------------------------
// 合成スキル表示
//----------------------------------
function bro3dasuDrawCompositionSkill() {
	// 合成スキル枠
	if (location.pathname == "/union/learn.php") {
		j$("div[id=card_uraomote] div[class*=card rarerity] span[class=status_frontback]").parent().after(
			'<span style="position:relative; top:-194px;">' +
				'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; top:70px; left:6px; z-index:-1">' +
				'<ul id="compositFrame" style="position:relative; top:-10px; left:14px; font-size:12px;"></ul>' +
			'</span>'
		);
	} else if (location.pathname == "/busyodas/busyodas_result.php" || location.pathname == "/busyodas/busyodas_continuty_result.php"){
		j$("div[class=getCardInfoArea] div[class*=card rarerity] span[class=status_frontback]").parent().after(
			'<span style="position:relative; top:-125px;">' +
				'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; left:6px; z-index:-1">' +
				'<ul id="compositFrame" style="position:relative; top:-82px; left:14px; font-size:12px;"></ul>' +
			'</span>'
		);
	} else if (location.pathname == "/card/trade.php"){
		j$("div[class*=card rarerity] span[class=status_frontback]").parent().after(
			'<span style="position:relative; top:-125px;">' +
				'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; left:6px; z-index:-1">' +
				'<ul id="compositFrame" style="position:relative; top:-82px; left:14px; font-size:12px;"></ul>' +
			'</span>'
		);
	} else if (location.pathname == "/card/trade_card.php"){
		j$("div[class*=card rarerity] span[class=status_frontback]").parent().after(
			'<span style="position:relative; top:-227px;">' +
				'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; left:6px; z-index:-1">' +
				'<ul id="compositFrame" style="position:relative; top:-82px; left:14px; font-size:12px;"></ul>' +
			'</span>'
		);
	} else {
		if (! isFirefox) {
			// Google Chrome でスキル LVUP ボタンが押せなくなる対策
			var items = j$("div[class*=card rarerity] ul[class=front_skill]");
			j$.each(items, function(i){
				var item = j$(this);
				var count = j$("li", item).length;
				var offset = 90 + 21 * count + (!count * -5);
				item.append(
					'<span style="position:relative; top:-'+offset+'px; left:-6px;">' +
						'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; left:6px; z-index:-1">' +
						'<ul id="compositFrame" style="position:relative; top:-82px; left:6px; font-size:12px;"></ul>' +
					'</span>'
				);
			});
			if (items.length == 0) {
				j$("div[class*=card rarerity] span[class=status_frontback]").parent().prepend(
					'<span style="position:relative; top:108px;">' +
						'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; left:6px; z-index:-1">' +
						'<ul id="compositFrame" style="position:relative; top:-82px; left:14px; font-size:12px;"></ul>' +
					'</span>'
				);
			}
		} else {
			j$("div[class*=card rarerity] span[class=status_frontback]").parent().after(
				'<span style="position:relative; top:-211px;">' +
					'<img src="http://' + location.hostname + '/20141224-01/extend_project/w945/img/card/common/bg_card_skill.png" style="position:relative; left:6px; z-index:-1">' +
					'<ul id="compositFrame" style="position:relative; top:-82px; left:14px; font-size:12px;"></ul>' +
				'</span>'
			);
		}
	}

	// 合成スキル算出
	if (location.pathname != "/union/learn.php") {
		function func(){
			var skill1NameInfo = j$("ul[class^=back_skill] span[class*=skillName1 skill_name]", this);
			var cardName = j$("div[class*=omote_4sk] div[class=name] span[class=name]", this).text();
			if (skill1NameInfo.length != 0) {
				var compositionText = "";
				var skill1Info = skill1NameInfo.text().match(/[:：](.*)LV/);
				if (skill1Info == null) {
					compositionText = "<li style='margin-top:3px !important;'>合成表なし</li><li style='margin-top:3px !important;'>　</li><li style='margin-top:3px !important;'>　</li><li style='margin-top:3px !important;'>　</li>";
				} else {
					var skill1 = RegExp.$1.replace(/(^\s+)|(\s+$)/g, "");;
					if (typeof gousei_skill[skill1] != 'undefined') {
						for(var i = 0; i <=3; i++){
							var compSkill = gousei_skill[skill1][i];
							compositionText = compositionText + "<li style='margin-top:1px !important;'>" + gousei_skill[skill1][i] + "</li>";
						}
					} else {
						compositionText = "<li style='margin-top:1px !important;'>合成表なし</li><li style='margin-top:3px !important;'>　</li><li style='margin-top:3px !important;'>　</li><li style='margin-top:3px !important;'>　</li>";
					}

					if (cardName.indexOf("パンダ") > 0) {
						compositionText = "<li style='margin-top:1px !important;'>" + skill1 + "</li><li style='margin-top:3px !important;'>　</li><li style='margin-top:3px !important;'>　</li><li style='margin-top:3px !important;'>　</li>";
					}
				}
				j$("#compositFrame", this).html(compositionText);
			}
		};
		j$("div[class*=cardWrapper]").each(func);
		j$("div[class*=card_wrapper_v2]").each(func);
	} else {
		// 付与合成

		// ベースカードの情報取得
		var baseLv = j$("#bacecard div[class=level] span").text();
		var baseScoreInfo = j$("#bacecard span[class=score]").text().match(/スコア:(\d+)$/);
		var baseScore = RegExp.$1;
		var baseSkill = [];
		j$("#bacecard ul[class^=back_skill] span[class*=skillName]").each(
			function(){
				var skillText = j$(this).text().match(/[：:](.*)LV/);
				if(skillText != null){
					baseSkill.push(RegExp.$1.replace(/(^\s+)|(\s+$)/g, ""));
				}
			}
		);

		// 素材カードの情報取得
		j$("#cardFileList div[class=cardColmn]").each(
			function() {
				var cardNo = j$("span[class=cardno]", this).text();
				var cardName = j$("span[class=name]", this).text();
				var level = j$("div[class=level] span", this).text();
				var scoreInfo = j$("span[class=score]", this).text().match(/スコア:(\d+)$/);
				var score = RegExp.$1;
				var att = parseFloat(j$("li[class=status_att]", this).text());
				var wdef = parseFloat(j$("li[class=status_wdef]", this).text());
				var int = parseFloat(j$("li[class=status_int]", this).text());
				var skillName1Info = j$("span[class*=skillName1]", this).text().match(/^[^:]+:(.+)LV\d+$/);
				var skillName1 = RegExp.$1;
				// 合成確率算出
				var compositionText = "";
				var res = calcCompositionSkillRate(baseLv, baseScore, baseSkill, cardNo, cardName, level, score, att, wdef, int, skillName1);
				if(res == null) {
					compositionText = "<li style='margin-top:3px !important;'>合成表なし</li><li style='margin-top:3px !important;'></li><li style='margin-top:3px !important;'></li><li style='margin-top:3px !important;'></li>";
				} else {
					var result = res[0];
					var probSuffix = res[1] ? "(!)" : "";
					for(var i = 0; i < result.length; i++) {
						compositionText = compositionText + "<li style='margin-top:2px !important; font-size:8pt;'>" + result[i].n + ":" + result[i].skill + "&nbsp;" + result[i].probability + "%" + probSuffix + "</li>";
					}
				}

				// 画面描画
				j$("#compositFrame", this).html(compositionText);
				j$("div[class=control]", this).css("z-index", "5000");
			}
		);
	}
}

//----------------------------------
// 合成スキル確率算出
//----------------------------------
function calcCompositionSkillRate(baseLv, baseScore, baseSkills, useCardNo, useCardName, useLv, useScore, useAtk, useDef, useInt, useSkillName1) {
	var noCardInfo = false;

	// 基礎確率の算出
	var useCardInfo = card_list[useCardNo];
	if (typeof useCardInfo == 'undefined') {
		if (typeof gousei_skill[useSkillName1] == 'undefined') {
			return null;
		} else {
			noCardInfo = true;
			useCardInfo = {n:useCardName, a:0, i:0, d1:0, bs:useSkillName1};
		}
	}

	// 合計LV・スコアの計算
	var total_lv	= parseInt(baseLv) + parseInt(useLv);
	var total_score = parseInt(baseScore) + parseInt(useScore);

	var score_table = 0;
	var lv_table	= 0;
	for (var x = 0; x < scr_list.length; x++) {
		if (scr_list[x][0] > total_score) {
			break;
		}
	}
	score_table = x;

	for (var x = 0; x < lv_list.length; x++) {
		if (lv_list[x][0] > total_lv) {
			break;
		}
	}

	lv_table = x;
	var probability = [ 0.00, 0.00, 0.00 ,0.00 ];
	probability[0] = (parseFloat( (1 - parseFloat(scr_list[score_table][1] / 100)) * parseFloat(lv_list[lv_table][1] / 100)) * 100).toFixed(2);	// 大
	probability[1] = (parseFloat( (1 - parseFloat(scr_list[score_table][1] / 100)) * parseFloat(lv_list[lv_table][2] / 100)) * 100).toFixed(2);	// 中
	probability[2] = (parseFloat( (1 - parseFloat(scr_list[score_table][1] / 100)) * parseFloat(lv_list[lv_table][3] / 100)) * 100).toFixed(2);	// 小
	probability[3] = (parseFloat( (1 - parseFloat(scr_list[score_table][1] / 100)) * parseFloat(lv_list[lv_table][4] / 100)) * 100).toFixed(2);	// 隠

	// 1ポイントあたりのステータス増分計算
	var pointAtk = parseFloat(useCardInfo.a)  * 0.094;		// 攻撃
	var pointDef = parseFloat(useCardInfo.d1) * 0.094;		// 防御
	var pointInt = parseFloat(useCardInfo.i)  * 0.0016;		// 知力

	// 実際に割り振ったポイント数の計算
	var AlloPointAtk = parseInt((useAtk - parseFloat(useCardInfo.a))  / pointAtk + 0.5);		// 攻撃
	var AlloPointDef = parseInt((useDef - parseFloat(useCardInfo.d1)) / pointDef + 0.5);		// 防御
	var AlloPointInt = parseInt((useInt - parseFloat(useCardInfo.i))  / pointInt + 0.5);		// 知力

	// 合成レシピ並び替え用配列
	var SkillArray = [];

	// 合成レシピと割り当てポイント数を配列に入れる
	if (useCardNo > 9000) {
		var panda_probability = 100;
		if (useCardName.indexOf("中指南") >= 0) {
			panda_probability = 50;
		} else if(useCardName.indexOf("小指南") >= 0) {
			panda_probability = 25;
		}
		for (var k = 0; k < 4; k++ ){
			probability[k] = panda_probability;
		}

		SkillArray.push( {"point":0, "n":"攻", "skill": useCardInfo.bs, "probability":0.00, "flg":3 });		// 攻撃
		SkillArray.push( {"point":0, "n":"防", "skill": useCardInfo.bs, "probability":0.00, "flg":2 });		// 防御
		SkillArray.push( {"point":0, "n":"知", "skill": useCardInfo.bs, "probability":0.00, "flg":1 });		// 知力
		SkillArray.push( {"point":0, "n":"隠", "skill": useCardInfo.bs, "probability":0.00, "flg":0 });		//
	}
	else {
		SkillArray.push( {"point":AlloPointAtk, "n":"攻", "skill": gousei_skill[useCardInfo.bs][0], "probability":0.00, "flg":3 });		// 攻撃
		SkillArray.push( {"point":AlloPointDef, "n":"防", "skill": gousei_skill[useCardInfo.bs][1], "probability":0.00, "flg":2 });		// 防御
		SkillArray.push( {"point":AlloPointInt, "n":"知", "skill": gousei_skill[useCardInfo.bs][2], "probability":0.00, "flg":1 });		// 知力
		SkillArray.push( {"point":0,			"n":"隠", "skill": gousei_skill[useCardInfo.bs][3], "probability":0.00, "flg":0 });		//
	}

	SkillArray.sort(
		function(a,b){
			if(a.point < b.point) return 1;
			if(a.point > b.point) return -1;
			if(a.flg   < b.flg	) return 1;
			if(a.flg   > b.flg	) return -1;
			return 0;
		}
	);

	// 合成確率を代入
	for (var x = 0; x < 3; x++) {
		SkillArray[x].probability = parseFloat(probability[x]);
	}
	// 合成確率でソート
	SkillArray.sort(
		function(a,b){
			if(a.probability < b.probability) return 1;
			if(a.probability > b.probability) return -1;
			return 0;
		}
	);
	// 隠しの合成確率は最後に代入 2013.01.18 追加
	SkillArray[3].probability = parseFloat(probability[3]);

	// 合成元カードに存在するスキルは表示しない
	var result = [];
	for (var x = 0; x < 4; x++) {
		var found = false;
		for (var j = 0; j < baseSkills.length; j++) {
			if(SkillArray[x].skill == baseSkills[j]) {
				found = true;
				break;
			}
		}
		if(found == false){
			result.push(SkillArray[x]);
		}
	}

	return [result, noCardInfo];
}
